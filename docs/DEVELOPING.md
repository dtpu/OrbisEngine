...
Operating detail for people working in this repo. Start with the [README](../README.md).
Code identifiers, env vars and the package still use the project's old name, Wander.
...
[shared assets](shared-assets.md) for credentials, publishing, and recovery.
...
[interactive bottle prototype](interactive-world.md) for manifest, head-anchor, and
verification limits.
...
To speed up first visits, [prepare the worlds on the Mac](shared-assets.md#prepared-world-cache)
...
For a teammate running reconstruction, follow [author credential setup](shared-assets.md#author-credentials):
...
controlled comparison. See [the kitchen review](kitchen-review.md#video-first-policy) and
[live kitchen/gym results](video-input-evaluation.md). The real video path works, but the new
...
accept a world. See [quality judging](quality-judging.md) for the review contract and remaining
...
[paid stage accounting and recovery](paid-recovery.md) before retrying an interrupted run.
...
remain untracked. See the [code map](#code-map) and [object packaging](objects.md).

For an unattended improvement run, write the run brief and follow
[unattended runs](unattended-runs.md). It covers recovering source clips from S3,
...
Compact animation is lossless by default; see [person motion](person-motion.md) for integrity
checks, original-PLY fallback and explicit quantization opt-in. The [kitchen review](kitchen-review.md)
...
The [pipeline walkthrough plan](pipeline-walkthrough.md) describes a simple visual presentation

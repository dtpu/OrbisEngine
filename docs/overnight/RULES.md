...
- Notable failures on old or new inputs trigger the investigation policy in `TONIGHT.md`:
  ...
  A failed row alone is insufficient when useful diagnosis remains feasible within the budget.
...
- Enforce the resource ceilings in `TONIGHT.md`. Check the resource ledger before each paid
  stage/batch, reserve for in-flight work and final validation, and stop launching work if the
  remaining budget is insufficient or cannot be bounded. Stop every Modal job you started
  once it is idle; never stop another teammate's job.

...
//   bun scripts/capture-walk-attack.ts [--clips elevator,lobby,stairs2,atrium,hpwide] [--secs 6] [--out dir]
...
const state = () => {
  const w = window.wander,
    p = w.camera.position,
    c = w.walk!.cellAt(p.x, p.z);
  return {
    x: +p.x.toFixed(3),
    y: +p.y.toFixed(3),
    z: +p.z.toFixed(3),
    floor: +w.walk!.floor.toFixed(3),
    cellFloor: Number.isNaN(c.floor) ? null : +c.floor.toFixed(3),
    dist: c.dist,
    inside: c.inside,
    blockedUnder: w.walk!.blockedAt(p.x, p.z),
    edge: +w.edge.toFixed(2),
  };
};
async function hold(page: Page, code: string, ms: number) {
  const tr = [];
  await page.keyboard.down(code);
  const t0 = performance.now();
  while (performance.now() - t0 < ms) {
    await page.waitForTimeout(SAMPLE_MS);
    tr.push({ t: +((performance.now() - t0) / 1000).toFixed(2), ...(await page.evaluate(state)) });
  }
  await page.keyboard.up(code);
  return tr;
}
async function turn(page: Page, deg: number) {
  // drag path, as a touch would; headless Chrome grants pointer lock on the first click, so release it first
  await page.evaluate(() => document.exitPointerLock?.());
  const px = (deg * Math.PI) / 180 / LOOK_RAD_PER_PX;
  await page.mouse.move(W / 2, H / 2);
  await page.mouse.down();
  for (let i = 1; i <= 20; i++) {
    await page.mouse.move(W / 2 + (px * i) / 20, H / 2);
...
const summarise = (tr: TraceSample[], start: WalkState) => {
  const far = Math.max(...tr.map((s) => Math.hypot(s.x - start.x, s.z - start.z)));
  const onNaN = tr.filter((s) => s.cellFloor === null).length;
  let deeper = 0;
  for (let i = 1; i < tr.length; i++) if (!tr[i].inside && tr[i].dist > tr[i - 1].dist) deeper++;
  const end = tr[tr.length - 1];
  return {
    unitsMoved: +far.toFixed(3),
    samples: tr.length,
    samplesOnUnknownFloor: onNaN,
    samplesOutside: tr.filter((s) => !s.inside).length,
    endDist: end.dist,
    endInside: end.inside,
    endFloor: end.floor,
    endY: end.y,
    endEdge: end.edge,
    stepsDeeperIntoVoid: deeper,
  };
};
type YawReport = ReturnType<typeof summarise> & { yawReached: number };
interface AttackReport {
  ...
  box: number[];
  start: WalkState;
  yaws: Record<string, YawReport>;
  climb?: unknown;
  afterR?: WalkState;
  rightStrafe?: unknown;
  worst?: [string, YawReport];
  worstShot?: string;
}
let hull = 0;
...
try {
  ...
  for (const clip of CLIPS) {
    ...
    const r: AttackReport = {
      errors,
      grid: await page.evaluate(() => window.wander.walk!.grid),
      upm: await page.evaluate(() => +window.wander.upm.toFixed(4)),
      clampOn: await page.evaluate(() => window.wander.clampOn),
      box: await page.evaluate(() =>
        window.wander.clampBox.min
          .toArray()
          .concat(window.wander.clampBox.max.toArray())
          .map((v) => +v.toFixed(2)),
      ),
    ...
    for (const yaw of [0, 45, 90, 135, 180, 225, 270, 315]) {
      ...
      const got = await page.evaluate(
        () =>
          +window.wander.THREE.MathUtils.radToDeg(
            new window.wander.THREE.Euler().setFromQuaternion(
              window.wander.camera.quaternion,
              'YXZ',
            ).y,
          ).toFixed(0),
      );
      ...
      if (clip === 'stairs2' && yaw === 0) {
        r.climb = {
          trace: tr
            .filter((_, i) => i % 4 === 0)
            .map((s) => ({ t: s.t, z: s.z, floor: s.floor, y: s.y })),
          floors: [...new Set(tr.map((s) => s.floor.toFixed(2)))].length,
          endFloor: tr[tr.length - 1].floor,
          largestJump: +Math.max(...tr.slice(1).map((s, i) => s.floor - tr[i].floor)).toFixed(3),
        };
        await page.screenshot({ path: path.join(OUT, 'stairs2-climb-shipped-end.png') });
        await page.keyboard.press('KeyR');
        await page.waitForTimeout(200);
        r.afterR = await page.evaluate(state);
      }
      if (clip === 'elevator' && yaw === 45)
        await page.screenshot({ path: path.join(OUT, 'elevator-attack-yaw45-end.png') });
      if (clip === 'hpwide' && yaw === 180)
        await page.screenshot({ path: path.join(OUT, 'hpwide-attack-yaw180-end.png') });
      if (clip === 'atrium' && yaw === 0)
        await page.screenshot({ path: path.join(OUT, 'atrium-attack-yaw0-end.png') });
    }
    if (clip === 'elevator') {
      // right strafe from 2 s in: where does it stop, and against what
      await page.keyboard.press('KeyR');
      await page.waitForTimeout(100);
      await turn(page, 0);
      await hold(page, 'KeyW', 2500);
      const tr = await hold(page, 'KeyD', 8000);
      r.rightStrafe = {
        ...summarise(tr, tr[0]),
        endX: tr[tr.length - 1].x,
        endBlockedUnder: tr[tr.length - 1].blockedUnder,
        movingOverLastSecond: +(
          Math.max(...tr.slice(-7).map((s) => s.x)) - Math.min(...tr.slice(-7).map((s) => s.x))
        ).toFixed(4),
      };
    ...
    r.worst = Object.entries(r.yaws).sort(
      (a, b) => b[1].samplesOnUnknownFloor - a[1].samplesOnUnknownFloor,
    )[0];
    await page.keyboard.press('KeyR');
    await page.waitForTimeout(100);

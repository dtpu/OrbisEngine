...
"""Package N per-track lhm_animate runs as ONE public/worlds/<clip>-4d/ with a people manifest.
  ...
  worker/.venv-da3/bin/python scripts/package_multiperson.py \
...
def transform_ply_sized(src, dst, R, t, scale, centre):
    ...
    if scale != 1.0:
        ...
        for n in ("scale_0", "scale_1", "scale_2"):
            v[n] = (v[n].astype(np.float64) + np.log(scale)).astype(np.float32)  # PLY holds log sigma
...
def main():
    ...
    ap.add_argument("--motion", action="append", required=True, metavar="TRACK=DIR",
    ...
    ap.add_argument("--height-stride", type=int, default=4,
                    help="sample every Nth frame when measuring standing height and the floor")
    ap.add_argument("--shared-scale", default="auto",
    ...
    for c in cams["cameras"]:
        ...
        m2[:3, :3] = R @ m[:3, :3]
        m2[:3, 3] = R @ m[:3, 3] + t
        out_cams.append(dict(sourceIndex=c["sourceIndex"], time=c["time"], camera_to_world=m2.tolist(),
                             source_intrinsics=c["source_intrinsics"], source_image_size=c.get("source_image_size")))
    (out / "cameras.json").write_text(json.dumps(dict(
        ...
        frameAlign=fa, source=str(a.cameras), sourceClip=str(a.clip), cameras=out_cams), indent=1))
    ...
    for tid, motion in pairs:
        ...
        regs[tid] = json.loads((motion / "registration.json").read_text()) if (motion / "registration.json").exists() else {}
    ...
    if a.shared_scale in ("none",) or len(pairs) < 2:
        ...
        reference = max(roi_points, key=roi_points.get) if a.shared_scale in ("auto", "floor") else int(a.shared_scale)
        multipliers = {tid: heights[reference] / heights[tid] for tid, _ in pairs}
    shared = dict(mode=a.shared_scale, referenceTrack=reference,
                  reason=("Body size is whatever each track's own first-frame depth registration made it, and "
                          ...
                          if reference is not None else "Each track keeps its own registration scale."),
                  depthRoiPoints=roi_points, heightsBeforeUnits={str(k): v for k, v in heights.items()},
    ...
    shared["floorCheck"] = dict(
        ...
        floorSpreadUnits=float(max(np.median(v) for v in floors.values()) - min(np.median(v) for v in floors.values())),
        floorSpreadAsBodyHeights=float((max(np.median(v) for v in floors.values())
                                        - min(np.median(v) for v in floors.values())) / heights[pairs[0][0]]),
    ...
    if reference is not None:
        ...
        for tid, m in measured.items():
            ...
            alt[str(tid)] = float((target - np.median(m["cameraY"])) / denom) if abs(denom) > 1e-6 else 1.0
        ...
        ks = sorted(measured)
        if len(ks) == 2:
            A = [heights[k] / float(np.median(measured[k]["feet"] - measured[k]["cameraY"])) for k in ks]
            C = [float(np.median(measured[k]["cameraY"])) for k in ks]
            den = A[0] - A[1]
            shared["consistentFloorY"] = float((A[0] * C[0] - A[1] * C[1]) / den) if abs(den) > 1e-9 else None
            shared["consistentFloorNote"] = ("The one floor plane at which matching the two bodies' heights "
                                             "and standing them on one floor are the same answer.")
        shared["floorMatchedMultipliers"] = dict(
            multipliers=alt, mode="--shared-scale floor", targetFloorY=target,
            targetSource=("median of the tracks' own feet" if a.floor_y is None else "--floor-y (measured Marble world)"),
            resultingHeightRatio=float(max(alt[str(k)] * heights[k] for k in heights)
    ...
    if a.shared_scale == "floor" and reference is not None:
        multipliers = {tid: shared["floorMatchedMultipliers"]["multipliers"][str(tid)] for tid, _ in pairs}
    ...
    for rank, (tid, motion) in enumerate(pairs):
        ...
        seq_out = dict(seq)
        seq_out.update(frames=frames, count=len(frames), frame_sha256=hashes,
                       gaussiansPerFrame=sorted(counts)[0] if len(counts) == 1 else sorted(counts),
                       allNativeGaussianAttributesFinite=finite_all, people_only=True,
                       ...
                       camera0Reframe=dict(rotation=R.tolist(), translation=t.tolist(),
                       ...
                       lowerBodyProvenance=a.lower_body, trackIndex=tid, personId=name,
                       manifest="../people.json", sharedScaleMultiplier=mult,
        ...
        reg = json.loads((motion / "registration.json").read_text()) if (motion / "registration.json").exists() else {}
        ...
        people.append(dict(
            id=name, track=tid, sequence=f"{name}/sequence.json", directory=name,
            label=f"person {tid}", frames=len(frames), fps=seq["fps"],
            sourceIndices=seq["sourceIndices"], timestamps=seq["timestamps"],
            visibleSampleRuns=runs(present), firstSample=present[0], lastSample=present[-1],
            gaussiansPerFrame=seq_out["gaussiansPerFrame"],
            bodyHeightUnits=body, feetY=feet,
            registrationScale=reg.get("uniformScale"), registration=reg,
            sizeCorrection=dict(multiplier=mult, appliedAboutSourceCamera=True,
                                bodyHeightBeforeUnits=heights[tid], referenceTrack=reference),
            quality=quality.get(tid),
            transform=dict(translation=[0.0, 0.0, 0.0], quaternionXYZW=[0.0, 0.0, 0.0, 1.0], scale=1.0,
        ...
        print(json.dumps({k: people[-1][k] for k in
                          ("id", "track", "frames", "bodyHeightUnits", "feetY", "registrationScale")}, indent=1))

    heights = [p["bodyHeightUnits"] for p in people]
    manifest = dict(
        schema="wander.people/1",
        clip=str(a.clip), sourceSha256=hashlib.sha256(Path(a.clip).read_bytes()).hexdigest(),
        fps=people[0]["fps"], samples=tracks_doc["samples"], duration=tracks_doc["duration"],
        sourceIndices=tracks_doc["sourceIndices"], timestamps=tracks_doc["timestamps"],
        cameras="cameras.json", primary=people[0]["id"], peopleCount=len(people), people=people,
        sharedScale=shared,
        coordinates=f"OpenGL, {frame_desc}. Every person is in this one frame; per-person transforms "
                    "are identity.",
        frameAlign=None if fa is None else dict(tiltFromYDeg=fa["tiltFromYDeg"], source=fa["source"],
                                                adopted=fa["adopted"], file=str(a.cameras)),
        sharedPlacement=dict(
            bodyHeightUnits=people[0]["bodyHeightUnits"], feetY=people[0]["feetY"],
        ...
        tracking=dict(source=str(Path(a.tracks) / "tracks.json"), method=tracks_doc["method"],
    ...
    print(json.dumps(dict(out=str(out), people=len(people),
                          bodyHeights=[round(h, 4) for h in heights],
                          bodyHeightRatio=manifest["sharedPlacement"]["bodyHeightRatio"]), indent=1))

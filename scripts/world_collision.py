...
turn 9.80665 m/s^2 into units.
  ...
  python scripts/world_collision.py --world elevator-4d --spz public/marble-elevator-clean.spz
"""
...
def read_spz_xyz(path):
    ...
    fixed = (pos_b[:, :, 0].astype(np.int32) | (pos_b[:, :, 1].astype(np.int32) << 8)
...
def fit_floor(P, seed_y=None):
    ...
    if seed_y is None:
        h, e = np.histogram(P[:, 1], 400); seed_y = float(0.5 * (e[h.argmax()] + e[h.argmax() + 1]))
    c = np.array([0., 0., seed_y]); inl = np.zeros(len(P), bool)
    for band in (0.20, 0.12, 0.055, 0.035):
        for _ in range(10):
            d = P[:, 1] - (P[:, 0] * c[0] + P[:, 2] * c[1] + c[2]); inl = np.abs(d) < band
            if inl.sum() < 500:
                break
            A = np.stack([P[inl, 0], P[inl, 2], np.ones(int(inl.sum()))], 1)
            c, *_ = np.linalg.lstsq(A, P[inl, 1], rcond=None)
    n = unit([-c[0], 1., -c[1]])
    d = float(n @ np.array([0., c[2], 0.]))
...
def height_field(P, n, d, cell, band, min_count, pct):
    ...
    u = unit(np.cross(n, [0, 0, 1])); w = unit(np.cross(u, n))
    gu = Q @ u; gw = Q @ w
    # stray splats metres outside the room would otherwise set the grid extent
    lo_u, hi_u = np.percentile(gu, [0.2, 99.8]); lo_w, hi_w = np.percentile(gw, [0.2, 99.8])
    keep = (gu >= lo_u) & (gu <= hi_u) & (gw >= lo_w) & (gw <= hi_w)
    gu, gw, tq = gu[keep], gw[keep], tq[keep]
    iu = np.floor(gu / cell).astype(np.int64); iw = np.floor(gw / cell).astype(np.int64)
    u0, u1, w0, w1 = int(iu.min()), int(iu.max()), int(iw.min()), int(iw.max())
    W, H = u1 - u0 + 1, w1 - w0 + 1
    key = (iw - w0) * W + (iu - u0)
    order = np.argsort(key, kind='stable'); key = key[order]; tq = tq[order]
    starts = np.flatnonzero(np.r_[True, key[1:] != key[:-1]]); ends = np.r_[starts[1:], len(key)]
    heights = np.full(W * H, np.nan, np.float32); counts = np.zeros(W * H, np.int32)
    for s, e in zip(starts, ends):
        if e - s >= min_count:
            heights[key[s]] = np.percentile(tq[s:e], pct); counts[key[s]] = e - s
    return dict(axisU=u.tolist(), axisW=w.tolist(), cell=cell, originU=u0 * cell, originW=w0 * cell,
                width=W, height=H, heights=heights.reshape(H, W), counts=counts.reshape(H, W))


def occupancy_grid(P, seed_y, upm, cell_m, stature_m, min_count, block_count, window_m, pct,
                   band_lo, band_hi, step_up, drop_max_m, hull_m, extent_m, close_m, below_m, bin_stature, bin_count, seed_xz=(0.0, 0.0)):
    ...
    cell = cell_m * upm; st = stature_m * upm; win = window_m * upm; drop = drop_max_m * upm; step = step_up * st
    ...
    lo_x, hi_x = np.percentile(P[:, 0], [0.2, 99.8]); lo_z, hi_z = np.percentile(P[:, 2], [0.2, 99.8])
    keep = (P[:, 0] >= lo_x) & (P[:, 0] <= hi_x) & (P[:, 2] >= lo_z) & (P[:, 2] <= hi_z)
    Q = P[keep]
    ix = np.floor(Q[:, 0] / cell).astype(np.int64); iz = np.floor(Q[:, 2] / cell).astype(np.int64)
    x0, x1, z0, z1 = int(ix.min()), int(ix.max()), int(iz.min()), int(iz.max())
    W, H = x1 - x0 + 1, z1 - z0 + 1
    key = (iz - z0) * W + (ix - x0)
    order = np.lexsort((Q[:, 1], key)); key = key[order]; ys = Q[order, 1]
    starts = np.flatnonzero(np.r_[True, key[1:] != key[:-1]]); ends = np.r_[starts[1:], len(key)]
    cols = {}; cands = {}
    for s, e in zip(starts, ends):
        col = ys[s:e]; cols[key[s]] = col; out = []
        top = np.searchsorted(col, col + win, 'right'); i = 0
        while i < len(col):
            if top[i] - i >= min_count: out.append(float(col[i + int(pct / 100 * (top[i] - i - 1))])); i = top[i]
            else: i += 1
        cands[key[s]] = out
    ...
    i0 = min(W - 1, max(0, int(np.floor(seed_xz[0] / cell)) - x0)); j0 = min(H - 1, max(0, int(np.floor(seed_xz[1] / cell)) - z0))
            ...
            for j in (j0 - r, j0 + r) if r else (j0,):
                if 0 <= i < W and 0 <= j < H:
                    for h in cands.get(j * W + i, ()):
                        if -drop <= h - seed_y <= step: found.append((abs(h - seed_y), j * W + i, h))
        for j in range(j0 - r + 1, j0 + r):
            for i in (i0 - r, i0 + r):
                if 0 <= i < W and 0 <= j < H:
                    for h in cands.get(j * W + i, ()):
                        if -drop <= h - seed_y <= step: found.append((abs(h - seed_y), j * W + i, h))
        if found: _, seed_k, seed_h = min(found); break
    floor = np.full(W * H, np.nan, np.float32); seen = np.zeros(W * H, bool)
    ...
    while q:
        ...
        for h in cands.get(k, ()):
            if -drop <= h - carry <= step and (best is None or abs(h - carry) < abs(best - carry)): best = h
        if best is not None: floor[k] = best; carry = best
        i, j = k % W, k // W
        for ni, nj in ((i - 1, j), (i + 1, j), (i, j - 1), (i, j + 1)):
            if 0 <= ni < W and 0 <= nj < H and not seen[nj * W + ni]: q.append((nj * W + ni, carry))
    # distance (cells, 4-connected) to the nearest floor-known column, and that column's floor
    dist = np.full(W * H, 65535, np.int64); ref = np.full(W * H, seed_h, np.float32)
    known = np.flatnonzero(np.isfinite(floor))
    q = deque(int(k) for k in known); dist[known] = 0; ref[known] = floor[known]
    while q:
        k = q.popleft(); i, j = k % W, k // W
        for ni, nj in ((i - 1, j), (i + 1, j), (i, j - 1), (i, j + 1)):
            if 0 <= ni < W and 0 <= nj < H:
                nk = nj * W + ni
                if dist[nk] == 65535: dist[nk] = dist[k] + 1; ref[nk] = ref[k]; q.append(nk)
    blocked = np.zeros(W * H, np.uint8); counts = np.zeros(W * H, np.int32)
    ...
    binh = bin_stature * st; y0 = seed_y - below_m * upm; solid = np.zeros(W * H, np.int64)
    for k, col in cols.items():
        counts[k] = len(col); f = ref[k]
        blocked[k] = int(((col >= f + band_lo * st) & (col <= f + band_hi * st)).sum()) >= block_count
        b = np.floor((col - y0) / binh).astype(np.int64); b = b[(b >= 0) & (b < 32)]
        if len(b):
            bc = np.bincount(b, minlength=32); solid[k] = int(sum(1 << i for i in np.flatnonzero(bc >= bin_count)))
    ...
    D = dist <= rc
    far = np.full(W * H, 65535, np.int64); q = deque()
    for k in range(W * H):
        i, j = k % W, k // W
        if not D[k] or i == 0 or j == 0 or i == W - 1 or j == H - 1: far[k] = 0; q.append(k)
    while q:
        k = q.popleft(); i, j = k % W, k // W
        for ni, nj in ((i - 1, j), (i + 1, j), (i, j - 1), (i, j + 1)):
            if 0 <= ni < W and 0 <= nj < H:
                nk = nj * W + ni
                if far[nk] == 65535: far[nk] = far[k] + 1; q.append(nk)
    inside = ((far > rc) | (dist <= int(round(hull_m * upm / cell)))).astype(np.uint8)
    return dict(cell=cell, originX=x0 * cell, originZ=z0 * cell, width=W, height=H,
                floor=floor.reshape(H, W), blocked=blocked.reshape(H, W), counts=counts.reshape(H, W),
                dist=dist.reshape(H, W), hullCells=int(round(hull_m * upm / cell)), inside=inside.reshape(H, W),
                solid=solid.reshape(H, W), solidY0=float(y0), solidBin=float(binh), solidBins=32,
                upm=upm, stature=st, seedY=seed_y, seedCell=[seed_k % W, seed_k // W], seedFloor=seed_h,
...
def ransac_walls(P, n, d, max_walls, tol, min_inliers, iters, rng, floor_pts):
    ...
    for _ in range(max_walls * 4):
        ...
        Q = pts[inl]; cm = Q.mean(0)
        _, _, vt = np.linalg.svd(Q - cm, full_matrices=False)
        nn = vt[2]; nn -= (nn @ n) * n; nn = unit(nn)            # re-fit, forced vertical
        dd = float(nn @ cm)
        inl = np.abs(pts @ nn - dd) < tol; Q = pts[inl]
        along = unit(np.cross(n, nn))
        s = Q @ along; h = Q @ n - d
        # sheet test: occupied 0.1 u bins on the plane, and their extent
        bs = np.floor(s / 0.1).astype(np.int64); bh = np.floor(h / 0.1).astype(np.int64)
        occ = np.unique(bs * 100000 + bh)
        ubs = np.unique(bs); ubh = np.unique(bh)
        # a bin column counts only when it holds enough of the sheet, so a stray splat a metre
        # away does not stretch the extent
        cols = np.bincount(bs - bs.min()); good_cols = np.flatnonzero(cols > 0.2 * cols.max()) + bs.min()
        rows = np.bincount(bh - bh.min()); good_rows = np.flatnonzero(rows > 0.2 * rows.max()) + bh.min()
        ...
        fd = floor_pts @ nn - dd; fs = floor_pts @ along
        near = (np.abs(fd) < 0.5) & (fs >= 0.1 * good_cols.min()) & (fs <= 0.1 * (good_cols.max() + 1))
        pos = int((fd[near] > 0.05).sum()); neg = int((fd[near] < -0.05).sum())
        one_sided = (min(pos, neg) <= 0.35 * max(pos, neg, 1))
        rec = dict(normal=nn.tolist(), d=dd, inliers=int(inl.sum()), along=along.tolist(),
                   spanAlong=float(span_along), spanUp=float(span_up), occupiedBins=int(len(occ)),
        ...
        if span_along >= 1.0 and span_up >= 0.8 and one_sided:
            ...
            if neg < pos:
                rec['normal'] = (-nn).tolist(); rec['d'] = -dd
                rec['along'] = (-along).tolist()
                rec['extentAlong'] = [-rec['extentAlong'][1], -rec['extentAlong'][0]]
            walls.append(rec)
        else:
            rec['rejected'] = ('not a sheet' if not (span_along >= 1.0 and span_up >= 0.8)
...
def main():
    ...
    ap.add_argument('--upm', type=float, default=None, help='world units per metre; default 1/metresPerWorldUnit, else 1')
    ap.add_argument('--grid-cell-m', type=float, default=0.10)
    ap.add_argument('--grid-min-count', type=int, default=8, help='splats a 10 cm column needs in one slab to have a floor')
    ap.add_argument('--grid-block-count', type=int, default=8, help='splats in the body band that make a column a wall')
    ap.add_argument('--grid-window-m', type=float, default=0.15, help='slab height a floor surface is read from')
    ap.add_argument('--grid-pct', type=float, default=10.0)
    ap.add_argument('--grid-band', type=float, nargs=2, default=(0.2, 1.1), help='body band, fractions of stature above the floor')
    ap.add_argument('--grid-step-up', type=float, default=0.2, help='one stair, as a fraction of stature (fourd.html WALK_STEP_UP)')
    ap.add_argument('--grid-drop-m', type=float, default=0.6, help='deeper than this is a pit, not a step down (fourd.html WALK_DROP_MAX_M)')
    ap.add_argument('--grid-hull-m', type=float, default=1.0, help='how far from known floor an unknown column is still walked over (fourd.html WALK_HULL_M)')
    ap.add_argument('--grid-extent-m', type=float, default=20.0, help='half-extent of the grid about camera 0 (fourd.html WALK_EXTENT_M)')
    ap.add_argument('--grid-close-m', type=float, default=2.0, help='closing radius: gaps up to twice this between known floor count as inside (fourd.html WALK_CLOSE_M)')
    ap.add_argument('--grid-below-m', type=float, default=0.5, help='solid bins start this far under the seed floor')
    ap.add_argument('--grid-bin-stature', type=float, default=0.1, help='solid bin height, fraction of stature')
    ap.add_argument('--grid-bin-count', type=int, default=15, help='splats a bin needs to be solid (a real rail column holds hundreds per bin; ceiling fuzz and glass 6-22 in a whole band)')
    ap.add_argument('--seed-y', type=float, default=None, help='floor height under camera 0 to flood from; default the lowest strong y layer. '
    ...
    P[:, 1] *= -1; P[:, 2] *= -1                                  # Marble OpenCV -> viewer (Rx(pi))
    ...
    for cand, key in (('placement.json', 'metresPerWorldUnit'), ('people.json', None)):
        ...
        if os.path.exists(p):
            ...
            if v:
                mpu, mpu_src = float(v), cand; break
    ...
    floor_pts = (np.outer(hf['originU'] + (ii + 0.5) * a.cell, u_ax)
                 + np.outer(hf['originW'] + (jj + 0.5) * a.cell, w_ax) + d * n)
    walls, rejected = ransac_walls(P, n, d, a.walls, a.wall_tol, a.wall_min_inliers, 400, rng, floor_pts)
    ...
    for i in range(1, len(h) - 1):
        if h[i] >= h[i - 1] and h[i] >= h[i + 1] and h[i] >= 0.2 * h.max():
            seed_y = float(0.5 * (e[i] + e[i + 1])); break
    if a.seed_y is not None: seed_y = a.seed_y
    og = occupancy_grid(P, seed_y, upm, a.grid_cell_m, 1.7, a.grid_min_count, a.grid_block_count, a.grid_window_m,
                        a.grid_pct, a.grid_band[0], a.grid_band[1], a.grid_step_up, a.grid_drop_m, a.grid_hull_m, a.grid_extent_m,
                        a.grid_close_m, a.grid_below_m, a.grid_bin_stature, a.grid_bin_count)
    og_filled = int(np.isfinite(og['floor']).sum()); og_blocked = int(og['blocked'].sum())
    out = dict(
        schema='wander.collision/2',
        world=a.world, spz=os.path.relpath(a.spz, ROOT),
        ...
        metresPerWorldUnit=mpu, metresPerWorldUnitSource=mpu_src,
        splats=int(len(xyz)), opaqueSplats=int(len(P)), minAlpha=a.min_alpha,
        floor=dict(normal=n.tolist(), d=d, yAtOrigin=float(d / n[1]) if abs(n[1]) > 1e-6 else None,
                   tiltFromYDeg=tilt, inliers=ninl,
        ...
        heightField=dict(
            axisU=hf['axisU'], axisW=hf['axisW'], cell=hf['cell'],
            originU=hf['originU'], originW=hf['originW'], width=hf['width'], height=hf['height'],
            # height ABOVE the floor plane along its normal; null = no vote, fall back to the plane
            heights=[[None if not np.isfinite(v) else round(float(v), 4) for v in row] for row in hf['heights']],
        ...
        occupancy=dict(
            cell=og['cell'], originX=og['originX'], originZ=og['originZ'], width=og['width'], height=og['height'],
            upm=upm, upmSource='--upm' if a.upm else ('1/metresPerWorldUnit' if mpu else 'assumed 1'), stature=og['stature'],
            seedY=og['seedY'], seedSource='--seed-y' if a.seed_y is not None else 'lowest strong y layer', reachedCells=og['reached'],
            seedCell=og['seedCell'], seedFloor=round(float(og['seedFloor']), 4), hullCells=og['hullCells'],
            ...
            solid=[[int(v) for v in row] for row in og['solid']], solidY0=og['solidY0'], solidBin=og['solidBin'], solidBins=og['solidBins'],
            ...
            floor=[[None if not np.isfinite(v) else round(float(v), 4) for v in row] for row in og['floor']],
            blocked=[[int(v) for v in row] for row in og['blocked']],
            filledCells=og_filled, blockedCells=og_blocked,
        ...
        assumptions=['walls are infinite planes clipped to their inlier extent; furniture and other '
    ...
    print(f'floor y0 {out["floor"]["yAtOrigin"]:.4f} tilt {tilt:.2f} deg inliers {ninl}; '

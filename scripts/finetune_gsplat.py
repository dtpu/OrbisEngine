...
def gaussian_window(size=11, sigma=1.5, dev='cuda'):
    ...
    g = torch.exp(-g * g / (2 * sigma * sigma)); g = g / g.sum()
...
class Model:
    ...
    def __init__(self, s, dev):
        self.p = {'means': torch.from_numpy(s['means']).to(dev),
    ...
    def render(self, viewmat, K, W, H, depth=False):
        ...
        out, alpha, info = rasterization(
            means=self.p['means'], quats=self.p['quats'], scales=torch.exp(self.p['log_scales']),
            opacities=torch.sigmoid(self.p['logit_op']),
            colors=colors, viewmats=viewmat[None], Ks=K[None], width=W, height=H,
            near_plane=0.02, far_plane=200.0, eps2d=0.3, packed=True,
            render_mode='RGB+ED' if depth else 'RGB', rasterize_mode='classic')
        if depth:
...
def project(means, v, K, W, H, margin=0.05):
    ...
    ok = (z > 0.02) & (u > -margin * W) & (u < (1 + margin) * W) & (vv > -margin * H) & (vv < (1 + margin) * H)
    ui = u.clamp(0, W - 1).long(); vi = vv.clamp(0, H - 1).long()
    return ok, z, ui, vi
...
def psnr(a, b, valid):
    d = ((a - b) ** 2)[valid.expand_as(a) if valid.dim() == a.dim() else valid[..., None].expand_as(a)]
    ...
    for g in opt.param_groups:
        ...
        if st is not None:
            ...
            if extra is not None:
                ...
                ea = torch.cat([ea, z], 0); eas = torch.cat([eas, z], 0)
            opt.state[new] = {'step': st['step'], 'exp_avg': ea.contiguous(), 'exp_avg_sq': eas.contiguous()}
    for k in model.init:
        model.init[k] = torch.cat([model.init[k][keep], extra[k]], 0) if extra is not None else model.init[k][keep]
    out = []
    for t, fill in side or []:
        t = t[keep]
        if extra is not None:
            t = torch.cat([t, torch.full((len(extra['means']),), fill, dtype=t.dtype, device=t.device)], 0)
        out.append(t)
    return out
...
def main():
    ...
    ap.add_argument('--train-stride', type=int, default=2, help='use every Nth non-held-out frame for training')
    ap.add_argument('--lambda-ssim', type=float, default=0.2)
    ap.add_argument('--sigma-pos', type=float, default=0.02, help='tether scale for means, native units (0.02 = ~5 cm)')
    ap.add_argument('--clamp-pos', type=float, default=0.05, help='hard clamp on displacement, native units (~13 cm)')
    ap.add_argument('--sigma-scale', type=float, default=0.5, help='tether on log-scale change')
    ap.add_argument('--lambda-reg', type=float, default=1e-3, help='weight of the tether (sum over visible splats / n_visible * lambda * 1e3)')
    ...
    ap.add_argument('--lambda-depth', type=float, default=0.0, help='weight of |log(rendered depth) - log(wvp depth)| on supported, non-person texels')
    ap.add_argument('--freeze-geometry', action='store_true', help='optimise colour and opacity only')
    # run5: observed mask / prune / densify / opacity entropy
    ap.add_argument('--observed-min-views', type=int, default=5, help='views a splat must pass the depth test in to count as observed')
    ap.add_argument('--observed-depth-tol', type=float, default=0.05, help='relative slack on the depth test (z <= wvp depth * (1+tol))')
    ap.add_argument('--reg-observed-scale', type=float, default=1.0, help='tether weight multiplier on observed splats (run5: 0.05)')
    ap.add_argument('--reg-unobserved-scale', type=float, default=1.0,
    ...
    ap.add_argument('--lambda-depth-tv', type=float, default=0.0,
    ...
    ap.add_argument('--tv-every', type=int, default=2, help='apply the off-path depth-TV term every Nth iteration')
    ap.add_argument('--tv-max-m', type=float, default=1.5, help='max lateral offset of the TV views, metres')
    ap.add_argument('--tv-edge', type=float, default=10.0, help='edge weight exp(-tv_edge * |grad rendered RGB|)')
    ap.add_argument('--prune', action='store_true', help='prune observed splats (transparent, then veil) during training')
    ...
    ap.add_argument('--prune-depth-cm', type=float, default=8.0, help='prune observed splats whose median depth sits this far in front of the wvp surface')
    ap.add_argument('--depth-views', type=int, default=48, help='training views sampled for the per-splat depth statistic')
    ap.add_argument('--densify', action='store_true', help='3DGS split/clone restricted to observed splats')
    ...
    ap.add_argument('--grow-scale3d', type=float, default=0.01, help='native units; below this a selected splat is cloned, above it is split')
    ap.add_argument('--max-splats', type=int, default=2_400_000)
    ap.add_argument('--lambda-opacity-entropy', type=float, default=0.0, help='push observed opacities to 0/1')
    ap.add_argument('--pseudo-dir', type=Path, default=None, help='extra pseudo-GT views (npz with viewmats + PNGs) mixed in at --pseudo-weight')
    ap.add_argument('--pseudo-weight', type=float, default=0.3)
    ap.add_argument('--init-npz', type=Path, default=None, help='start from a finetuned.npz instead of the Marble splats.npz')
    ...
    ap.add_argument('--m-per-unit', type=float, default=1.7 / 0.6518811, help='metres per native unit')
    a = ap.parse_args()
    ...
    s = np.load(a.data / 'splats.npz'); c = np.load(a.data / 'cameras.npz')
    model = Model(np.load(a.init_npz) if a.init_npz else s, dev)
    viewmats = torch.from_numpy(c['viewmats']).to(dev); K = torch.from_numpy(c['K']).to(dev)
    W, H = int(c['width']), int(c['height']); nF = len(viewmats)
    imgs, valid = load_images(a.data, nF, dev)
    dz = np.load(a.data / 'depth.npz')
    depth_layers = torch.from_numpy(dz['depth']).to(dev); depth_sup = torch.from_numpy(dz['supported']).to(dev); depth_index = dz['depth_index']

    def gt_depth(i):
        L = int(depth_index[i])
        d = F.interpolate(depth_layers[L][None, None], size=(H, W), mode='nearest')[0, 0]
        sup = F.interpolate(depth_sup[L][None, None].float(), size=(H, W), mode='nearest')[0, 0] > 0.5
        return d, sup

    test_idx = list(range(0, nF, a.holdout)); train_idx = [i for i in range(nF) if i % a.holdout != 0][::a.train_stride]
    print(f'{model.n} splats, {nF} frames ({len(train_idx)} train / {len(test_idx)} test) at {W}x{H}; load {time.time() - t0:.0f}s', flush=True)
    ...
    if use_obs:
        with torch.no_grad():
            ...
            obs, hit_cnt = observed_mask(model.means, viewmats[train_idx], K, W, H, depths, [valid[i] for i in train_idx],
                                         a.observed_depth_tol, a.observed_min_views)
            del gds, depths
            torch.cuda.empty_cache()
        n_obs = int(obs.sum())
        print(f'observed (depth test in >= {a.observed_min_views} of {len(train_idx)} views): {n_obs} '
              f'({100 * n_obs / model.n:.1f}%); median hits among visible {float(hit_cnt[vis].float().median()):.0f}', flush=True)
    ...
    bbox_lo = model.init['means'].min(0).values; bbox_hi = model.init['means'].max(0).values
    ...
    def evaluate(tag):
        ...
        with torch.no_grad():
            for i in test_idx:
                ...
                gd, sup = gt_depth(i); sel = sup & valid[i] & (al[..., 0] > 0.5)
                derr.append(float((torch.log(dep.clamp(min=0.05)) - torch.log(gd))[sel].abs().median()))
        m = float(np.mean(model_psnr)); dr = float(np.mean(derr))
        print(f'[{tag}] held-out masked PSNR: mean {m:.2f} dB (min {min(model_psnr):.2f}, max {max(model_psnr):.2f}); median |log depth ratio| vs wvp {dr:.3f}', flush=True)
    ...
    if a.pseudo_dir:
        ...
        pi = torch.from_numpy(np.stack([np.asarray(Image.open(a.pseudo_dir / f'{i:04d}.png').convert('RGB'))
                                        for i in range(len(pv))])).to(dev)
        pseudo = (pv, pi)
    ...
    opt = torch.optim.Adam([
        ...
        {'params': [model.p['f_dc']], 'lr': a.lr_color, 'name': 'f_dc'}], eps=1e-15)
    win = gaussian_window(dev=dev)
    log = []
    n_pruned_op = n_pruned_depth = n_cloned = n_split = 0
    grad_acc = torch.zeros(model.n, device=dev); grad_cnt = torch.zeros(model.n, device=dev)

    def reg_weights():
        return vis.float() * torch.where(obs, torch.full((), a.reg_observed_scale, device=dev),
                                         torch.full((), a.reg_unobserved_scale, device=dev))
    w_reg = reg_weights()
    order = np.random.permutation(train_idx); ptr = 0
    for it in range(1, a.iters + 1):
        use_pseudo = pseudo is not None and (it % 3 == 0)
        if use_pseudo:
            j = int(np.random.randint(len(pseudo[0])))
            vm = pseudo[0][j]; gt = pseudo[1][j].float() / 255; m = torch.ones((H, W, 1), device=dev)
            wgt = a.pseudo_weight
        else:
            if ptr >= len(order):
                order = np.random.permutation(train_idx); ptr = 0
            i = int(order[ptr]); ptr += 1
            vm = viewmats[i]; gt = imgs[i].float() / 255; m = valid[i].float()[..., None]; wgt = 1.0
        ...
        if a.lambda_depth > 0 and not use_pseudo:
            gd, sup = gt_depth(i); md = (sup & valid[i]).float()
            dl = ((torch.log(dep.clamp(min=0.05)) - torch.log(gd)).abs() * md).sum() / md.sum().clamp(min=1)
        ...
        if a.lambda_depth_tv > 0 and it % a.tv_every == 0:
            ...
            off_m = float(np.random.uniform(0.25, a.tv_max_m)) * (1.0 if np.random.rand() < 0.5 else -1.0)
            im2, al2, dp2, _ = model.render(offpath_view(fr, off_m), K, W, H, depth=True)
            ld = torch.log(dp2.clamp(min=0.05)); mk = (al2[..., 0] > 0.5).float()
            ix = (im2[:, 1:] - im2[:, :-1]).abs().mean(-1); iy = (im2[1:] - im2[:-1]).abs().mean(-1)
            ...
            tv = ((ld[:, 1:] - ld[:, :-1]).abs() * wx).sum() / wx.sum().clamp(min=1) \
                 + ((ld[1:] - ld[:-1]).abs() * wy).sum() / wy.sum().clamp(min=1)
        ...
        if a.lambda_opacity_entropy > 0 and int(obs.sum()) > 0:
            ...
            reg = reg + a.lambda_opacity_entropy * (-(o * torch.log(o) + (1 - o) * torch.log(1 - o))).mean()
        (loss + reg).backward()
        gate = vis.float()
        for k, p in model.params().items():
            if p.grad is None:
                continue
            g = torch.zeros_like(gate) if (a.freeze_geometry and k in ('means', 'quats', 'log_scales')) else gate
        ...
        if (do_prune or do_grow) and it % a.prune_every == 0:
            with torch.no_grad():
                # --- grow (observed only) ---
                extra = None; keep = torch.ones(model.n, dtype=torch.bool, device=dev)
                if do_grow:
                    ...
                    if int(sel.sum()) > 0 and room > 0:
                        ...
                        off = torch.einsum('nij,nj->ni', rot, torch.randn_like(sc).clamp(-2, 2) * sc)
                        extra = {'means': torch.cat([model.means[sp] + off, model.means[sp] - off, model.means[cl]], 0),
                                 'quats': torch.cat([model.quats[sp], model.quats[sp], model.quats[cl]], 0),
                                 'log_scales': torch.cat([model.log_scales[sp] - math.log(1.6), model.log_scales[sp] - math.log(1.6), model.log_scales[cl]], 0),
                                 'logit_op': torch.cat([model.logit_op[sp], model.logit_op[sp], model.logit_op[cl]], 0),
                ...
                if do_prune:
                    ...
                    n_pruned_op += int((dead & keep).sum()); keep &= ~dead
                    if it >= a.prune_depth_start:
                        off_d = depth_offset(model.means, dv_views, K, W, H, dv_depths, dv_valid)
                        veil = obs & torch.nan_to_num(off_d, nan=0.0).lt(-a.prune_depth_cm / CM_PER_UNIT)
                        n_pruned_depth += int((veil & keep).sum()); keep &= ~veil
                if extra is not None or not bool(keep.all()):
                    (vis, obs, orig_idx) = surgery(model, opt, keep, extra,
                                                   side=[(vis, True), (obs, True), (orig_idx, -1)])
                    n_vis = max(1, int(vis.sum())); w_reg = reg_weights()
                    grad_acc = torch.zeros(model.n, device=dev); grad_cnt = torch.zeros(model.n, device=dev)
                    print(f'  it {it}: refine -> {model.n} splats (pruned op {n_pruned_op}, depth {n_pruned_depth}; '
                          f'cloned {n_cloned}, split {n_split})', flush=True)
                else:
                    grad_acc.zero_(); grad_cnt.zero_()

        if it % 50 == 0 or it == 1:
            rec = {'it': it, 'l1': float(l1.detach()), 'ssim': float(ss.detach()), 'depth': float(dl.detach()),
                   'tv': float(tv.detach()), 'reg': float(reg.detach()), 'n': model.n,
                   'psnr_train': psnr(img.detach(), gt, m[..., 0] > 0.5), 't': time.time() - t0}
            log.append(rec)
            print(f'it {it}: l1 {rec["l1"]:.4f} ssim {rec["ssim"]:.3f} depth {rec["depth"]:.4f} tv {rec["tv"]:.4f} reg {rec["reg"]:.4f} '
                  f'train psnr {rec["psnr_train"]:.2f} n {model.n} | {rec["t"]:.0f}s VRAM {torch.cuda.max_memory_allocated() / 1e9:.1f} GB', flush=True)
        if it % a.eval_every == 0 and it < a.iters:
            evaluate(f'it {it}'); render_set(f'it{it}')
    ...
    with torch.no_grad():
        ...
        op0 = torch.sigmoid(model.init['logit_op'])[vis]; op1 = torch.sigmoid(model.logit_op)[vis]
        stats = {
            'psnr_before': before, 'psnr_after': after, 'psnr_before_list': before_list, 'psnr_after_list': after_list,
            'depth_ratio_before': dr_before, 'depth_ratio_after': dr_after,
            'n_splats': model.n, 'n_splats_init': int(len(orig_idx)), 'n_visible': n_vis,
            'n_observed': int(obs.sum()), 'n_new': int((~alive).sum()),
            'n_pruned_opacity': n_pruned_op, 'n_pruned_depth': n_pruned_depth, 'n_cloned': n_cloned, 'n_split': n_split,
            'disp_cm_unobserved_p90': float((model.means - model.init['means']).norm(dim=1)[vis & ~obs].quantile(0.9)) * CM_PER_UNIT if int((vis & ~obs).sum()) else None,
            'disp_native_median': float(d.median()), 'disp_native_p90': float(d.quantile(0.9)), 'disp_native_max': float(d.max()),
            'disp_cm_median': float(d.median()) * CM_PER_UNIT, 'disp_cm_p90': float(d.quantile(0.9)) * CM_PER_UNIT,
            ...
            'iters': a.iters, 'args': vars(a) | {k: str(getattr(a, k)) for k in ('data', 'out', 'pseudo_dir', 'init_npz')},
            'log': log, 'runtime_s': time.time() - t0}
    json.dump(stats, open(a.out / 'stats.json', 'w'), indent=1)
    np.savez(a.out / 'finetuned.npz', means=model.means.detach().cpu().numpy(), quats=model.quats.detach().cpu().numpy(),
             log_scales=model.log_scales.detach().cpu().numpy(), logit_opacities=model.logit_op.detach().cpu().numpy(),
             f_dc=model.f_dc.detach().cpu().numpy(), visible=vis.cpu().numpy(), orig_index=orig_idx.cpu().numpy(),
             scale0=s['scale0'], origin_c2w=s['origin_c2w'], R_raw_to_native=s['R_raw_to_native'], fb=s['fb'])
    print(f'PSNR before {before:.2f} -> after {after:.2f} dB; depth ratio {dr_before:.3f} -> {dr_after:.3f}; '
          ...
          f'opacity killed {stats["opacity_dropped_below_0.05"]}, new {stats["opacity_raised_above_0.5_from_below_0.1"]}; {time.time() - t0:.0f}s', flush=True)

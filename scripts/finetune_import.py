...
as a gaussian .ply. Inverse of finetune_export.py:  raw = scale0 * R^T (native - t[origin]).
    ...
    uv run --locked --group inference python scripts/finetune_import.py --npz run1/finetuned.npz --out public/marble-corridor-finetuned
"""
...
def write_spz(out: Path, xyz, alpha, f_dc, log_scale, quat_xyzw, fb: int, header_template: bytes):
    ...
    pos_b = np.stack([fixed & 0xFF, (fixed >> 8) & 0xFF, (fixed >> 16) & 0xFF], 2).astype(
        np.uint8
    )  # (n,3,3)
...
def main():
    ...
    ap.add_argument(
        "--world",
        default=ROOT / "public/marble-corridor-recon-f0-v2.spz",
        type=Path,
        help="original spz (header template + round-trip check)",
    )
    ...
    R = s["R_raw_to_native"]
    t = s["origin_c2w"][:3, 3]
    scale0 = float(s["scale0"])
    fb = int(s["fb"])
    xyz = (
        (s["means"].astype(np.float64) - t) @ R
    ) * scale0  # (R^T applied via right-multiplication)
    q_native = np.concatenate([s["quats"][:, 1:4], s["quats"][:, 0:1]], 1).astype(
        np.float64
    )  # wxyz -> xyzw
    ...
    frozen = (~vis) & (oi >= 0) & (oi < spz["n"])
    src = oi[frozen]  # oi indexes --init-npz, not Marble
    print(
        f"round trip on frozen splats: max |xyz - original| {np.abs(xyz[frozen] - spz['xyz'][src]).max():.2e}, "
        f"max |log_scale - original| {np.abs(log_scale[frozen] - spz['log_scale'][src]).max():.2e}, "
        f"max |quat.quat - 1| {np.abs(np.abs((quat[frozen] * spz['quat'][src]).sum(1)) - 1).max():.2e}",
        flush=True,
    )
    print(
        f"{len(xyz)} splats (original {spz['n']}: {(oi < 0).sum()} created, {spz['n'] - (oi >= 0).sum()} pruned); "
        f"moved {vis.sum()}; bbox raw {xyz.min(0).round(2)} .. {xyz.max(0).round(2)} (original {spz['xyz'].min(0).round(2)} .. {spz['xyz'].max(0).round(2)})",
        flush=True,
    )
    write_spz(a.out.with_suffix(".spz"), xyz, alpha, f_dc, log_scale, quat, fb, spz["header"])
    write_ply(
        a.out.with_suffix(".ply"),
        xyz.astype(np.float32),
        alpha.astype(np.float32),
        f_dc.astype(np.float32),
        log_scale.astype(np.float32),
        quat.astype(np.float32),
    )
    chk = read_spz(a.out.with_suffix(".spz"))
    print(
        f"wrote {a.out.with_suffix('.spz')} ({a.out.with_suffix('.spz').stat().st_size / 1e6:.1f} MB, re-read {chk['n']} splats, max pos err {np.abs(chk['xyz'] - xyz).max():.2e}), "
        f"{a.out.with_suffix('.ply')} ({a.out.with_suffix('.ply').stat().st_size / 1e6:.0f} MB)",
        flush=True,
    )

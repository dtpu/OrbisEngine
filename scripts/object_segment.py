...
Generalises `worker/experiments/worn_object_segment.py` (branch aayan/vm-objects) so the same
tool serves all four motion cases in docs/objects.md, not just worn ones:
...
class Models:
    ...
    def sam_points(self, im, inp, emb, points):
        ...
        """
        pi = self.sam_proc(im, input_points=[[[[float(x), float(y)]] for x, y in points]],
                           return_tensors='pt')
        with self.torch.no_grad():
            out = self.sam(input_points=pi['input_points'], image_embeddings=emb,
                           multimask_output=True)
        masks = self.sam_proc.image_processor.post_process_masks(
            out.pred_masks.cpu(), pi['original_sizes'].cpu(), pi['reshaped_input_sizes'].cpu())[0]
    ...
    def person(self, im):
        ...
        with self.torch.no_grad():
            ...
            up = F.interpolate(logits.float(), size=a.shape[:2], mode='bilinear', align_corners=False)
    ...
    def clipseg(self, im, prompts):
        ...
        import torch.nn.functional as F
        with self.torch.no_grad():
            inp = self.clip_proc(text=prompts, images=[im] * len(prompts), padding=True, return_tensors='pt')
            logits = self.clip(**inp).logits
        if logits.ndim == 2:
            logits = logits[None]
        up = F.interpolate(self.torch.sigmoid(logits)[:, None], size=(im.size[1], im.size[0]),
...
def bbox_of(m, pad, shape):
    ...
    return [int(max(0, xs.min() - pad)), int(max(0, ys.min() - pad)),
            int(min(W - 1, xs.max() + pad)), int(min(H - 1, ys.max() + pad))]
...
def roi_from_joint(tracks_dir, track, joint, radius, frames):
    ...
    return {int(f): (float(np.interp(f, si, xy[:, 0])), float(np.interp(f, si, xy[:, 1])), float(radius))
            for f in frames}
...
def roi_from_track2d(path, radius, frames):
    ...
    pts = t['observations'] if isinstance(t, dict) and 'observations' in t else (t['track'] if isinstance(t, dict) else t)
    ...
    lo, hi = si.min(), si.max()
    return {int(f): (float(np.interp(f, si, xs)), float(np.interp(f, si, ys)), float(radius))
            for f in frames if lo <= f <= hi}
...
def propagate_seeds(get, seeds, frames, win=21, levels=4):
    ...
    for direction in (1, -1):
        ...
        for i, f in enumerate(seq):
            ...
            p1, st, _ = cv2.calcOpticalFlowPyrLK(a, b, p0, None, winSize=(win, win), maxLevel=levels)
...
def cutout_for_clip(rgb, m, pad=0.25, bg=(124, 116, 104)):
    ...
    canvas = np.zeros((side, side, 3), np.uint8); canvas[:] = bg
...
def propose_and_score(M, big, prompts, args, prior=None, prev_centroid=None, seed_xy=None):
    ...
    k = int(order[0])
    diag.update(clipProb=round(float(probs[k]), 4), combined=round(float(score[k]), 4),
                bestPx=int(cands[k].sum()),
                top=[dict(px=int(cands[j].sum()), clip=round(float(probs[j]), 3),
                          combined=round(float(score[j]), 3)) for j in order[:5]])
    if args.debug_dir:
...
def main():
    ...
    ap.add_argument('--frames', default='', help='"a:b" or "a,b,c" source frame indices (default: all)')
    ap.add_argument('--prompt', required=True, help='what the object is, in words: "a clear plastic water bottle"')
    ap.add_argument('--negative-prompt', default='a person, a hand')
    ap.add_argument('--out', required=True)

    ap.add_argument('--roi-joint', default='', help='TRACK:JOINT:RADIUS, e.g. 1:21:110 (right wrist)')
    ...
    ap.add_argument('--seed-from-track2d', default='',
    ...
    ap.add_argument('--grid', type=int, default=8, help='sam-clip: NxN point prompts across the ROI')
    ap.add_argument('--clip-floor', type=float, default=0.01,
                    help='sam-clip: minimum CLIP margin, sim(prompt) - max sim(negatives)')
    ap.add_argument('--max-area-frac', type=float, default=0.35, help='sam-clip: largest proposal, as a fraction of the ROI')
    ap.add_argument('--dedupe-iou', type=float, default=0.85)
    ap.add_argument('--prior-weight', type=float, default=0.0, help='sam-clip: weight of the CLIPSeg prior, 0 disables it')
    ap.add_argument('--prev-sigma', type=float, default=0.35, help='sam-clip: centroid continuity, in ROI widths')
    ap.add_argument('--require-person', action='store_true',
    ...
    ap.add_argument('--min-area-px', type=int, default=60, help='in ROI-crop pixels, before rescaling back')
    ...
    if a.frames and ':' in a.frames:
        lo, hi = a.frames.split(':'); frames = list(range(int(lo), int(hi) + 1))
    elif a.frames:
    ...
    if a.clip:
        ...
        def get(f):
            return Image.open(os.path.join(a.images, names[f])).convert('RGB') if f < len(names) else None
    ...
    for f in frames:
        ...
        if f in rois:
            cx, cy, r = rois[f]
            x0 = int(np.clip(cx - r, 0, W - 1)); x1 = int(np.clip(cx + r, 1, W))
            y0 = int(np.clip(cy - r, 0, H - 1)); y1 = int(np.clip(cy + r, 1, H))
        else:
            x0, y0, x1, y1 = 0, 0, W, H
        if x1 - x0 < 8 or y1 - y0 < 8:
            row['skipped'] = 'roi degenerate'; rows.append(row); continue
        row['roi'] = [x0, y0, x1, y1]

        crop = im.crop((x0, y0, x1, y1))
        cw, ch = crop.size
        s = max(1.0, zoom_to / max(cw, ch))
        big = crop.resize((int(round(cw * s)), int(round(ch * s))), Image.LANCZOS) if s > 1.0 else crop
        ...
        if a.require_person and person_full.sum() < 500:
            row['skipped'] = 'no person'; rows.append(row); continue
        ...
        if a.method == 'clipseg' or a.prior_weight > 0:
            ...
            if peak >= a.clipseg_floor:
                ...
                row['skipped'] = f'clipseg peak {peak:.3f} below floor'; rows.append(row); continue
        ...
        if a.method == 'sam-clip':
            ...
            if f in seed_pts:
                ...
                if not (0 <= sxy[0] < big.size[0] and 0 <= sxy[1] < big.size[1]):
                    row['skipped'] = 'seed outside roi'; rows.append(row); continue
                row['seed'] = [round(seed_pts[f][0], 1), round(seed_pts[f][1], 1)]
            best, diag = propose_and_score(M, big, prompts, a, prior, prev_centroid, sxy)
            row.update(diag)
            if best is None:
                row['skipped'] = diag.get('reject', 'no proposal'); rows.append(row); continue
        else:
            seed = prior
            if seed is None or seed.sum() < 12:
                row['skipped'] = 'clipseg seed too small'; rows.append(row); continue
            ...
            neg = [pnt for pnt in neg if not seed[int(np.clip(pnt[1], 0, bh - 1)), int(np.clip(pnt[0], 0, bw - 1))]]
            ...
            for m, sc in zip(masks, sam_scores):
                ...
                inter = float((m & seed).sum()); union = float((m | seed).sum()) or 1.0
                iou = inter / union
                if m.sum() > 0.75 * bw * bh:
                    iou *= 0.1
                if iou > best_iou:
        ...
        if best is None or best.sum() < a.min_area_px:
            row['skipped'] = f'object {0 if best is None else int(best.sum())} px'; rows.append(row); continue
        ...
        if areas and (ratio > a.max_area_jump or ratio < 1.0 / a.max_area_jump):
            row['skipped'] = f'area jumped {ratio:.2f}x against the running median'; rows.append(row); continue
        areas.append(float(best.sum() / (s * s)))

        # RGBA cutout at crop-native resolution: the appearance stage's input
        crop_mask = np.asarray(Image.fromarray((best * 255).astype(np.uint8)).resize((cw, ch), Image.BILINEAR)) > 127
        cut = np.dstack([np.asarray(crop), (crop_mask * 255).astype(np.uint8)])
        ys2, xs2 = np.nonzero(crop_mask)
        if len(xs2) < 8:
            row['skipped'] = 'cutout empty after rescale'; rows.append(row); continue
        tb = [xs2.min(), ys2.min(), xs2.max() + 1, ys2.max() + 1]
        Image.fromarray(cut[tb[1]:tb[3], tb[0]:tb[2]]).save(os.path.join(a.out, 'object-crop', f'{f:05d}.png'))
        ...
        for sub, m in (('person', person_only), ('object', obj_full), ('dynamic', dynamic)):
            Image.fromarray((m * 255).astype(np.uint8)).save(os.path.join(a.out, sub, stem + '.png'))

        bys, bxs = np.nonzero(best)
        prev_centroid = (float(bxs.mean()), float(bys.mean()))
        cys, cxs = np.nonzero(obj_full)
        row.update(object_px=int(obj_full.sum()),
        ...
        rows.append(row)
        print(f"f{f}: object {int(obj_full.sum())} px  {row.get('clipProb', row.get('sam_iou_with_clipseg'))}", flush=True)

    if sheet:
        cols = min(8, len(sheet)); rn = (len(sheet) + cols - 1) // cols
    ...
    summary = dict(prompt=a.prompt, negativePrompt=a.negative_prompt, clip=a.clip or a.images,
                   frames=len(rows), framesWithObject=len(kept),
                   medianObjectPx=float(np.median([r['object_px'] for r in kept])) if kept else 0.0,
                   medianOverlapWithPerson=float(np.median([r['overlap_with_person_px'] for r in kept])) if kept else 0.0,
                   rejects={}, perFrame=rows)
    for r in rows:
        if 'skipped' in r:
            k = r['skipped'].split(' ')[0]

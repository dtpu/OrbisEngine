...
def load_mpu(path):
    ...
    d = json.load(open(path))
    for v in (
        d.get("floorFit", {}).get("metresPerWorldUnit"),
        d.get("metresPerWorldUnit"),
    ...
    if h:
        ...
        print(
            "no floor fit in %s; |g| from the 1.70 m stature prior over bodyHeightUnits %.3f"
            % (path, h)
        )
        return 1.70 / float(h)
        ...
        for i, j in set(wins):
                ...
                dh.append(
                    min(
                        (
                            float(
                                np.hypot(
                                    J[tr][joint][0] - sub[k]["x"], J[tr][joint][1] - sub[k]["y"]
                                )
                            )
                            for tr in J
                        ),
            ...
            keep.append(
                dict(
                    pts=sub,
                    parabolaResidPx=r,
                    imageGravityPxPerS2=g,
                    travelPx=travel,
                    handDistPx=[float(v) for v in dh],
                    medianHandDistPx=float(np.median(dh)),
                )
            )
    return keep
...
def dedupe(cands):
    ...
    for c in cands:
        f = sorted(c["pts"])
        k = (f[0], f[-1])
        if k not in best or (len(c["pts"]), -c["parabolaResidPx"]) > (
            len(best[k]["pts"]),
            -best[k]["parabolaResidPx"],
        ):
            best[k] = c
    return list(best.values())


def ballistic_screen(
    cands, cameras, people, fps, max_reproj_px, max_depth_m=30.0, max_speed_ms=25.0
):
    ...
    for c in cands:
        ...
        c = dict(
            c,
            ballisticReprojPxRms=best[0],
            ballisticReprojPxMax=best[1],
            speedMs=speed,
            depthM=float(best[3]),
        )
...
def body_screen(cands, P, dilate, max_overlap, max_aspect, min_fill, min_obs):
    ...
    for c in cands:
        ...
        c = dict(
            c,
            medianMaskOverlap=float(np.median(ov)),
            medianBboxAspect=float(np.median(asp)),
            medianFill=float(np.median(fill)),
        )
        c["rejected"] = None
        if c["medianMaskOverlap"] > max_overlap:
            c["rejected"] = (
                "body part: %.0f%% of the blob bbox lies inside the person mask (> %.0f%%)"
                % (100 * c["medianMaskOverlap"], 100 * max_overlap)
            )
        elif c["medianBboxAspect"] > max_aspect:
            c["rejected"] = "edge, not a compact mover: bbox aspect %.1f (> %.1f)" % (
                c["medianBboxAspect"],
                max_aspect,
            )
        elif c["medianFill"] < min_fill:
            c["rejected"] = (
                "edge, not a compact mover: blob fills %.0f%% of its bbox (< %.0f%%)"
                % (100 * c["medianFill"], 100 * min_fill)
            )
        elif len(c["pts"]) < min_obs:
            c["rejected"] = (
                "%d observations (< %d): too short to separate a parabola from noise"
                % (len(c["pts"]), min_obs)
            )
        out.append(c)
    return out
...
def merge_spans(cands, cameras, people, fps, max_reproj_px, max_gap, max_depth_m, max_speed_ms):
    ...
    while changed and len(cands) > 1:
        ...
        for i in range(len(cands) - 1):
            ...
            ok = ballistic_screen(
                [cand], cameras, people, fps, max_reproj_px, max_depth_m, max_speed_ms
            )
            if ok:
                cands[i : i + 2] = ok
                changed = True
...
def record(c, fps):
    ...
    return dict(
        ...
        parabolaResidPx=c["parabolaResidPx"],
        imageGravityPxPerS2=c["imageGravityPxPerS2"],
        travelPx=c["travelPx"],
        medianHandDistPx=c["medianHandDistPx"],
        handDistPx=c["handDistPx"],
        ballisticReprojPxRms=c.get("ballisticReprojPxRms"),
        ballisticReprojPxMax=c.get("ballisticReprojPxMax"),
        speedMs=c.get("speedMs"),
        depthM=c.get("depthM"),
        bboxPx=[[int(v) for v in pts[f]["bx"]] for f in fs],
        medianMaskOverlap=c.get("medianMaskOverlap"),
        medianBboxAspect=c.get("medianBboxAspect"),
        medianFill=c.get("medianFill"),
        rejected=c.get("rejected"),
        free=c.get("rejected") is None,
    )
...
def write_crops(clip, recs, out_dir, pad=1.5, min_side=48):
    ...
    while fi <= fs[-1]:
        ...
        for i, x, y, s in want.get(fi, []):
            ...
            recs[i].setdefault("crops", []).append(
                dict(
                    frame=fi,
                    box=[x0, y0, x1, y1],
                    laplacianVar=sharp,
...
def main():
    ...
    ap.add_argument(
        "--crops", default=None, help="directory for per-observation crops and contact sheets"
    )
    ...
    ap.add_argument(
        "--top-k", type=int, default=15, help="strongest movers kept per frame before linking"
    )
    ap.add_argument(
        "--max-span",
        type=int,
    ...
    if det is None:
        ...
        if a.cache:
            np.savez_compressed(
                a.cache,
                det=json.dumps({str(k): v for k, v in det.items()}),
                hf=np.array(sorted(Hs)),
                hm=np.array([Hs[k] for k in sorted(Hs)]),
            )

    tot = sum(len(v) for v in det.values())
    print(
        "candidates: %d over %d frames (median %.1f/frame)"
        % (tot, len(det), np.median([len(v) for v in det.values()])),
        flush=True,
    )

    det = top_k(det, a.top_k)
    print(
        "after keeping the %d strongest movers per frame: %d candidates"
        % (a.top_k, sum(len(v) for v in det.values())),
        flush=True,
    )
    ...
    bs = ballistic_screen(
        pf, a.cameras, a.people, a.fps, a.max_reproj_px, a.max_depth_m, a.max_speed_ms
    )
    print(
        "pass the 3D ballistic reprojection screen (<= %.1f px): %d" % (a.max_reproj_px, len(bs)),
        flush=True,
    )
    kept = nms(
        bs, key=lambda c: (c["ballisticReprojPxRms"] / max(len(c["pts"]), 1), -len(c["pts"]))
    )
    kept = merge_spans(
        kept,
        a.cameras,
    ...
    print(
        "after merging split arcs (gap <= %d frames, union must still fit): %d"
        % (a.merge_gap, len(kept)),
        flush=True,
    )
    ...
    print(
        "clear of the person mask, compact and long enough (overlap <= %.0f%%, aspect <= %.1f, fill >= %.0f%%, "
        ">= %d obs): %d, rejected %d"
        % (
            100 * a.max_mask_overlap,
            a.max_aspect,
            100 * a.min_fill,
            a.min_obs,
            len(kept),
    ...
    out = dict(
        schema="wander.object-flights/1",
        clip=a.clip,
        tracks=a.tracks,
        first=first,
        last=last,
        fps=a.fps,
        rule=(
            "a tracklet is a free span iff (1) after chaining the frame-to-frame homographies "
        ...
        candidateCount=tot,
        trackletCount=len(tl),
        parabolaSurvivors=len(pf),
        ballisticSurvivors=len(bs),
        flights=recs,
        rejectedCandidates=rej,
    )
    json.dump(out, open(a.out, "w"), indent=1)

    def line(r):
        return (
            "  f%-3d-%-3d  %.2f-%.2f s  %2d obs  parabola %.1f px  ballistic %.2f px rms  "
            ...
            % (
                ...
                r["nObs"],
                r["parabolaResidPx"],
                r["ballisticReprojPxRms"],
                r["speedMs"],
                r["depthM"],
                r["travelPx"],
                r["medianHandDistPx"],
                max(r["sizePx"]),
                100 * r["medianMaskOverlap"],
                r["medianBboxAspect"],
                100 * r["medianFill"],
            )
        )

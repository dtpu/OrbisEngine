...
interface PublishBase {
  ...
  archives: Record<string, AssetEntry>;
}
interface PublishTask extends ScannedFile {
  kind: 'files' | 'archives';
...
const cacheDir = path.join(ROOT, '.context/shared-storage');
...
let previous: { value: Partial<CatalogPointer & PublishBase>; etag: string | null | undefined } = {
  value: { files: {}, archives: {} },
  etag: null,
};
try {
  previous = await getJSON<CatalogPointer>(s3, config.catalogKey);
} catch (e) {
  if (
    (e as StorageFailure).name !== 'NoSuchKey' &&
    (e as StorageFailure).$metadata?.httpStatusCode !== 404
  )
    throw e;
}
...
const progress = setInterval(
  () =>
    console.log(
      `Published blobs ${done}/${tasks.length}; new ${uploaded}, ${(bytes / 1e9).toFixed(2)} GB uploaded`,
    ),
  15000,
);
async function worker() {
  while (cursor < tasks.length && !failed) {
    ...
    try {
      ...
      const sha =
        old?.size === t.size && old?.mtimeMs === t.mtimeMs ? old.sha256 : await hashFile(t.file);
            ...
            (async () => {
              ...
              for (let attempt = 0; ; attempt++) {
                try {
                  ...
                  if (
                    attempt >= 5 ||
                    (((e as StorageFailure).$metadata?.httpStatusCode as number) < 500 &&
                      ![408, 409, 429].includes(
                        (e as StorageFailure).$metadata?.httpStatusCode as number,
                      ))
                  )
                    throw e;
      ...
      if (after.size !== t.size || after.mtimeMs !== t.mtimeMs)
        throw new Error(`File changed during publish; retry after generation finishes: ${t.path}`);
      next[t.kind][t.path] = { key, size: t.size, sha256: sha, contentType: mime(t.path) };
      cache[t.file] = { size: t.size, mtimeMs: t.mtimeMs, sha256: sha };
      done++;
    } catch (e) {
      failed = e;
    }
...
try {
  ...
  for (const t of tasks) {
    ...
    if (s.size !== t.size || s.mtimeMs !== t.mtimeMs)
      throw new Error(`Changed during publish: ${t.path}; retry`);
  }
  const id = randomUUID();
  const archiveKey = `archive/snapshots/${id}.json`,
    snapshot = `viewer/snapshots/${id}.json`;
  const putJSON = (
    Key: string,
    value: unknown,
    condition: Pick<PutObjectCommandInput, 'IfMatch' | 'IfNoneMatch'> = {},
  ) =>
    s3.send(

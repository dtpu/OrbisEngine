...
"""The filmed silhouette as a per-person, per-sample measurement of where the avatar is.
    ...
    python scripts/silhouette_rows.py --world public/worlds/hpwide-4d --scale 0.5725 --floor-coef a,b,c
...
def load_track_masks(npz_path: Path):
    ...
    for k in z.files:
        if k == "shape":
            continue
        t, s = k.split("_")
        out.setdefault(int(t[1:]), {})[int(s[1:])] = np.unpackbits(z[k])[: h * w].reshape(h, w).astype(bool)
    return out, (h, w), ms
...
def mask_rows(m: np.ndarray, ms: float, full_h: int):
    ...
    return dict(top=float(top), bottom=float(bot),
...
def avatar_rows(P, R, C, K):
    ...
    if ok.sum() < 50:
        return None
    v = v[ok]
    return dict(top=float(np.percentile(v, TOP_PCT)), bottom=float(np.percentile(v, BOT_PCT)),
...
def measure(frames_world, cams_by_sample, masks, track, samples, hw_full, ms, feet_world, up):
    ...
    for i, (P, s) in enumerate(zip(frames_world, samples)):
        ...
        if mr is None or ar is None:
            out.append(None); continue
        F = feet_world[i]
        ray = F - C; z = float(np.linalg.norm(ray)); u = ray / z
        w = up - (up @ u) * u; w /= max(np.linalg.norm(w), 1e-9)
        eps = 0.01 * z
        d = {}
        ...
        out.append(dict(sample=int(s), mask=mr, avatar=ar, z=z, u=u.tolist(), w=w.tolist(),
...
def summarize(rows):
    ...
    for r in rows:
        ...
        if r["feetInFrame"]:
            ...
            hr.append((r["avatar"]["bottom"] - r["avatar"]["top"]) / max(1.0, r["mask"]["bottom"] - r["mask"]["top"]))
    q = lambda v: (float(np.median(v)) if len(v) else float("nan"))
    return dict(n=sum(r is not None for r in rows), nFeet=len(dbot), headRowOffsetPx=q(dtop),
                feetRowOffsetPx=q(dbot), heightRatio=q(hr))
...
if __name__ == "__main__":
    ...
    ap.add_argument("--floor-coef", default=None, help="a,b,c of y = a x + b z + c, for the floor-implied depth")
    a = ap.parse_args()
    root = Path(__file__).resolve().parents[1]
    man = json.loads((a.world / "people.json").read_text())
    clip = man["clip"]; name = Path(clip).stem
    ...
    cams_by_sample = {s: (by_src[si], a.scale, pos0) for s, si in enumerate(man["sourceIndices"]) if si in by_src}
    ...
    for p in man["people"]:
        ...
        rows = measure(frames_world, cams_by_sample, masks, p["track"], samples, hw_full, ms, feet_world, np.array([0, 1.0, 0]))
        ...
        for r, f in zip(rows, feet):
            ...
            zf = ""
            if coef is not None:
                C = np.array(cams_by_sample[r['sample']][0]["camera_to_world"])[:3, 3] * a.scale + pos0
                u = np.array(r["u"]); n = np.array([-coef[0], 1.0, -coef[1]]); d = coef[2]
                t = (d - C @ n) / (u @ n) if abs(u @ n) > 1e-6 else float("nan")
                zf = f" zFloor {t:6.3f} ({t / r['z']:.3f})"
            print(f"  s{r['sample']:02d} mask {r['mask']['top']:6.1f}-{r['mask']['bottom']:6.1f}{'c' if r['mask']['croppedBottom'] else ' '} "

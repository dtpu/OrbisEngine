// A synthetic immersive-vr session, injected before page scripts by scripts/verify-xr.mjs.
//
...
(() => {
  ...
  const EW = n('fakew', 2064), EH = n('fakeh', 2208), VFOV = n('fakevfov', 96), IPD = 0.063;
  ...
  const mul = (a, b) => {           // a * b, both column-major
  ...
  class Emitter {
    ...
    dispatchEvent(e) { for (const f of this._l[e.type] ?? []) f.call(this, e); }
  }
  ...
  class FakeLayer {
    ...
    getViewport(view) { return { x: view.eye === 'left' ? 0 : this._eyeW, y: 0, width: this._eyeW, height: this._eyeH }; }
  }
  ...
  class FakeFrame {
    ...
    getViewerPose() {
      ...
      const pm = proj(VFOV, EW / EH, this.session.renderState.depthNear, this.session.renderState.depthFar);
      return {
    ...
    getPose(space) { return { transform: { matrix: space._matrix ?? pose(0, 1.2, -0.2, 0) }, emulatedPosition: false }; }
  ...
  const source = (handedness, dx) => {
    ...
    const p = Math.cos(-0.61), q = Math.sin(-0.61);
    targetRaySpace._matrix = new Float32Array([1, 0, 0, 0, 0, p, q, 0, 0, -q, p, 0, dx, 1.1, -0.25, 1]);
    return {
      handedness, targetRayMode: 'tracked-pointer', targetRaySpace, gripSpace: targetRaySpace,
      profiles: ['oculus-touch-v3'], hand: undefined,
      gamepad: { get axes() { return drv.axes[handedness] ?? [0, 0, 0, 0]; }, buttons: [], mapping: 'xr-standard', connected: true },
    };
  };
  ...
  class FakeSession extends Emitter {
    ...
    async requestReferenceSpace() { return new FakeRefSpace(); }
  ...
  xr.requestSession = async (mode, init) => { drv.presenting = true; drv.frames.length = 0; return new FakeSession(mode, init); };

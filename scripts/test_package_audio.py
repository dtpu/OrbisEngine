...
def tone(path, channels=1, seconds=1, frequency=440):
    with wave.open(str(path), 'wb') as w:
        w.setnchannels(channels); w.setsampwidth(2); w.setframerate(8000)
        w.writeframes(b''.join(struct.pack('<h', round(math.sin(i * frequency / 8000 * math.tau) * 3000)) * channels for i in range(round(8000 * seconds))))


class Packaging(unittest.TestCase):
    def setUp(self):
        self.tmp = tempfile.TemporaryDirectory(); self.addCleanup(self.tmp.cleanup)
        self.root = Path(self.tmp.name); self.world = self.root / 'world'; self.world.mkdir()
        tone(self.world / 'original.wav', channels=2)
        self.base = {'schema': 'wander.audio/1', 'source': {'hasAudio': False}, 'timeline': {'durationSeconds': 1},
                     'original': {'url': 'original.wav', 'offsetSeconds': 0}, 'provenance': {'attribution': 'Owned fixture'}}
        (self.world / 'audio.json').write_text(json.dumps(self.base))
        tone(self.root / 'voice.wav'); tone(self.root / 'bed.wav', channels=2, frequency=220)
        (self.root / 'head.json').write_text(json.dumps({'positions': [[0, 1, 0], [1, 1, 0]], 'times': [0, 1]}))
        self.config = {'personIds': ['speaker'], 'tracks': [
            {'id': 'voice', 'kind': 'dialogue', 'file': 'voice.wav', 'personId': 'speaker', 'reviewed': True, 'anchorFile': 'head.json'},
    ...
    def test_package_retains_fallback_attribution_and_exact_samples(self):
        ...
        self.assertEqual((self.world / result['tracks'][0]['url']).read_bytes(), (self.root / 'voice.wav').read_bytes())
    ...
    def test_rejects_unreviewed_unknown_identity_and_bad_head(self):
        ...
        (self.root / 'head.json').write_text(json.dumps({'positions': [[0, 1, 0], [1, 1, 0]], 'times': [1, 0]}))

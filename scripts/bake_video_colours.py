...
Usage:
  uv run --locked --group inference python scripts/bake_video_colours.py --diag            # depth-ratio check on a few frames
  uv run --locked --group inference python scripts/bake_video_colours.py                   # full corridor bake (recon-f0-v2)
  uv run --locked --group inference python scripts/bake_video_colours.py --world public/marble-corridor-recon-f131.spz --origin-frame 549 --height 0.7965 --out public/marble-corridor-f131-baked
  uv run --locked --group inference python scripts/bake_video_colours.py --cameras public/worlds/tos-hall-walk-4d/cameras.json --anchors .context/tos/pi3x/anchors.npz \
...
def read_spz(path: Path):
    ...
    o = 16
    pos_b = np.frombuffer(raw, np.uint8, n * 9, o).reshape(n, 3, 3)
    o += n * 9
    alpha_b = np.frombuffer(raw, np.uint8, n, o)
    o += n
    col_b = np.frombuffer(raw, np.uint8, n * 3, o).reshape(n, 3)
    o += n * 3
    scale_b = np.frombuffer(raw, np.uint8, n * 3, o).reshape(n, 3)
    o += n * 3
    rot_b = np.frombuffer(raw, np.uint8, n * 3, o).reshape(n, 3)
    o += n * 3
...
def write_gaussian_ply(spz, rgb01: np.ndarray, out: Path):
    ...
    rows[:, 13] = q[:, 3]
    rows[:, 14:17] = q[:, 0:3]  # rot_0 = w, rot_1..3 = xyz
    names = [
        "x",
        "y",
        "z",
...
def read_wvp(path: Path):
    ...
    o = 8 + hl
    depth_rgba = np.frombuffer(raw, np.uint8, L * Hh * W * 4, o).reshape(L, Hh, W, 4)
    o += L * Hh * W * 4
    masks = np.frombuffer(raw, np.uint8, F * Hh * W, o).reshape(F, Hh, W)
    o += F * Hh * W
    assert o == len(raw)
    q = (depth_rgba[..., 0].astype(np.uint32) | (depth_rgba[..., 1].astype(np.uint32) << 8)).astype(
        np.float32
    ) / 65535.0
...
class WvpSource:
    ...
    def cam(self, f):
        return self.c2w[f], self.K, self.size

    def depth_map(self, f):
        dl = self.H["depthIndex"][f]
        return self.depth[dl], self.supported[dl]

    def mask(self, f):
        return self.masks[f].astype(np.float32) / 255.0

    def image(self, f):
        return self.read(f)

    def sample_of(self, f):
        return f
...
class JsonSource:
    ...
    def sample_of(self, f):
        return self.by_idx[f][0]

    def cam(self, f):
        c = self.by_idx[f][1]
        return (
            np.array(c["camera_to_world"], np.float64),
            np.array(c["source_intrinsics"], np.float32),
            tuple(c["source_image_size"]),
        )

    def depth_map(self, f):
        if self.depth_dir:
            d = np.load(self.depth_dir / f"depth_{f:04d}.npy").astype(np.float32)
    ...
    def mask(self, f):
        ...
        m = (
            np.asarray(
                Image.open(
                    self.mask_dir / self.pattern.format(sample=self.sample_of(f), idx=f)
                ).convert("L"),
                np.float32,
            )
            / 255.0
    ...
    def image(self, f):
        ...
        im = (
            Image.open(self.frames_dir / self.pattern.format(sample=self.sample_of(f), idx=f))
            .convert("RGB")
            .resize(self.vsize, Image.BILINEAR)
        )
...
def static_cloud_from_anchors(anchors: Path, cameras: Path, anchor_samples, conf_pct=20.0):
    ...
    print(
        f"static cloud: {len(pts)} points from {len(poses)} anchors, scale {scale:.4f}, bbox {pts.min(0).round(2)} .. {pts.max(0).round(2)}",
        flush=True,
    )
    return pts
...
def main():
    ...
    ap.add_argument(
        ...
        default=None,
        type=Path,
        help="corridor wvp container (default when --cameras is absent)",
    )
    ap.add_argument(
        "--cameras",
        default=None,
        type=Path,
        help="cameras.json (OpenGL camera_to_world, source_intrinsics, source_image_size)",
    )
    ...
    ap.add_argument(
        ...
        default=None,
        type=Path,
        help="image per registered camera instead of --clip",
    )
    ap.add_argument(
        "--frames-pattern",
        default="f_{sample:04d}.png",
        help="{sample} = camera index in cameras.json, {idx} = sourceIndex",
    )
    ap.add_argument(
        "--mask-dir",
        default=None,
        type=Path,
        help="optional person masks, same pattern (white = person)",
    )
    ap.add_argument(
        "--scene-points",
        default=None,
        type=Path,
        help="static point cloud .npy (N,3) in the cameras.json frame for per-camera depth",
    )
    ap.add_argument(
        "--anchors",
        default=None,
        type=Path,
        help="Pi3X anchors.npz -> static point cloud (cached next to it as static-scene.npy)",
    )
    ...
    ap.add_argument(
        ...
        default=None,
        type=Path,
        help="per-frame depth_{sourceIndex:04d}.npy instead of a point cloud",
    )
    ap.add_argument(
        "--depth-res",
        type=int,
        nargs=2,
        default=[480, 200],
        help="z-buffer resolution for point-cloud depth",
    )
    ap.add_argument(
        "--person",
        default=ROOT / "public/worlds/overnight-corridor-video-projection/person/frame_000.ply",
        type=Path,
    )
    ap.add_argument(
        "--height",
        type=float,
        default=0.828,
        help="person height in world units (the preset's ?height=)",
    )
    ap.add_argument("--scale0", type=float, default=None, help="override scale0 (native -> world)")
    ap.add_argument(
        "--origin-frame", type=int, default=0, help="source frame whose camera is the Marble origin"
    )
    ap.add_argument("--stride", type=int, default=1, help="use every Nth registered frame")
    ap.add_argument(
        "--size", type=int, nargs=2, default=[960, 540], help="decode size for the imagery"
    )
    ap.add_argument(
        "--tol",
        type=float,
        default=0.25,
        help="occlusion tolerance: reject when splat/map depth > 1+tol (soft to 1+2tol)",
    )
    ap.add_argument(
        "--front",
        type=float,
        default=0.35,
        help="down-weight splats nearer than the map by more than this ratio (0 = accept all, like the viewer)",
    )
    ...
    ap.add_argument(
        ...
        type=float,
        default=0.3,
        help="weight for texels the depth map does not support",
    )
    ap.add_argument("--min-alpha", type=float, default=0.05, help="skip near-transparent splats")
    ap.add_argument(
        "--count-sat", type=int, default=20, help="frame count at which coverage saturates"
    )
    ap.add_argument(
        "--std-max",
        type=float,
        default=0.25,
        help="colour std across frames at which consistency reaches 0",
    )
    ap.add_argument("--min-count", type=int, default=3)
    ap.add_argument(
        "--out", default=ROOT / "public/marble-corridor-baked", type=Path, help="output stem"
    )
    ap.add_argument(
        "--share", default=Path(__file__).resolve().parents[1] / ".context/share", type=Path
    )
    ...
    n = spz["n"]
    print(
        f"world: {a.world.name}, {n} splats, bbox {spz['xyz'].min(0)} .. {spz['xyz'].max(0)}",
        flush=True,
    )
    VW, VH = a.size
    if a.cameras:
        ...
        if a.depth_dir is None:
            if a.scene_points:
                ...
                if cache.exists():
                    ...
                    scene = static_cloud_from_anchors(
                        a.anchors, a.cameras, [int(x) for x in a.anchor_samples.split(",")]
                    )
                    np.save(cache, scene)
            else:
        ...
        src = JsonSource(
            ...
            frames_pattern=a.frames_pattern,
            scene_points=scene,
        ...
        src = WvpSource(
            a.wvp
            or ROOT / "public/worlds/overnight-corridor-video-projection/video-projection.wvp",
            a.clip or ROOT / "public/clips/corridor-walk-elder.mp4",
            (VW, VH),
        )
    ...
    A = origin_c2w[:3, :3] @ flip / scale0
    b = origin_c2w[:3, 3]
    print(
        f"source: {len(src.frames)} registered frames; bodyH {bodyH:.4f} native -> scale0 {scale0:.4f}; origin frame {a.origin_frame} at {b.round(3)}",
        flush=True,
    )

    P = torch.from_numpy((spz["xyz"] @ A.T + b).astype(np.float32)).to(dev)
    Nrm = torch.from_numpy(
        (splat_normals(spz["quat"], spz["log_scale"]) @ (origin_c2w[:3, :3] @ flip).T).astype(
            np.float32
        )
    ).to(dev)
    ...
    sum_rgb = torch.zeros(n, 3, device=dev)
    sum_rgb2 = torch.zeros(n, 3, device=dev)
    sum_w = torch.zeros(n, device=dev)
    count = torch.zeros(n, device=dev, dtype=torch.int32)
    ratio_log = []
    frames = src.frames[:: a.stride]
    fig_frames = (
        [int(x) for x in a.fig_frames.split(",")]
        if a.fig_frames
        else [src.frames[0], src.frames[len(src.frames) // 2]]
    )
    diag_frames = (
        [src.frames[int(k * (len(src.frames) - 1))] for k in (0, 0.2, 0.5, 0.75, 1.0)]
        if a.diag
        else None
    )
    src_imgs = {}
    for f in (f for f in src.frames if f in frames or f in fig_frames):
        ...
        if m is None:
            ...
            MH, MW = m.shape
            mt = torch.from_numpy(np.ascontiguousarray(m)).to(dev).reshape(-1)
            mk = mt[
                torch.clamp((vv * (MH / SH_)).long(), 0, MH - 1) * MW
                + torch.clamp((uu * (MW / SW)).long(), 0, MW - 1)
            ]
        ...
        if a.diag:
            ...
            print(
                f"frame {f}: {idx.numel()} splats in frustum, {sp.float().mean().item() * 100:.0f}% on supported depth, log(splat/map depth) median {np.median(lr):+.3f} (ratio {np.exp(np.median(lr)):.3f}), p10 {np.exp(np.percentile(lr, 10)):.3f} p90 {np.exp(np.percentile(lr, 90)):.3f}",
                flush=True,
            )
            continue
        un = uu / SW
        vn = vv / SH_
        e = (
            smoothstep(0, a.feather, un)
            * smoothstep(0, a.feather, 1 - un)
            * smoothstep(0, a.feather, vn)
            * smoothstep(0, a.feather, 1 - vn)
        )
        ...
        view = P[idx] - camPos
        view = view / torch.clamp(view.norm(dim=1, keepdim=True), 1e-6)
        ...
        if frames.index(f) % 50 == 0:
            print(
                f"frame {f}: {idx.numel()} in frustum, {(w > 0.02).sum().item()} weighted, {time.time() - t0:.0f}s",
                flush=True,
            )
    ...
    if a.diag:
        ...
        ax.set_xlabel("log(Marble splat depth / source depth)  (0 = agree)")
        ax.legend()
        ax.set_title(
            f"{a.world.name}, scale0 {scale0:.3f}: Marble geometry vs source depth along the same rays"
        )
        fig.tight_layout()
        fig.savefig(a.share / f"{a.tag}-depth-ratio.png", dpi=110)
        print(f"wrote {a.share / (a.tag + '-depth-ratio.png')}")
        return

    sum_rgb = sum_rgb.cpu().numpy()
    sum_rgb2 = sum_rgb2.cpu().numpy()
    ...
    std = np.sqrt(var)
    conf = np.where(
        observed, np.clip(count / a.count_sat, 0, 1) * np.clip(1 - std / a.std_max, 0, 1), 0
    ).astype(np.float32)
    frac = observed.mean()
    print(
        f"colour std across frames, observed splats: median {np.median(std[observed]):.3f}, p90 {np.percentile(std[observed], 90):.3f}; {100 * (std[observed] < 0.1).mean():.0f}% under 0.1",
        flush=True,
    )
    print(
        f"observed: {observed.sum()} / {n} splats ({100 * frac:.1f}%), of live {100 * observed[spz['alpha'] > a.min_alpha].mean():.1f}%; median count among observed {np.median(count[observed]):.0f}; runtime {time.time() - t0:.0f}s",
        flush=True,
    )
    ...
    np.savez_compressed(
        str(out) + "-bake.npz",
        sum_rgb=sum_rgb.astype(np.float32),
        sum_w=sum_w.astype(np.float32),
        count=count.astype(np.int16),
        scale0=scale0,
        origin_frame=a.origin_frame,
    )
    print(
        f"wrote {out.with_suffix('.ply')} ({out.with_suffix('.ply').stat().st_size / 1e6:.0f} MB), {out.with_suffix('.spz')} ({out.with_suffix('.spz').stat().st_size / 1e6:.0f} MB), {str(out) + '-weights.bin'}",
        flush=True,
    )

    fig, ax = plt.subplots(1, 2, figsize=(11, 4))
    ax[0].hist(count[count > 0], bins=60, range=(0, count.max() + 1), color="#4a7ebb")
    ax[0].set_xlabel("source frames observing the splat (count > 0 only)")
    ax[0].set_ylabel("splats")
    ax[0].set_yscale("log")
    ax[0].set_title(
        f"{100 * (count > 0).mean():.1f}% of {n} splats seen by >= 1 frame, {100 * frac:.1f}% observed (>= {a.min_count} frames)"
    )
    ax[1].hist(std[observed], bins=50, range=(0, 0.4), color="#c46a2a")
    ax[1].set_xlabel("colour std across observing frames (0 = every frame agrees)")
    ax[1].set_ylabel("splats")
    ax[1].set_title(f"weights.bin = min(1, count/{a.count_sat}) * max(0, 1 - std/{a.std_max})")
    fig.tight_layout()
    fig.savefig(a.share / f"{a.tag}-coverage.png", dpi=110)
    ...
    def render(rgb, f, offset=(0, 0, 0), res=(480, 270)):
        c2w, K, (SW, SH_) = src.cam(f)
        c = c2w.copy()
        c[:3, 3] += c[:3, :3] @ np.array(offset)
        w2c = np.linalg.inv(c).astype(np.float32)
        cam = Pn @ w2c[:3, :3].T + w2c[:3, 3]
        z = -cam[:, 2]
        ok = (z > 0.1) & (spz["alpha"] > 0.3)
        zs = np.where(ok, z, 1)
        uu = (K[0, 2] + K[0, 0] * cam[:, 0] / zs) * res[0] / SW
        vv = (K[1, 2] - K[1, 1] * cam[:, 1] / zs) * res[1] / SH_
        ok &= (uu >= 0) & (uu < res[0]) & (vv >= 0) & (vv < res[1])
        ii = np.nonzero(ok)[0]
        pix = vv[ii].astype(int) * res[0] + uu[ii].astype(int)
        order = np.argsort(-z[ii])
        pix = pix[order]
        ii = ii[order]
        img = np.zeros((res[1] * res[0], 3), np.float32)
        img[pix] = rgb[ii]
    ...
    for f in fig_frames:
        s_img = (
            np.asarray(Image.fromarray(src_imgs[f]).resize((480, 270), Image.BILINEAR)).astype(
                np.float32
            )
            / 255
        )
        ...
        print(
            f"frame {f}: mean |render - source| over covered pixels: Marble {errs[0]:.3f}, baked {errs[1]:.3f}",
            flush=True,
        )
    f0, f1 = fig_frames
    ...
    rows = [
        (f0, (0, 0, 0), f"frame {f0} pose"),
        (f1, (0, 0, 0), f"frame {f1} pose"),
        (f1, (side, 0, 0), f"frame {f1} + 0.5 m right (off path)"),
    ]
    fig, ax = plt.subplots(len(rows), 3, figsize=(15, 3 * len(rows) * SH_ / SW * 16 / 9))
    for i, (f, off, label) in enumerate(rows):
        ax[i, 0].imshow(src_imgs[f])
        ax[i, 0].set_title(f"source frame {f}" + ("" if off == (0, 0, 0) else " (for reference)"))
        ax[i, 1].imshow(render(marble_rgb, f, off))
        ax[i, 1].set_title(f"Marble colour, {label}")
        ax[i, 2].imshow(render(baked, f, off))
        ax[i, 2].set_title(f"baked colour, {label}")
        for j in range(3):
            ax[i, j].axis("off")
    fig.tight_layout()
    fig.savefig(a.share / f"{a.tag}-before-after.png", dpi=100)
    print(
        f"wrote {a.share / (a.tag + '-coverage.png')}, {a.share / (a.tag + '-before-after.png')}",
        flush=True,
    )

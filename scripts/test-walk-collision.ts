...
import {
  ...
  withExplicitWalkSupport,
  type WalkCollisionGrid,
} from '../src/walk-collision.ts';
...
test('explicit support admits outside-hull floor without clearing obstacles', () => {
  const cell = { floor: NaN, inside: 0, dist: 17, solid: 128 };
  assert.deepEqual(withExplicitWalkSupport(cell, -3.1), {
    floor: -3.1,
    inside: 1,
    dist: 0,
    solid: 128,
  });
  assert.equal(cell.inside, 0);
});

test('off-edge or invalid support retains the original outside-hull refusal', () => {
  const cell = { floor: NaN, inside: 0, dist: 17, solid: 0 };
  for (const support of [undefined, NaN, Infinity])
    assert.equal(withExplicitWalkSupport(cell, support), cell);
  assert.equal(withExplicitWalkSupport({ ...cell, floor: 1 }, 0).floor, 1);
});

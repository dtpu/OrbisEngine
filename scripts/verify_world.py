...
Three steps, all unattended:
  ...
  3. ask the VLM (worker/experiments/vlm_judge.py, OPENAI_API_KEY) what specifically differs, on a
...
def main():
    ...
    ap.add_argument('--scale0', type=float, default=1.0, help='SfM-to-world scale; --fit-scale finds it')
    ap.add_argument('--frames', default=None, help='comma-separated sourceIndex (default: 5 spread over the clip)')
    ...
    ap.add_argument('--prompt-file', default=None, help='JSON from scripts/world_prompt.py, for the correction step')
    ap.add_argument('--model', default='gpt-6-astra')
    ap.add_argument('--pass-score', type=int, default=55, help='median score at or above which the world passes')
    ...
    if a.fit_scale:
        ...
        for c in cameras_for(a.cameras, frames):
            ...
            sc = 480 / sw
            K = np.array(c['source_intrinsics'], np.float64) * sc; K[2, 2] = 1.0
            cap.set(cv2.CAP_PROP_POS_FRAMES, c['sourceIndex'])
            ok, r = cap.read()
            if not ok:
                continue
            reals.append(cv2.resize(r, (480, int(round(sh * sc))))); ks.append(K)
        ...
        for s0 in grid:
            ...
            for real, K, c2w in zip(reals, ks, c2ws):
                ...
                gc.append(grad_corr(real, cv2.cvtColor((np.clip(img, 0, 1) * 255).astype(np.uint8), cv2.COLOR_RGB2BGR)))
    ...
    for c in cameras_for(a.cameras, frames):
        ...
        sw, sh = c['source_image_size']
        s = a.width / sw
        K = np.array(c['source_intrinsics'], np.float64) * s; K[2, 2] = 1.0
        H = int(round(sh * s))
        ...
        ok, real = cap.read()
        if not ok:
            print(f'cannot read frame {fi}', file=sys.stderr); continue
        real = cv2.resize(real, (a.width, H), interpolation=cv2.INTER_AREA)
        pair = np.hstack([label(real, f'REAL  clip frame {fi}', (80, 255, 255)),
                          label(ren, f'WORLD  {Path(a.world).name}  same pose')])
        p = out / f'pair-f{fi}.png'
        cv2.imwrite(str(p), pair)
        per_frame.append(dict(frame=fi, pair=str(p),
    ...
    cv2.imwrite(str(sheet_p), cv2.resize(sheet, (sheet.shape[1] // 2, sheet.shape[0] // 2), interpolation=cv2.INTER_AREA))
    if a.share:
        cv2.imwrite(str(Path(a.share) / f'{a.tag}-verify.png'), cv2.imread(str(sheet_p)))

    report = dict(world=a.world, clip=a.clip, cameras=a.cameras, scale0=a.scale0,
                  scaleCurve=curve,
                  frames=per_frame, sheet=str(sheet_p))
    ...
    if not a.no_vlm:
        if 'OPENAI_API_KEY' not in os.environ:
            ...
            if median is not None and median < a.pass_score:
                ...
                report['correction'] = ask_images(a.model, [f['pair'] for f in per_frame], msg, detail='low')

...
CLIPS = {
    ...
    'elevator': ('elevator-4d', '.context/run/elevator/pi3x', 'marble-elevator-clean.spz',   -0.6145, 1.0800),
    'lobby':    ('lobby-4d',    '.context/own/lobby/pi3x',    'marble-lobby-clean.spz',      -0.5464, 1.5300),
    'atrium':   ('atrium-4dpp', '.context/run/atrium/pi3x',   'marble-atrium-clean.spz',     -0.9015, 1.0221),
    'living':   ('living-4dpp', '.context/own/living/pi3x',   'marble-living-finetuned.spz', -0.7257, 0.3750),
    'stairs2':  ('stairs2-4d',  '.context/run/stairs2/pi3x',  'marble-stairs2-clean.spz',    -0.9026, 0.9760),
    'tos31':    ('tos31-4d',    None,                         'marble-tos31-image.spz',      -0.5894, 1.2834),
    'selfie':   ('selfie-4d',   '.context/run/selfie/pi3x',   'marble-selfie-finetuned.spz', -2.5498, 1.4560),
    'bedroom':  ('bedroom-4d',  '.context/own/bedroom/pi3x',  'marble-bedroom-clean.spz',    -1.696,  0.673),
}
...
def align_rotation(g):
    ...
    g = unit(g); y = np.array([0., 1., 0.])
    v = np.cross(g, y); s = np.linalg.norm(v); c = float(g @ y)
    if s < 1e-12:
        return np.eye(3)
...
def read_ply_xyz(p: Path):
    ...
    return np.stack([d[:, props.index('x')], d[:, props.index('y')], d[:, props.index('z')]], 1).astype(np.float64)
...
def est_track(C):
    ...
    r = C[:, 1] - A @ k; ss = ((C[:, 1] - C[:, 1].mean()) ** 2).sum()
    P = C[:, [0, 2]] - C[:, [0, 2]].mean(0); s = np.linalg.svd(P, compute_uv=False)
    return unit([-k[0], 1., -k[1]]), float(1 - (r ** 2).sum() / ss if ss > 0 else np.nan), float(s[1] / s[0])
...
def est_walls(P, seed, clip='', k=24, nsamp=120000):
    ...
    B = P[nb]; B = B - B.mean(1, keepdims=True)
    ...
    def basin(g0):
        g = unit(g0); nw = 0
        for _ in range(40):
            wall = np.abs(N @ g) < np.sin(np.radians(12)); nw = int(wall.sum())
            if nw < 500:
                return None, 0
            _, v2 = np.linalg.eigh(N[wall].T @ N[wall]); gn = v2[:, 0]
            if gn @ g < 0: gn = -gn
            if ang(gn, g) < 1e-3:
                g = gn; break
            g = gn
    ...
    ax = unit(np.cross(unit(seed), [1., 0., 0.]) if abs(unit(seed)[0]) < 0.9 else np.cross(unit(seed), [0., 0., 1.]))
    seeds = [seed, np.array([0., 1., 0.]),
             align_rotation(ax * np.sin(np.radians(30)) + unit(seed) * np.cos(np.radians(30))).T @ unit(seed),
...
def est_body(world):
    ...
    if not sj.exists():
        return None, 0
    seq = json.load(open(sj)); base = sj.parent
    fr = seq['frames'][::max(1, len(seq['frames']) // 60)]
    A = []
    for fn in fr:
        X = read_ply_xyz(base / fn); X = X - X.mean(0)
        _, _, vt = np.linalg.svd(X, full_matrices=False); a = vt[0]
        A.append(-a if a[1] < 0 else a)
    A = np.array(A)
    _, v = np.linalg.eigh(A.T @ A); g = v[:, -1]
...
def world_floor(spz, seed_y):
    ...
    X = s['xyz'].copy(); X[:, 1] *= -1; X[:, 2] *= -1
    R = X[s['alpha'] > 0.5].astype(np.float64)
    if seed_y is None or not np.isfinite(seed_y) or not ((R[:, 1] > seed_y - 0.2) & (R[:, 1] < seed_y + 0.2)).any():
        h, e = np.histogram(R[:, 1], 400); seed_y = float(0.5 * (e[h.argmax()] + e[h.argmax() + 1]))
    c = np.array([0., 0., seed_y]); inl = np.zeros(len(R), bool)
    for band in (0.20, 0.12, 0.055, 0.055):
        for _ in range(10):
            d = R[:, 1] - (R[:, 0] * c[0] + R[:, 2] * c[1] + c[2]); inl = np.abs(d) < band
            if inl.sum() < 500:
                break
...
def clip_context(clip):
    ...
    for fr in frames.values():
        for op, _ in fr[::max(1, len(fr) // 40)]:
            ...
            probe.append([op[f, 0].mean() * scale, op[f, 2].mean() * scale]); foot.append(lo * scale)
    probe = np.array(probe)
    mx, mz, footY = float(np.median(probe[:, 0])), float(np.median(probe[:, 1])), float(np.median(foot))
    coef, nin, band = None, 0, None
    for b in (0.45, 0.90, 1.60):
        reg = (np.abs(W[:, 0] - mx) < 1.6) & (np.abs(W[:, 2] - mz) < 2.0) & (np.abs(W[:, 1] - footY) < b)
        if reg.sum() < 5000:
            continue
    ...
    tilt = float(np.degrees(np.arctan(np.hypot(coef[0], coef[1])))) if coef is not None else float('nan')
    ...
    return dict(W=W, frames=frames, coef=coef, planeInliers=int(nin), planeTiltDeg=tilt,
                planeOk=bool(plane_ok), planeBand=band, stature=stature, footY=footY,
                mpu=1.70 / (stature * scale), scale=scale, floor=floor, walked=(mx, mz))


def contact_series(frames, R, scale, coef, pos0=(0., 0., 0.)):
    """Feet (1st percentile of opaque splats, measured along the WORLD's up) minus the plane."""
    out = []
    for op, _ in frames:
        P = op @ R.T
        y = P[:, 1]; ys = np.sort(y); low = float(ys[int(len(ys) * 0.01)])
...
def analyse(clip):
    ...
    g_walls = g_floor = None; n_walls = n_floor = 0; camh = camspan = float('nan'); dfloor = 0.0
    if pi3x and (ROOT / pi3x / 'static-scene.npy').exists():
        P = np.load(ROOT / pi3x / 'static-scene.npy').astype(np.float64)
        Cs = np.array([np.array(c['camera_to_world'])[:3, 3]
    ...
    cands = {}
    if g_walls is not None: cands['walls'] = g_walls
    if g_floor is not None: cands['floor'] = g_floor
    ...
    if body_ok: cands['body'] = g_body
    ref = dict(cands); ref['track'] = g_track
    if g_body is not None and not body_ok: ref['body'] = g_body

    ctx = clip_context(clip)
    cwf, niw = ctx['coef'], ctx['planeInliers']
    nwf = unit([-cwf[0], 1., -cwf[1]])
    ...
    trials = {}
    for name, gc in list(ref.items()) + [('none', np.array([0., 1., 0.]))]:
        ...
        trials[name] = dict(tiltDeg=tilt_from_y(gc),
    ...
    if clip in CONTACT_CLIPS and ctx['planeOk'] and cands:
        ...
        if trials['none']['driftCm'] <= trials[src]['driftCm']:
            ...
            src, g = 'none', np.array([0., 1., 0.])
            rule = (f'NONE: the feet drift {trials["none"]["driftCm"]:.1f} cm raw, less than under any '
        ...
        src, g, rule = 'floor', g_floor, 'floor (seeded by walls); no walking contact, so the trend rule cannot judge'
    ...
    return dict(
        pos0Units=pos0, perPersonUnits=percon,
        clip=clip, world=world, cameras=str(campath.relative_to(ROOT)), spz=spz,
        gravitySfm=list(map(float, g)), source=rule, adopted=src,
        candidates=trials, estimatorSpreadDeg=spread, metresPerWorldUnitAvatar=ctx['mpu'],
        rotationRowMajor=[[float(x) for x in r] for r in R], quaternionXYZW=quat_xyzw(R),
        ...
        recommended=bool(clip in CONTACT_CLIPS and ctx['planeOk'] and src not in ('none', 'track')
                         and spread < 6.0 and trials[src]['driftCm'] < 0.25 * trials['none']['driftCm']),
        angleDeg=ang(g, nwf), tiltFromYDeg=tilt_from_y(g),
        estimators=dict(
            track=dict(g=list(map(float, g_track)), tiltDeg=tilt_from_y(g_track), r2=r2, anisotropy=aniso),
            walls=None if g_walls is None else dict(g=list(map(float, g_walls)), tiltDeg=tilt_from_y(g_walls), normals=n_walls),
            floor=None if g_floor is None else dict(g=list(map(float, g_floor)), tiltDeg=tilt_from_y(g_floor),
                                                    inliers=n_floor, camHeightUnits=camh, camHeightSpanUnits=camspan,
                                                    vsWallsDeg=ang(g_floor, g_walls)),
            body=None if g_body is None else dict(g=list(map(float, g_body)), tiltDeg=tilt_from_y(g_body), frames=n_body),
        ),
        worldFloor=dict(normal=list(map(float, nwf)), coef=[float(x) for x in cwf],
                        tiltDeg=ctx['planeTiltDeg'], inliers=niw, ok=ctx['planeOk'],
                        bandUnits=ctx['planeBand'], footYUnits=ctx['footY']),
        note=('Rotation to apply to everything in the SfM frame -- cameras.json, person PLYs, '
...
def refit_constant(ctx, R, scale):
    ...
    """
    med = {t: float(np.median(contact_series(fr, R, scale, ctx['coef'])))
...
def residuals(clip, verbose=True):
    ...
    def score(RR, sc, p0, extra=None, pos_xz=(0., 0.)):
        segs, ends = [], []
        for t, fr in ctx['frames'].items():
            r = contact_series(fr, RR, sc, coef, [p0[0] + pos_xz[0], p0[1], p0[2] + pos_xz[1]])
            if extra is not None:
                k = min(len(r), len(extra)); r = r[:k] + extra[:k]
            segs.append(r - np.median(r)); ends.append(trend_endpoints(r))
        v = np.concatenate(segs)
        return dict(rmsCm=float(np.sqrt(np.mean(v ** 2)) * mpu * 100),
    ...
    cen = lambda RR: np.median(np.concatenate(
        [[(op @ RR.T).mean(0) * scale for op, _ in fr] for fr in ctx['frames'].values()]), 0)
    d = cen(np.eye(3)) - cen(R)

    out = dict(clip=clip, tiltDeg=fa['tiltFromYDeg'], adopted=fa['adopted'],
               metresPerWorldUnitAvatar=mpu, planeOk=ctx['planeOk'],
               planeTiltDeg=ctx['planeTiltDeg'], planeInliers=ctx['planeInliers'],
               contact=clip in CONTACT_CLIPS, estimatorSpreadDeg=fa['estimatorSpreadDeg'])
    out['raw'] = score(np.eye(3), scale, [0, 0, 0])
    out['rotation'] = score(R, scale, [0, 0, 0], pos_xz=(float(d[0]), float(d[2])))
    if pj.exists():
        P = json.load(open(pj))
        out['shipped_placesolve'] = score(np.eye(3), P['registrationScale'], P['pos0'],
                                          extra=np.array(P['offsetUnits'])[:, 1])
    if ppl.exists():
        cd = json.load(open(ppl)).get('floorFit', {}).get('sharedCameraDrift', {}).get('offsetYUnits')
        if cd:
            out['shipped_camdrift'] = score(np.eye(3), scale, [0, 0, 0], extra=np.array(cd))
    if verbose:
        if not ctx['planeOk']:
            print(f"{clip:9s} REFUSED: the world floor under his feet fits at {ctx['planeTiltDeg']:.1f} deg "
                  f"with {ctx['planeInliers']} inliers, so no centimetre here would mean anything", flush=True)
            return out
        f = lambda k: '      --       ' if k not in out else f"{out[k]['rmsCm']:5.1f} / {out[k]['driftCm']:5.1f}"
        flag = '' if clip in CONTACT_CLIPS else '   (no walking contact: not a metric)'
        print(f"{clip:9s} {out['tiltDeg']:5.1f}d {out['adopted']:5s} +-{out['estimatorSpreadDeg']:4.1f}d | "
              f"raw {f('raw')} | rot {f('rotation')} | camdrift {f('shipped_camdrift')} | "
              f"place_solve {f('shipped_placesolve')}{flag}", flush=True)
    return out
def scale_report(clip):
    ...
    for op, _ in frames[::max(1, len(frames) // 40)]:
        P = op @ R.T; ys = np.sort(P[:, 1]); lo = ys[int(len(ys) * 0.01)]
        f = P[:, 1] < lo + (P[:, 1].max() - P[:, 1].min()) * 0.04
        pr.append([P[f, 0].mean() * preset_scale, P[f, 2].mean() * preset_scale])
    pr = np.array(pr); mx, mz = float(np.median(pr[:, 0])), float(np.median(pr[:, 1]))
    fy = float(coef[0] * mx + coef[1] * mz + coef[2])
    reg = (np.abs(W[:, 0] - mx) < 1.5) & (np.abs(W[:, 2] - mz) < 2.0)
    yy = W[reg][:, 1]; yy = yy[(yy > fy - 0.6) & (yy < fy + 4.0)]
    hh, ee = np.histogram(yy, 160); cen = 0.5 * (ee[:-1] + ee[1:]); abv = cen > fy + 0.9
    ...
    def cam_m(s, RR):
        ...
        return float(h * 1.70 / (stature * s)), float(np.ptp(P[:, 1] - (coef[0] * P[:, 0] + coef[1] * P[:, 2] + coef[2])) * 1.70 / (stature * s))
    ...
    return dict(clip=clip, statureNative=stature, presetScale=preset_scale,
                camMPreset=cam_m(preset_scale, np.eye(3))[0], camSpanPreset=cam_m(preset_scale, np.eye(3))[1],
                camMPresetRot=cam_m(preset_scale, R)[0], camSpanPresetRot=cam_m(preset_scale, R)[1],
                floorToCeilUnits=f2c_u, scaleFromCamera=s_cam, scaleFromCeiling=s_ceil,
                scaleCameraRange=[s_cam_lo, s_cam_hi], scaleCeilingRange=[s_ceil_lo, s_ceil_hi],
...
def solve_raw(pi3x: Path, clip: str, anchor_samples: str | None = None):
    ...
    if not scene.exists():
        ...
        n = len(cams)
        samples = ([int(x) for x in anchor_samples.split(',')] if anchor_samples
    ...
    g_track, r2, aniso = est_track(C)
    g_walls, n_walls = est_walls(P, g_track, clip)
    seed = g_walls if g_walls is not None else np.array([0., 1., 0.])
    ...
    c0 = np.array(cams[0]['camera_to_world'], float); R0 = orthonormal(c0[:3, :3])
    ...
    spread = max((ang(a, b) for a in cands.values() for b in cands.values()), default=0.0)
    return dict(
        schema='wander.framealign/2', clip=clip, cameras=str(cams_p), staticScene=str(scene),
        gravityRaw=[float(x) for x in unit(g)], gravityCam0=[float(x) for x in g_cam],
        tiltFromYDeg=tilt_from_y(g_cam), source=rule, adopted=src, estimatorSpreadDeg=spread,
        rotationRowMajor=[[float(x) for x in r] for r in R], quaternionXYZW=quat_xyzw(R),
        cameraHeightUnits=camh, cameraHeightSpanUnits=camspan, floorInliers=int(n_floor),
        estimators=dict(
            track=dict(g=[float(x) for x in g_track], tiltDeg=tilt_from_y(g_track), r2=r2, anisotropy=aniso),
            walls=None if g_walls is None else dict(g=[float(x) for x in g_walls], tiltDeg=tilt_from_y(g_walls), normals=int(n_walls)),
            floor=dict(g=[float(x) for x in g_floor], tiltDeg=tilt_from_y(g_floor), inliers=int(n_floor),
        ...
        note='Read by scripts/sfm_frame.camera0_reframe: the packagers, bake_video_colours and '
             'finetune_export_bedroom keep camera 0 at the origin and put gravityRaw on +y. '
...
def main():
    ...
    ap.add_argument('--write', action='store_true', help='solve: write framealign.json beside the manifest; graph: beside the Pi3X cameras.json')
    ap.add_argument('--pi3x', type=Path, help='graph: the run\'s Pi3X directory (cameras.json, anchors.npz)')
    ap.add_argument('--anchor-samples', default=None, help='graph: anchor sample indices if static-scene.npy has to be built')
    a = ap.parse_args()
    ...
    if a.cmd == 'graph':
        ...
        print(f"{a.clip}: camera 0 is {r['tiltFromYDeg']:.1f} deg off gravity, adopted {r['adopted']} "
              ...
              f"over the floor ({r['floorInliers']} inliers)", flush=True)
        if a.write:
    ...
    if a.cmd == 'scale':
        print(f"{'clip':9s} {'preset s':>8s} | {'camera height, m, at the preset scale':^40s} | "
              f"{'s: camera 1.15-1.85 m':>21s} {'s: ceiling 2.3-2.9 m':>20s} | {'defensible s':>13s}")
        for clip in clips:
            r = scale_report(clip)
            cl, ch = r['scaleCameraRange']; el, eh = r['scaleCeilingRange']
            lo, hi = max(cl, el), min(ch, eh)
            band = 'no overlap' if not (lo <= hi) else f'{lo:.2f}-{hi:.2f}'
            if not np.isfinite(el): band = f'{cl:.2f}-{ch:.2f} (cam only)'
            print(f"{r['clip']:9s} {r['presetScale']:8.4f} | raw {r['camMPreset']:5.2f} span {r['camSpanPreset']:5.2f}"
                  f"  ->  rot {r['camMPresetRot']:5.2f} span {r['camSpanPresetRot']:4.2f} | "
                  f"{cl:9.2f}-{ch:<10.2f} {el:8.2f}-{eh:<10.2f} | {band:>13s}", flush=True)
        return

    if a.cmd == 'residuals':
        print('contact residual after ONE constant per person, RMS / trend across the clip, cm in the', flush=True)
        print('avatar-metre (1.70 m over the stature). The TREND is the held-out number: nothing fits it.', flush=True)
        for clip in clips:
            residuals(clip)
        return

    print(f"{'clip':9s} | {'tilt deg + feet drift over the clip, cm (avatar-metre)':^62s} | "
          f"{'':^22s} | adopted", flush=True)
    rows = []
    for clip in clips:
        r = analyse(clip); rows.append(r)
        c = r['candidates']
        cel = lambda k: '    --      ' if k not in c else f"{c[k]['tiltDeg']:5.1f} {c[k]['driftCm']:5.1f}"
        print(f"{clip:9s} | walls {cel('walls')} | floor {cel('floor')} | body {cel('body')} | "
              ...
              f"{r['worldFloor']['inliers']:7d} inl {'OK ' if r['worldFloor']['ok'] else 'BAD'}", flush=True)
...
# ---------------------------------------------------------------- residual comparison
# Appended: `python3 scripts/frame_align.py residuals` scores the SAME contact quantity under three

...
"""Package an lhm_animate.py motion run as public/worlds/<clip>-4d/person/ in the corridor layout.
  ...
  worker/.venv-da3/bin/python scripts/package_person_sequence.py .context/<clip>/lhm-motion \
...
def quat_from_matrix(R):
    ...
    m = R; t = np.trace(m)
    if t > 0:
        s = np.sqrt(t + 1) * 2; w = s / 4; x = (m[2, 1] - m[1, 2]) / s; y = (m[0, 2] - m[2, 0]) / s; z = (m[1, 0] - m[0, 1]) / s
    elif m[0, 0] > m[1, 1] and m[0, 0] > m[2, 2]:
        s = np.sqrt(1 + m[0, 0] - m[1, 1] - m[2, 2]) * 2; w = (m[2, 1] - m[1, 2]) / s; x = s / 4; y = (m[0, 1] + m[1, 0]) / s; z = (m[0, 2] + m[2, 0]) / s
    elif m[1, 1] > m[2, 2]:
        s = np.sqrt(1 + m[1, 1] - m[0, 0] - m[2, 2]) * 2; w = (m[0, 2] - m[2, 0]) / s; x = (m[0, 1] + m[1, 0]) / s; y = s / 4; z = (m[1, 2] + m[2, 1]) / s
    else:
        s = np.sqrt(1 + m[2, 2] - m[0, 0] - m[1, 1]) * 2; w = (m[1, 0] - m[0, 1]) / s; x = (m[0, 2] + m[2, 0]) / s; y = (m[1, 2] + m[2, 1]) / s; z = s / 4
...
def transform_ply(src, dst, R, t):
    ply = PlyData.read(src); v = ply["vertex"].data.copy()
...
def main():
    ...
    ap.add_argument("motion"); ap.add_argument("--cameras", required=True); ap.add_argument("--clip", required=True)
    ap.add_argument("--out", required=True)
    ap.add_argument("--lower-body", default="Source-observed: legs and feet are in frame for the whole shot; independent MultiHMR pose per sample, no inferred gait",
                    help="lowerBodyProvenance text written to sequence.json (say so when legs are hidden or cut)")
    a = ap.parse_args()
    motion = Path(a.motion); out = Path(a.out); person = out / "person"; person.mkdir(parents=True, exist_ok=True)
    ...
    for name in seq["frames"]:
        ...
        counts.add(n); finite_all &= finite
        hashes.append(hashlib.sha256((person / name).read_bytes()).hexdigest()); frames.append(name)
    ...
    for c in cams["cameras"]:
        m = np.array(c["camera_to_world"]); m[:3, :3] = orthonormal(m[:3, :3])
        m2 = np.eye(4); m2[:3, :3] = R @ m[:3, :3]; m2[:3, 3] = R @ m[:3, 3] + t
    ...
    (out / "cameras.json").write_text(json.dumps(dict(
        ...
        frameAlign=fa,
        source=str(a.cameras), sourceClip=str(a.clip), cameras=out_cams), indent=1))

    seq_out = dict(seq)
    seq_out.update(frames=frames, count=len(frames), frame_sha256=hashes, gaussiansPerFrame=sorted(counts)[0] if len(counts) == 1 else sorted(counts),
                   ...
                   camera0Reframe=dict(rotation=R.tolist(), translation=t.tolist(), source="cameras.json camera 0 of the Pi3X export"),
                   lowerBodyProvenance=a.lower_body,
                   sourceClip=str(a.clip), sourceSha256=hashlib.sha256(Path(a.clip).read_bytes()).hexdigest())
    (person / "sequence.json").write_text(json.dumps(seq_out, indent=2))
    print(json.dumps(dict(frames=len(frames), gaussians=sorted(counts), finite=finite_all, out=str(out)), indent=1))

...
def _poses(track_dir):
    if track_dir not in _POSE_CACHE:
        ...
        _POSE_CACHE[track_dir] = (
            np.asarray(ps["sourceIndices"], float),
...
def joints2d(tracks_json, joint):
    ...
    def at(sf):
        return {
            t: np.array([np.interp(sf, idx, uv[:, k]) for k in range(2)])
            for t, (idx, uv) in per.items()
        }
...
def fit_flight(
    ...
    hand_thr = wrist_world(
        os.path.join(tracks_dir, f"track_{thrower:02d}"),
        None,
        C,
        scale[thrower],
        [f0 - 1, f0],
    ...
    hand_cat = wrist_world(
        os.path.join(tracks_dir, f"track_{catcher:02d}"),
        None,
        C,
        scale[catcher],
        [f1, f1 + 1],
    ...
    def resid(p, use_hands):
        ...
        if use_hands:
            r = np.concatenate(
                [r, (P[0] - anchor_release) / sig_hand, (P[-1] - anchor_catch) / sig_hand]
            )
        return r
    ...
    for name, hands in (("imageOnly", False), ("handAnchored", True)):
        ...
        fits[name] = dict(
            p0=s.x[:3].tolist(),
            v0=s.x[3:6].tolist(),
            reprojPxRms=float(np.sqrt((rp**2).mean())),
            reprojPxMax=float(rp.max()),
    ...
    return dict(
        ...
        catcher=int(catcher),
        joint=int(joint),
        frames=[int(f) for f in fl],
        timesSec=tsec.tolist(),
        observations2d=uv.tolist(),
        fits=fits,
        anchorRelease=anchor_release.tolist(),
        anchorCatch=anchor_catch.tolist(),
        spanM=float(np.linalg.norm(P[-1] - P[0]) * mpu),
        apexYUnits=float(P[:, 1].max()),
        flightSec=float(tsec[-1]),
        constantY=float(obj_const),
        driftCorrected=True,
    )
...
def main():
    ...
    ap.add_argument(
        "--tracks", help="tracks.json, for reading 2D wrists when picking thrower/catcher"
    )
    ap.add_argument(
        "--thrower", type=int, default=None, help="override; default is the nearest wrist"
    )
    ...
    for frames, uv in spans:
        ...
        print(
            "f%-4d-%-4d  %2d obs  thrower %d -> catcher %d  reproj rms %.2f px max %.2f  "

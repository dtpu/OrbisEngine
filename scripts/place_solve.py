...
# ---------------------------------------------------------------- io
def read_ply(p: Path):
    buf = p.read_bytes(); hi = buf.index(b'end_header\n') + 11
    head = buf[:hi].decode()
...
def world_splats(spz_path: Path, min_alpha=0.5):
    ...
    s = read_spz(spz_path)
    W = s['xyz'].copy(); W[:, 1] *= -1; W[:, 2] *= -1
    return W[s['alpha'] > min_alpha]


def person_frames(seq_json: Path):
    seq = json.load(open(seq_json)); base = seq_json.parent
...
def fit_plane(R, y0_pct=12, band=0.055, iters=12, seed=None):
    ...
    n = np.array([-coef[0], 1.0, -coef[1]]); L = np.linalg.norm(n)
...
def bench_pad_plane(bench_spz: Path, base_spz: Path):
    ...
    """
    A = read_spz(bench_spz); B = read_spz(base_spz)
    assert A['n'] > B['n'], (A['n'], B['n'])
    add = A['xyz'][B['n']:].copy(); add[:, 1] *= -1; add[:, 2] *= -1
    rng = np.random.default_rng(0)
    best = None
    for _ in range(400):
        i = rng.choice(len(add), 3, replace=False)
        p0, p1, p2 = add[i]
        nrm = np.cross(p1 - p0, p2 - p0); L = np.linalg.norm(nrm)
        if L < 1e-6:
            continue
        nrm = nrm / L
        if nrm[1] < 0:
    ...
    n = np.array([-c[0], 1.0, -c[1]]); L = np.linalg.norm(n)
    n, d = n / L, c[2] / L
    pad = add[np.abs(add @ n - d) < 0.02]
    # in-plane frame: t1 up-slope (steepest ascent in the plane), t2 lateral
    up = np.array([0.0, 1.0, 0.0])
    t1 = up - (up @ n) * n; t1 /= np.linalg.norm(t1)
    t2 = np.cross(n, t1)
    return dict(n=n, d=float(d), t1=t1, t2=t2, pad=pad, nAdded=int(len(add)), nPad=int(len(pad)),
...
def measure_floor(frames, scale, pos0, plane_coef, foot_band=0.04):
    ...
    for k, (op, _all) in enumerate(frames):
        y = op[:, 1]
        ys = np.sort(y); low = float(ys[int(len(ys) * 0.01)])
...
def measure_pad(frames, scale, pos0, pad, lat_cm=14.0, mpu=1.0, pct=6.0):
    ...
    for k, (op, _all) in enumerate(frames):
        ...
        al = P @ t1
        lt = P @ t2
        lt0 = float(np.median(lt[np.abs(s) < 0.6 / mpu])) if np.isfinite(s).any() else float(np.median(lt))
        keep = (np.abs(lt - lt0) < lat) & (al > lo_pad) & (al < hi_pad) & (s > -0.8 / mpu) & (s < 1.2 / mpu)
        per = []
        ...
        rows.append(dict(k=k, res=float(np.median(per)) if len(per) >= 6 else float('nan'),
                         n=len(per)))
    return rows
    ...
    for p_ in range(nP):
        for f in range(nF):
            ...
            if p_ < nk:
                ...
                r[2 * nF:] = -1.0 / sigma_c
            rows.append(r); rhs.append(-M[p_, f] / sigma_c)
    nData = len(rows)
    for f in range(nF):
        r = np.zeros(N); r[f] = Wa[f]; rows.append(r); rhs.append(0.0)
        r = np.zeros(N); r[nF + f] = Wb[f]; rows.append(r); rhs.append(0.0)
    for i in range(nF - 2):
        for off in (0, nF):
            r = np.zeros(N)
            r[off + i], r[off + i + 1], r[off + i + 2] = np.sqrt(lam), -2 * np.sqrt(lam), np.sqrt(lam)
...
def reproj_cost(cameras_json, sample_src, P_world, D_world, scale):
    ...
    for i, si in enumerate(sample_src):
        ...
        c2w = np.array(c['camera_to_world']); K = np.array(c['source_intrinsics'])
        ...
        u = ray / z
        D = D_world[i]
        par = float(D @ u); perp = float(np.linalg.norm(D - par * u))
...
CLIPS = {
    ...
    'gym': dict(world='public/marble-gym-clean2.spz', base='public/marble-gym-clean2.spz',
                seq=['public/worlds/gym-4d/person/sequence.json'], cameras='public/worlds/gym-4d/cameras.json',
                scale=1.0020, sizeScale=0.5800, floor=-0.7097, pos=None, contact='floor'),
    'elevator': dict(world='public/marble-elevator-clean.spz',
                     people='public/worlds/elevator-4d/people.json', cameras='public/worlds/elevator-4d/cameras.json',
                     scale=1.0800, floor=-0.6145, pos=[0.0, 0.0, 0.0], contact='floor'),
    'lobby': dict(world='public/marble-lobby-clean.spz',
                  seq=['public/worlds/lobby-4d/person/sequence.json'], cameras='public/worlds/lobby-4d/cameras.json',
                  scale=1.5300, floor=-0.5464, pos=None, contact='floor'),
    'living': dict(world='public/marble-living-finetuned.spz',
                   seq=['public/worlds/living-4dpp/person/sequence.json'], cameras='public/worlds/living-4dpp/cameras.json',
                   scale=0.3750, floor=-0.7257, pos=None, contact='floor'),
    ...
    'atrium': dict(world='public/marble-atrium-clean.spz',
                   seq=['public/worlds/atrium-4dpp/person/sequence.json'], cameras='public/worlds/atrium-4dpp/cameras.json',
                   scale=1.0221, sizeScale=0.8160, floor=-0.9015, pos=None, contact='floor'),
}
...
def load_tracks(cfg):
    ...
    if 'people' in cfg:
        man = json.load(open(ROOT / cfg['people'])); base = (ROOT / cfg['people']).parent
        out = []
        for p in man['people']:
...
def stats_cm(v, mpu):
    v = np.asarray(v, float); v = v[np.isfinite(v)] * mpu * 100.0
    if not len(v):
        return dict(n=0)
    return dict(n=int(len(v)), median=float(np.median(v)), mean=float(v.mean()),
                p10=float(np.percentile(v, 10)), p90=float(np.percentile(v, 90)),
                rms=float(np.sqrt((v ** 2).mean())), absmed=float(np.median(np.abs(v))),
...
def ray_geometry(cameras_json, sample_src, feet_world, n, scale, stature_world):
    ...
    for i, si in enumerate(sample_src):
        ...
        K = np.array(c['source_intrinsics']); f = float((K[0, 0] + K[1, 1]) / 2)
        d = feet_world[i] - C; z = float(np.linalg.norm(d))
        u = d / max(z, 1e-9)
        al = float(u @ n); be = float(np.sqrt(max(0.0, 1.0 - al * al)))
        w = (n - al * u)
        w = w / max(np.linalg.norm(w), 1e-9)
        s_px = f * stature_world / max(z, 1e-9)
        out.append(dict(C=C, u=u, w=w, z=z, f=f, alpha=al, beta=be,
                        costA=s_px / (2 * z), costB=f / z))
    return out


def silhouette_fit(feats, tracks, cameras, rows_per_track, scale, pos0, normal,
                   masks_path, sigma_px, sigma_contact, jerk, stature, size_prior):
    ...
    for pi, (tid, frames, tr, _low, seq) in enumerate(feats):
        ...
        for fi, (fr, src) in enumerate(zip(frames, seq['sourceIndices'])):
            ...
            u = F - C; depth = np.linalg.norm(u); u /= max(depth, 1e-9)
            w = normal - normal.dot(u) * u; w /= max(np.linalg.norm(w), 1e-9)
            data.append(dict(P=P, F=F, R=R, C=C, K=K, mask=mr, u=u, w=w,
                             contact=foot['res'], depth=depth))
        ...
        for _iteration in range(12):
            ...
            def add(i, jac, residual, sigma):
                row = np.zeros(len(x)); row[2*i:2*i+2] = np.asarray(jac[:2]) / sigma
                row[-1] = jac[2] / sigma
                equations.append(row); targets.append(-residual / sigma)
            for i,d in enumerate(data):
                ...
                other = avatar_rows(d['F'] + body*np.exp(.001) + D, d['R'], d['C'], d['K'])
                jac.append([(other[k]-ar[k])/.001 for k in ('top','bottom')])
                jac = np.array(jac).T
                mr = d['mask']
                if not mr['croppedTop'] and not mr['croppedBottom']:
                    # Explicit height term constrains depth; centre constrains registration.
                    add(i, jac[1]-jac[0], (ar['bottom']-ar['top'])-(mr['bottom']-mr['top']), sigma_px * np.sqrt(2))
                    add(i, (jac[0]+jac[1])/2, (ar['top']+ar['bottom']-mr['top']-mr['bottom'])/2, sigma_px / np.sqrt(2))
                else:
                    k = 'bottom' if mr['croppedTop'] else 'top'
                    add(i, jac[1 if k == 'bottom' else 0], ar[k]-mr[k], sigma_px)
                add(i, [normal.dot(d['u']), normal.dot(d['w']), 0], d['contact']+normal.dot(D), sigma_contact)
            for i in range(nf):
                for axis in range(2):
                    row = np.zeros(len(x)); row[2*i+axis] = 1 / (4*stature)
                    equations.append(row); targets.append(-row.dot(x))
            row = np.zeros(len(x)); row[-1] = 1 / max(size_prior, 1e-6)
            equations.append(row); targets.append(-row.dot(x))
            ...
            for i in range(1, nf-1):
                for axis in range(3):
                    ...
                    for j,c in ((i-1,1),(i,-2),(i+1,1)):
                        row[2*j:2*j+2] = c*np.array([basis[j]['u'][axis],basis[j]['w'][axis]]) / jerk
                    equations.append(row); targets.append(-row.dot(x))
        ...
        for i,d in enumerate(data):
            ...
            if not d['mask']['croppedTop'] and not d['mask']['croppedBottom']:
                heights.append((ar['bottom']-ar['top'])/(d['mask']['bottom']-d['mask']['top']))
        ...
        result[tid] = dict(offsetUnits=offsets.tolist(), sizeScale=size, sourceIndices=seq['sourceIndices'],
                           measuredFrames=len(contacts), heightRatioMedian=float(np.median(heights)) if heights else None,
                           contactResidualUnits=contacts, edgeResidualsPx=edge_errors, sigmaPx=sigma_px)
    return result


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument('--clip', required=True, help='a name in CLIPS, or any clip name with --world/--person')
    ...
    ap.add_argument('--fail-on-size', action='store_true',
    ...
    ap.add_argument('--sigma-contact-cm', type=float, default=2.5, help='the foot/back measurement noise floor')
    ap.add_argument('--sigma-px', type=float, default=12.0, help='source pixels of reprojection drift that cost as much as sigma-contact')
    ap.add_argument('--silhouette-sigma-px', type=float, default=1.0, help='tracked silhouette edge noise in source pixels; 0 disables the term')
    ap.add_argument('--silhouette-size-prior', type=float, default=0.15, help='log-size prior width for one fixed body scale per track; 0 locks size')
    ap.add_argument('--masks', type=Path, help='tracked masks.npz; defaults to .context/run/<clip>/tracks/masks.npz')
    ap.add_argument('--measure-only', action='store_true')
    ap.add_argument('--scale', type=float, default=None, help='override the registration scale')
    ap.add_argument('--floor', type=float, default=None)
    a = ap.parse_args()
    if a.world:
        rel = lambda q: str(Path(q).resolve().relative_to(ROOT)) if Path(q).is_absolute() else str(q)
        cfg = dict(world=rel(a.world), base=rel(a.world), cameras=rel(a.cameras), contact=a.contact,
                   scale=a.scale, floor=a.floor, pos=None)
        if a.people:
        ...
        if a.clip not in CLIPS:
            sys.exit(f'unknown clip {a.clip}; pass --world/--person/--cameras/--scale/--floor instead')
    ...
    for tid, seq, tr, _man in tracks:
        ...
        op0 = fr[0][0]
        ys = np.sort(op0[:, 1]); l0 = float(ys[int(len(ys) * 0.01)])
        if low0 is None:
    ...
    scale = cfg['scale']
    pos0 = np.array(cfg['pos'], float) if cfg['pos'] is not None else np.array([0.0, cfg['floor'] - low0 * scale, 0.0])
    ...
    for op, _al in feats[0][1][::10]:
        Q = op - op.mean(0)
        v = np.linalg.svd(Q[rng.choice(len(Q), min(5000, len(Q)), replace=False)], full_matrices=False)[2][0]
        pr = op @ v; st.append(np.percentile(pr, 99.3) - np.percentile(pr, 0.7))
    stature = float(np.median(st)) * scale
    mpu = 1.70 / stature                      # metres per world unit, if the person is 1.70 m
    print(f'  scale {scale}  pos0 {pos0.round(4)}  stature {stature:.4f} u  ->  1 u = {mpu:.4f} m', flush=True)
    ...
    if cfg['contact'] == 'pad':
        ...
        print(f"  pad: {pad['nPad']:,}/{pad['nAdded']:,} added splats, incline {pad['inclineDeg']:.1f} deg", flush=True)
        meta['pad'] = dict(normal=pad['n'].tolist(), d=pad['d'], inclineDeg=pad['inclineDeg'], nPad=pad['nPad'])
        rows_per_track = [measure_pad(fr, scale, pos0 + tr, pad, mpu=mpu) for _t, fr, tr, _l, _s in feats]
        normal = pad['n']
        probe_xyz = None
    else:
        probe = []
        for _t, fr, tr, _l, _s in feats:
            for op, _al in fr[::max(1, len(fr) // 40)]:
                ys = np.sort(op[:, 1]); lo = float(ys[int(len(ys) * 0.01)])
                f = op[:, 1] < lo + (op[:, 1].max() - op[:, 1].min()) * 0.04
                probe.append([op[f, 0].mean() * scale + pos0[0] + tr[0], 0, op[f, 2].mean() * scale + pos0[2] + tr[2]])
        ...
        footY = float(np.median([r['y'] for r in measure_floor(feats[0][1][::max(1, len(feats[0][1]) // 25)],
                                                              scale, pos0 + feats[0][2], [0.0, 0.0, 0.0])]))
        reg = (np.abs(W[:, 0] - mx) < 1.2) & (np.abs(W[:, 2] - mz) < 1.6) & (np.abs(W[:, 1] - footY) < 0.45)
        ...
        print(f'  floor plane y = {coef[0]:+.4f}x {coef[1]:+.4f}z {coef[2]:+.4f}, tilt {tilt:.2f} deg, '
              f'{nin:,} inliers; y at walked centre {yc:.4f} (preset floor {cfg["floor"]})', flush=True)
        meta['floorPlane'] = dict(coef=coef.tolist(), normal=nrm.tolist(), tiltDeg=tilt, inliers=nin, yAtWalkedCentre=yc)
    ...
    hs = np.array([(np.array(c['camera_to_world'])[:3, 3] * scale)[1]
                   - (cf[0] * (np.array(c['camera_to_world'])[:3, 3] * scale)[0]
                      + cf[1] * (np.array(c['camera_to_world'])[:3, 3] * scale)[2] + cf[2]) for c in cams])
    ...
    yy = W[reg2][:, 1]; yy = yy[(yy > cfg['floor'] - 0.6) & (yy < cfg['floor'] + 4.0)]
    hh, ee = np.histogram(yy, 160); cen = 0.5 * (ee[:-1] + ee[1:]); abv = cen > cfg['floor'] + 0.55
    ...
    meta['sizeCheck'] = dict(statureUnits=stature, metresPerWorldUnit=mpu, cameraHeightM=camM,
                             floorToCeilingM=f2c, pass_=bool(size_ok), ceilingPlausible=bool(ceil_ok),
                             ...
                             cameraHeightTrace=dict(
                                 ...
                                 medianM=camM, p10M=float(np.percentile(trace, 10)),
                                 p90M=float(np.percentile(trace, 90)),
                                 endToEndSlopeM=slope_m, drifted=bool(drifted),
                                 verdict=('drift' if drifted else
                                          'flat in band' if size_ok else 'flat out of band'),
    ...
    print(f'  SIZE CHECK: source camera {camM:.2f} m above the floor (want 1.15-1.85), '
          f'room {f2c:.2f} m floor to ceiling ({"plausible" if ceil_ok else "IMPLAUSIBLE"}) '
          f'-> {"PASS" if size_ok else "FAIL"}', flush=True)
    print(f'  CAMERA TRACE: {trace.min():.2f}-{trace.max():.2f} m over {len(trace)} cameras, '
          f'end to end {slope_m:+.2f} m -> {meta["sizeCheck"]["cameraHeightTrace"]["verdict"]}', flush=True)
    if not size_ok:
        ...
        want = 1.45 / camM
        msg = (f'SIZE CHECK FAILED for {a.clip}. Taking the person as 1.70 m makes 1 world unit '
    ...
    for kk, sv in before.items():
        print(f"    {kk:11s} n {sv['n']:4d}  median {sv['median']:+6.1f}  p10 {sv['p10']:+6.1f}  p90 {sv['p90']:+6.1f}  "
              f"rms {sv['rms']:5.1f}  |med| {sv['absmed']:4.1f}", flush=True)

    if a.measure_only:
        if a.report:
            json.dump(dict(clip=a.clip, mpu=mpu, scale=scale, pos0=pos0.tolist(), before=before, meta=meta,
    ...
    feet_world = np.array([[r['x'], r['y'], r['z']] if 'x' in r else [pos0[0], pos0[1], pos0[2]]
                           for r in rows_per_track[0]])
    if cfg['contact'] == 'pad':
        feet_world = np.array([[np.median(op[:, 0]) * scale + pos0[0], np.median(op[:, 1]) * scale + pos0[1],
                                np.median(op[:, 2]) * scale + pos0[2]] for op, _al in feats[0][1]])
    G = ray_geometry(ROOT / cfg['cameras'], src[:len(feet_world)], feet_world, normal, scale, stature)
    ...
    A = np.array([g['alpha'] for g in G]); B = np.array([g['beta'] for g in G])
    ...
    print(f'\n  ray geometry: alpha (ray.normal) {A.min():+.2f}..{A.max():+.2f}; '
          ...
          f'{np.median([g["costB"] for g in G]):.0f} px', flush=True)

    lam, av, bv, kv, edf = choose_lambda(M, A, B, Wa, Wb, sigma_c, a.max_jerk_cm / 100.0 / mpu)
    delta = av[:, None] * np.array([g['u'] for g in G]) + bv[:, None] * np.array([g['w'] for g in G])
    fit = av[None, :] * A[None, :] + bv[None, :] * B[None, :] + kv[:, None]
    after_M = M + fit
    aft = {feats[i][0]: stats_cm(after_M[i], mpu) for i in range(len(feats))}
    slide = np.abs(av) * mpu * 100; across = np.abs(bv) * mpu * 100
    px = np.abs(av) * np.array([g['costA'] for g in G]) + np.abs(bv) * np.array([g['costB'] for g in G])
    print(f'  SOLVE: lam {lam:.4g}, effective dof {edf:.1f} of {nF} samples', flush=True)
    print(f'    slide along the ray  median {np.median(slide):5.1f} cm  max {slide.max():5.1f} cm', flush=True)
    print(f'    move across the ray  median {np.median(across):5.1f} cm  max {across.max():5.1f} cm', flush=True)
    print(f'    reprojection cost    median {np.median(px):5.1f} px  p90 {np.percentile(px,90):5.1f} px  (1920 px source)', flush=True)
    ...
    for kk in aft:
        if aft[kk]['rms'] < a.sigma_contact_cm:
            refusals.append(f"{kk}: after RMS {aft[kk]['rms']:.2f} cm is below the "
        ...
        if all(abs(aft[kk][f] - before[kk][f]) < 1e-4 for f in ('median', 'rms', 'p10', 'p90')):
            refusals.append(f"{kk}: after equals before to four decimals; this solve changed nothing")
    ...
    out = a.out or ((ROOT / cfg['people']).parent if 'people' in cfg else tracks[0][1].parent.parent) / 'placement.json'
    doc = dict(
        schema='wander.placement/1', clip=a.clip,
        note=('Per-sample placement correction, solved by scripts/place_solve.py. The viewer applies it '
        ...
        world=cfg['world'], registrationScale=scale, floorY=float(cf[2] if 'floorPlane' in meta else cfg['floor']),
        pos0=pos0.tolist(), metresPerWorldUnit=mpu, samples=int(nF), contact=cfg['contact'],
        surfaceNormal=[float(x) for x in normal], sourceIndices=[int(x) for x in src[:nF]],
        lambdaGcv=float(lam), effectiveDof=float(edf),
        sigmaContactCm=a.sigma_contact_cm, sigmaPx=a.sigma_px,
        ...
        perPerson={feats[i][0]: dict(constantUnits=[float(x) for x in (feats[i][2] + kv[i] * normal)],
                                     solvedCm=float(kv[i] * mpu * 100),
                                     manifestTranslation=[float(x) for x in feats[i][2]])
                   for i in range(len(feats))},
        reprojectionCostPx=dict(median=float(np.median(px)), p90=float(np.percentile(px, 90)), max=float(px.max())),
    ...
    if a.silhouette_sigma_px > 0 and masks_path.exists() and cfg['contact'] == 'floor':
        fits = silhouette_fit(feats, tracks, cams, rows_per_track, scale, pos0, normal,
                              masks_path, a.silhouette_sigma_px, sigma_c,
                              a.max_jerk_cm / 100 / mpu, stature, a.silhouette_size_prior)
        for tid, fit_person in fits.items():
            entry = doc['perPerson'][tid]
            entry['constantUnits'] = entry['manifestTranslation']
            entry.update({k:v for k,v in fit_person.items() if k not in ('contactResidualUnits', 'edgeResidualsPx')})
            entry['solvedCm'] = 0.0
            doc['residualCm']['after'][tid] = stats_cm(fit_person['contactResidualUnits'], mpu)
            print(f"  SILHOUETTE {tid}: height {fit_person['heightRatioMedian']}, contact RMS "
                  f"{doc['residualCm']['after'][tid]['rms']:.2f} cm (fit residual, not held-out)", flush=True)
        if fits:
            ...
            doc['silhouetteFit'] = dict(masks=str(masks_path), sigmaPx=a.silhouette_sigma_px,
                                       edgeResidualPx=dict(median=float(np.median(edges)), p90=float(np.percentile(edges,90))),
            ...
            doc['noiseFloor']['refusals'] = [f'{tid}: fitted contact is below the contact noise floor'
                for tid, res in doc['residualCm']['after'].items() if res['rms'] < a.sigma_contact_cm]
            ...
            for phase in doc['residualCm'].values():
                for stat in phase.values():
                    for key in stat:
                        if key != 'n': stat[key] *= factor
            sc = doc['surface']['sizeCheck']
            sc['statureUnits'] *= primary_size
            for key in ('metresPerWorldUnit','cameraHeightM','floorToCeilingM'): sc[key] *= factor
            ct = sc['cameraHeightTrace']
            ct['metres'] = [v*factor for v in ct['metres']]
            for key in ('medianM','p10M','p90M','endToEndSlopeM'): ct[key] *= factor
            sc['pass_'] = 1.15 <= sc['cameraHeightM'] <= 1.85
            ct['drifted'] = abs(ct['endToEndSlopeM']) > .20
            ct['verdict'] = 'drift' if ct['drifted'] else 'flat in band' if sc['pass_'] else 'flat out of band'
            doc['silhouetteFit']['rulerNote'] = 'Metres assume the fitted primary body is 1.70 m; quote body-heights without an independent ruler.'

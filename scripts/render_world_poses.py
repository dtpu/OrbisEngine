...
try:
    ...
    def _composite(order, u, v, rad, a00, a01, a11, op, rgb, W, H, img, T):
        for k in range(order.shape[0]):
            ...
            r = int(rad[i])
            x0 = max(int(u[i]) - r, 0)
            x1 = min(int(u[i]) + r + 1, W)
            y0 = max(int(v[i]) - r, 0)
            y1 = min(int(v[i]) + r + 1, H)
            if x1 <= x0 or y1 <= y0:
                continue
            a = a00[i]
            b = a01[i]
...
def render(g, c2w, K, size, scale0, bg=0.0):
    ...
    u_, _, vt = np.linalg.svd(Rc)
    Rc = u_ @ vt  # re-orthonormalise
    ...
    _composite(
        ...
        a01,
        a11,
        g["alpha"][idx].astype(np.float32),
        g["rgb"][idx].astype(np.float32),
        W,
        H,
        img,
        T,
...
def main():
    ...
    for c in cameras_for(a.cameras, frames):
        ...
        K[2, 2] = 1.0
        img, alpha = render(
            g, np.array(c["camera_to_world"]), K, (a.width, int(round(sh * s))), a.scale0
        )

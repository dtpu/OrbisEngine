...
if (!out) throw new Error('Usage: bun scripts/capture-shared-demo.ts /absolute/evidence/directory');
...
const results = { status, clips: {} as Record<string, object> };
for (const clip of ['elevator', 'lobby', 'stairs2', 'atrium', 'tos31']) {
  ...
  try {
    ...
    page.on('response', (r) => {
      ...
      if (r.headers()['x-wander-asset-source'] === 's3') remote.push(r.url());
    });
    ...
    assert.ok(frame, 'Demo frame must be present');
    assert.equal(
      await frame.evaluate(() => window.wander.demo),
      clip,
      'Wrapper must load the requested preset',
    );
    ...
    const state = await frame.evaluate(() => ({
      people: window.wander.people.map((p) => ({ loaded: p.loaded, frames: p.nF })),
      walk: !!window.wander.walk,
    }));
    results.clips[clip] = {
      readySeconds,
      state,
    ...
    assert.ok(remote.some((u) => u.includes('/worlds/')));
    assert.ok(remote.some((u) => u.includes('.spz')));

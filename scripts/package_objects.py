...
`--from-rigid-object` converts the latter, and docs/objects.md §6 is the field map.
"""
...
def load_cameras(path):
    ...
    for c in cams:
        ...
        u, _, vt = np.linalg.svd(m); R.append(u @ vt)
    R = np.array(R)
...
def from_rigid_object(path, mpu, last_source_frame=None):
    ...
    Field map: docs/objects.md §6. The old format's `attachment.transform` is 16 column-major
    ...
    seg = dict(kind='attached', fromSourceFrame=0, toSourceFrame=last_source_frame,
               parent=None, joint=None, jointName=o['attachment'].get('anchor'),
    ...
    m = o.get('measured', {})
    return dict(
        id=o.get('label', 'object'), label=o.get('label', 'object'),
        prompt=o.get('prompt', ''),
        motion='attached' if kind in ('worn', 'carried') else 'free',
        objectClass=kind,
        appearance=dict(kind='gaussians', model=os.path.join(os.path.basename(path), 'object.ply'),
                        modelSha256=o.get('model_sha256'), gaussians=o.get('gaussians'),
                        frame=o.get('frame', ''),
                        ...
                        invented=False),
        pose=dict(segments=[seg], orientation=dict(mode='fixed', provenance='inherited from the anchor frame')),
        evidence=dict(silhouetteIouFixed=o['attachment'].get('silhouette_iou_after_refine'),
                      medianCentroidReprojectionPx=o['attachment'].get('median_centroid_reprojection_px'),
                      usableFrames=m.get('usable_frames'), fusedFrames=m.get('fused_frames'),
...
def seam_sizes(flights, nsrc, blend_frames):
    ...
    for i, fl in enumerate(flights):
        ...
        out.append((int(max(1, min(blend_frames, (fr - prev_fc) // 2))),
...
def main():
    ...
    ap.add_argument('--fit', required=True, help='wander.object-fit/2 (N flights) or /1 (one flight)')
    ...
    ap.add_argument('--holder', type=int, default=None,
    ...
    ap.add_argument('--size-m', default='0.060,0.120,0.060',
    ...
    ap.add_argument('--prompt', default='', help='the word that segments this object; see object_segment.py')
    ap.add_argument('--motion', default='', choices=['', 'attached', 'free', 'handoff', 'worldDynamic'],
    ...
    ap.add_argument('--model', default='', help='appearance PLY, copied into <world>/objects/<id>/ '
                                                'and referenced relative to objects.json')
    ap.add_argument('--model-frame', default='object-local; origin at the model centroid; +y is the long axis')
    ...
    ap.add_argument('--flights-json', default='', help='detect_object_flights.py output, recorded as evidence')
    ap.add_argument('--from-rigid-object', default='',
    ...
    if fit.get('schema') == 'wander.object-fit/2':
        ...
        fr, fc = fit['flight']
        flights = [dict(flight=[int(fr), int(fc)], thrower=a.thrower, catcher=a.catcher,
                        fits=fit['fits'], frames=fit['frames'], spanM=fit['spanM'],
                        flightSec=fit['flightSec'], constantY=fit.get('constantY', 0.0))]
    for fl in flights:
        if fl.get('thrower') is None or fl.get('catcher') is None:
            raise SystemExit('flight %s has no thrower/catcher; pass --thrower/--catcher' % fl['flight'])
    ...
    hands = {t: joint_world(os.path.join(a.tracks_dir, f'track_{t:02d}'), C, scale[t], a.joint)
             for t in by_track}
    ...
    def arc_of(fl):
        ...
        p0 = np.array(b['p0']); v0 = np.array(b['v0']); fr = fl['flight'][0]
    ...
    d_rel = [arcs[i](flights[i]['flight'][0]) - hand_corr(flights[i]['thrower'], flights[i]['flight'][0])
             for i in range(len(flights))]
    d_cat = [arcs[i](flights[i]['flight'][1]) - hand_corr(flights[i]['catcher'], flights[i]['flight'][1])
             for i in range(len(flights))]
    ...
    raw = np.array([pos_corrected(float(f)) - np.array([0.0, drift_at(float(f)) + obj_const, 0.0])
                    for f in frames])

    size = [float(v) for v in a.size_m.split(',')]
    col = [float(v) for v in a.color_srgb.split(',')]

    appearance = dict(kind='proxy', shape='capsule', axis='local +y',
                      why=fit.get('appearanceNote', ''))
    if a.model:
        ...
        if os.path.abspath(a.model) != os.path.abspath(dest):
            with open(a.model, 'rb') as srcf, open(dest, 'wb') as dst:
                dst.write(srcf.read())
        appearance = dict(kind='gaussians', model=f'objects/{a.id}/object.ply',
                          modelSha256=sha256(dest), gaussians=ply_gaussian_count(dest),
                          frame=a.model_frame)
    appearance.update(
        sizeMetres=size, sizeWorldUnits=[v / mpu for v in size],
        colorSRGB=col, opacity=a.opacity, translucent=True,
        shapeProvenance=a.shape_provenance, colourProvenance=a.colour_provenance,
    ...
    if not ofiles:
        orients = [dict(mode='velocityAligned', spinRevPerSec=0.0,
                        provenance='no rotation was measured')] * max(len(flights), 1)
    elif len(ofiles) == 1 and len(flights) > 1:
        o = json.load(open(ofiles[0])); o.pop('perpendicularExtentPx', None)
        o = dict(o, appliedTo='every free span',
                 provenanceCaveat=('this rate was measured on ONE free span and applied to all %d; '
                                   'the other spans\' tumble is assumed, not measured' % len(flights)))
        orients = [o] * len(flights)
    else:
        orients = []
        for p in ofiles:
            o = json.load(open(p)); o.pop('perpendicularExtentPx', None)
            orients.append(o)
    ...
    for i, fl in enumerate(flights):
        ...
        p0 = np.array(best['p0']); v0 = np.array(best['v0'])
        if fr - b_in > cursor:
            segments.append(dict(kind='attached', fromSourceFrame=int(cursor), toSourceFrame=int(fr - b_in),
                                 parent=by_track[holder_at(cursor)]['id'], joint=a.joint,
        ...
        segments.append(dict(kind='blend', fromSourceFrame=int(fr - b_in), toSourceFrame=int(fr),
                             from_='attached', to='free', offsetWorldUnits=d_rel[i].tolist(),
        ...
        seg = dict(kind='free', model='ballistic', fromSourceFrame=int(fr), toSourceFrame=int(fc),
                   p0WorldUnits=p0.tolist(), v0WorldUnitsPerSec=v0.tolist(),
                   gWorldUnitsPerSec2=g.tolist(), t0SourceFrame=int(fr), fps=30.0,
                   space='driftCorrected', flightIndex=i,
                   throwerTrack=int(fl['thrower']), catcherTrack=int(fl['catcher']),
                   orientation=orients[i],
                   evidence=dict(tracked2dFrames=len(fl.get('frames', [])),
        ...
        segments.append(seg)
        segments.append(dict(kind='blend', fromSourceFrame=int(fc), toSourceFrame=int(fc + b_out),
                             from_='free', to='attached', offsetWorldUnits=d_cat[i].tolist(),
                             offsetMetres=float(np.linalg.norm(d_cat[i]) * mpu)))
        cursor = int(fc + b_out)
        ev_flights.append(seg['evidence'] | dict(freeSegmentSourceFrames=[int(fr), int(fc)],
                                                 throwerTrack=int(fl['thrower']),
                                                 catcherTrack=int(fl['catcher'])))
    if cursor <= nsrc - 1:
        segments.append(dict(kind='attached', fromSourceFrame=int(cursor), toSourceFrame=int(nsrc - 1),
                             parent=by_track[holder_at(cursor)]['id'], joint=a.joint, jointName=a.joint_name,
                             evidence=('object visible in that hand in the source frames'
                                       if not flights else
                                       'after the last detected throw; assumed held, not tracked')))

    motion = a.motion or ('handoff' if len(flights) > 1 else 'handoff' if flights else 'attached')
    obj = dict(
        id=a.id, label=a.label, prompt=a.prompt, motion=motion, objectClass=a.object_class,
        appearance=appearance,
        pose=dict(orientation=orients[0] if flights else
                  dict(mode='fixed', provenance='no free span; orientation is not measured'),
                  segments=segments),
        ...
        evidence=dict(
            ...
            attachedSourceFrames=int(nsrc - sum(fl['flight'][1] - fl['flight'][0] + 1 for fl in flights)),
            ...
            reprojectionPxRms=round(max([fl['fits']['handAnchored']['reprojPxRms']
                                         for fl in flights] or [0.0]), 3),
            reprojectionPxMax=round(max([fl['fits']['handAnchored']['reprojPxMax']
                                         for fl in flights] or [0.0]), 3),
            reprojectionPxRmsPerFlight=[round(float(fl['fits']['handAnchored']['reprojPxRms']), 3)
                                        for fl in flights],
            reprojectionPxMaxPerFlight=[round(float(fl['fits']['handAnchored']['reprojPxMax']), 3)
                                        for fl in flights],
            ...
            attachedProvenance=('every attached span is an ASSUMPTION: the detector only sees the object '
    ...
    obj['bakedTrack'] = dict(
        ...
        fps=30.0, sourceFrames=frames.tolist(),
        sampleIndex=(frames / (pj['sourceIndices'][-1] / (pj['samples'] - 1))).round(4).tolist(),
        positions=np.round(raw, 5).tolist(),
        quaternionsXYZW=np.round(
            bake_orientation(frames, raw, orients[0] if flights else {}, obj['pose']['segments']), 5).tolist(),
        visible=[True] * len(frames))

    out = dict(schema='wander.objects/2',
               clip=pj.get('clip'), fps=pj['fps'], samples=pj['samples'],
               sourceIndices=pj['sourceIndices'], timestamps=pj['timestamps'],
               metresPerWorldUnit=mpu,
               coordinates=('Raw SfM world, OpenGL, camera 0 = identity - the same frame the person '
               ...
               people='people.json', objects=[obj])
    ...
    print('%d free spans, %d segments, object constantY %.4f u' % (len(flights), len(segments), obj_const))
    for i, fl in enumerate(flights):
        print('  flight %d f%d-%d  track %d -> %d  release %.3f m  catch %.3f m  reproj %.2f px'
              % (i, fl['flight'][0], fl['flight'][1], fl['thrower'], fl['catcher'],
                 np.linalg.norm(d_rel[i]) * mpu, np.linalg.norm(d_cat[i]) * mpu,

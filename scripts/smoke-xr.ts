...
try {
  ...
  page.on('console', (m) => {
    if (m.type() === 'error') errs.push('c: ' + m.text());
  });
  await page.goto(`${B}/fourd.html?demo=tos31&xr=1&fakexr=1&xradapt=0`, {
    waitUntil: 'load',
    timeout: 120000,
  });
  ...
  console.log(
    ...
    JSON.stringify(
      await page.evaluate(() => ({
        btn: document.getElementById('xrBtn')!.style.display,
        hasBinding: typeof XRWebGLBinding,
        fake: !!window.__fakeXR,
      })),
    ),
  );
  ...
  const post = await page.evaluate(() => ({
    ...
    canvas: [
      window.wander.spark.renderer.domElement.width,
      window.wander.spark.renderer.domElement.height,
    ],
    camParent: window.wander.camera.parent?.name ?? null,
    xr: window.__xr,
    hud: document.getElementById('xrHud')!.textContent,
  }));

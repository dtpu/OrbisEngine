...
"""What is one world unit in metres, measured from something that is not the avatar?
  ...
  worker/.venv-da3/bin/python scripts/world_ruler.py --clip gym
  worker/.venv-da3/bin/python scripts/world_ruler.py --clip elevator --vlm --json-out share/ruler-elevator.json
  worker/.venv-da3/bin/python scripts/world_ruler.py --clip gym --rotfix --vlm --vlm-cache share/vlm-gym.json
...
CLIPS = {
    ...
    "elevator": ("public/marble-elevator-clean.spz", "public/worlds/elevator-4d", "public/clips/elevator.mp4"),
    "lobby":    ("public/marble-lobby-clean.spz",  "public/worlds/lobby-4d",    "public/clips/lobby.mp4"),
    "living":   ("public/marble-living-finetuned.spz", "public/worlds/living-4dpp", "public/clips/living.mp4"),
    "atrium":   ("public/marble-atrium-clean.spz", "public/worlds/atrium-4dpp", "public/clips/atrium.mp4"),
    "stairs2":  ("public/marble-stairs2-clean.spz", "public/worlds/stairs2-4d", "public/clips/stairs2.mp4"),
    "tos31":    ("public/marble-tos31-image.spz",  "public/worlds/tos31-4d",    "public/clips/tos31.mp4"),
    "selfie":   ("public/marble-selfie-finetuned.spz", "public/worlds/selfie-4d", "public/clips/selfie.mp4"),
    "bedroom":  ("public/marble-bedroom-clean.spz", "public/worlds/bedroom-4d",  "public/clips/bedroom.mp4"),
}
...
PRESET_SCALE0 = {"gym": 1.0020, "elevator": 1.0800, "lobby": 1.5300, "atrium": 1.0221,
                 "living": 0.3750, "stairs2": 0.9760, "tos31": 1.2834, "selfie": 1.4560,
                 "bedroom": 0.673}
...
def resolve_paths(clip_name, world=None, depth=None, cameras=None, video=None):
    ...
    if world is None:
        if legacy:
            ...
            candidates = [ROOT / "public" / f"marble-{clip_name}-{suffix}.spz"
            ...
            if world is None:
                ...
                if len(matches) != 1:
                    raise ValueError(f"cannot choose a Marble world for {clip_name}: "
    ...
    if cameras is None:
        if depth is not None:
            ...
            cameras = (wdir if legacy else ROOT / ".context" / "run" / clip_name / "pi3x") / "cameras.json"
...
def predict_depths(cams, clip: str, model_id=None, device=None, cache: Path | None = None):
    ...
    for cam in cams:
        ...
        if cp and cp.exists():
            z = np.load(cp)
            out[fi] = (cam, z["depth"].astype(np.float32),
    ...
    if want:
        if depthpro:
            ...
            model = DepthProForDepthEstimation.from_pretrained(
                model_id, dtype=torch.float16).eval().to(device)
        else:
        ...
        for cam, rgb, cp in want:
            ...
            inp = {k: (v.to(device).half() if depthpro and v.dtype == torch.float32 else v.to(device))
            ...
            if depthpro:
                ...
                dm = torch.nn.functional.interpolate(o.predicted_depth[None].float(),
                                                    size=rgb.shape[:2], mode="bicubic",
                                                    align_corners=False)[0, 0].cpu().numpy()
            out[cam["sourceIndex"]] = (cam, dm, fov)
            if cp:
                cp.parent.mkdir(parents=True, exist_ok=True)
                np.savez_compressed(cp, depth=dm.astype(np.float16),
                                    fov=np.array(np.nan if fov is None else fov))
    return [out[c["sourceIndex"]] for c in cams if c["sourceIndex"] in out]


def anchor_metric_depth(world_spz: Path, cams, clip: str, scale0: float, frames: list[int],
                        person_masks=None, ds=4, device=None, model_id=None,
                        depth_cache: Path | None = None) -> dict:
    import cv2
    ...
    for cam, dm, fov in predict_depths(cams, clip, model_id, device, depth_cache):
        ...
        if person_masks is not None and fi in person_masks:
            pm = cv2.resize(person_masks[fi].astype(np.uint8), (gw, gh),
                            interpolation=cv2.INTER_NEAREST).astype(bool)
            good &= ~pm
        if good.sum() < 500:
            continue
        r = np.log(dm_s[good] / dw[good])
        row = dict(sourceIndex=fi, n=int(good.sum()),
        ...
        if fov is not None:
            ...
            row.update(fovDeg=round(fov, 2), focalPredictedPx=round(f_pred, 1),
    ...
    return dict(available=True, mpu=float(np.median(vals)), frames=len(vals),
                spreadPct=float(100 * (vals.max() - vals.min()) / np.median(vals)),
                model=model_id, perFrame=per_frame,
                focalAgreement=(round(float(np.median(fa)), 3) if fa else None),
                focalWarning=(None if not fa or abs(float(np.median(fa)) - 1) < 0.12 else
                              f"the model's own field of view implies a focal "
...
def anchor_vlm(cams, clip: str, world_spz: Path, scale0: float, model="gpt-4o",
               cache: Path | None = None, world=None) -> dict:
    ...
    for cam in cams:
        ...
        if key in store:
            ...
            if not ok:
                continue
            b64 = base64.b64encode(cv2.imencode(".jpg", bgr, [cv2.IMWRITE_JPEG_QUALITY, 92])[1]).decode()
            req_body = dict(model=model, response_format=dict(type="json_object"),
            ...
            req = urllib.request.Request(
                "https://api.openai.com/v1/chat/completions", data=json.dumps(req_body).encode(),
        ...
        for it in (doc.get("items") or []):
            ...
            row = dict(sourceIndex=fi, object=it.get("object"), dimension=it.get("dimension"),
                       metres=it.get("metres"), confidence=it.get("confidence"),
                       p1=it.get("p1"), p2=it.get("p2"), spanPx=round(span_px, 1),
                       why=it.get("why"))
            ...
            if d_fp and d_fp > 1e-6:
                row.update(depthUnits=round(float(z), 4), worldUnitsFrontoParallel=round(d_fp, 4),
            ...
            if row.get("mpu") and row.get("mpuTwoRay"):
                ...
                if not (1 / VLM_RAY_TOL < r < VLM_RAY_TOL):
                    row["rejected"] = (f"the two readings of this object differ {abs(r - 1) * 100:.0f}%"
                                       f" -- an endpoint did not land on it")
            if span_px < VLM_MIN_SPAN_PX:
                row["rejected"] = (f"{span_px:.0f} px across: too few pixels to place the endpoints"
                                   f" to better than the tolerance")
            items.append(row)
    if cap is not None:
        cap.release()
    good = [i["mpu"] for i in items
            if i.get("mpu") and not i.get("rejected") and 0.05 < i["mpu"] < 60]
    if not good:
        return dict(available=False, items=items,
                    reason=("the VLM named nothing the world could measure"
                            if items else "the VLM named nothing measurable in the world"))
    return dict(available=True, mpu=float(np.median(good)), n=len(good), items=items,
                model=model,
...
def anchor_gravity(fit_json: Path | None, mpu_person: float) -> dict:
    ...
    if sg:
        out.update(sigma=float(sg),
                   lo=float(mpu_person * 9.80665 / (float(g) + 2 * float(sg))),
                   hi=float(mpu_person * 9.80665 / max(float(g) - 2 * float(sg), 1e-6)))
    out["note"] = ("the arc is fitted in metres with g = 9.81 and the metre is solved for; a wide "
...
def fit_scale0(world_spz: Path, cams, clip: str, mpu_person_ref: float, scale0_ref: float,
               person_masks=None, ds=4, device=None, model_id=None, span=(0.25, 4.0), steps=41,
               depth_cache: Path | None = None) -> dict:
    ...
    xyz, alpha = read_spz_xyz(world_spz)
    preds = [(cam, dm) for cam, dm, _ in
             predict_depths(cams, clip, model_id or METRIC_MODEL, device, depth_cache)]
    if not preds:
    ...
    for s0 in np.geomspace(span[0], span[1], steps):
        ...
        curve.append(dict(scale0=round(float(s0), 4), mpuWorld=round(mw, 4),
                          mpuPerson=round(mp, 4), logRatio=round(float(np.log(mw / mp)), 4)))
    if len(curve) < 3:
    ...
    return dict(available=True, fittedScale0=best, referenceScale0=scale0_ref,
...
def fit_scale0_vlm(world_spz: Path, cams, clip: str, mpu_person_ref: float, scale0_ref: float,
                   model: str, cache: Path | None, span=(0.25, 4.0), steps=41) -> dict:
    ...
    best = None
    if len(sign):
        i = int(sign[0]); w = lr[i] / (lr[i] - lr[i + 1])
        best = float(np.exp(np.log(sc[i]) + w * (np.log(sc[i + 1]) - np.log(sc[i]))))
    return dict(available=True, fittedScale0=best, referenceScale0=scale0_ref,
                ratioToReference=(round(best / scale0_ref, 3) if best else None), curve=curve,
...
def combine(anchors: dict, bands: dict, tol: float) -> dict:
    ...
    if not pts:
        out.update(ok=False, weaklyAnchored=True, adopted=None,
    ...
    for i, a in enumerate(names):
        for b in names[i + 1:]:
            r = pts[a] / pts[b]
            if abs(r - 1.0) > tol:
                disagree.append(dict(a=a, b=b, ratio=round(float(r), 4),
...
def person_masks_for(clip_name: str, ds_shape=None):
    ...
    if not p.exists():
        ...
        for wd in (ROOT / ".context" / "pose" / "masks").glob(f"{clip_name}-4d*.npz"):
            ...
            out = {int(i): cv2.dilate(m[k].astype(np.uint8), np.ones((9, 9), np.uint8)).astype(bool)
    ...
    return {k: cv2.dilate(v.astype(np.uint8), np.ones((9, 9), np.uint8)).astype(bool)
...
def main():
    ...
    ap.add_argument("--clip", required=True, help="clip name in run_clip.py's layout, or a historical clip")
    ...
    ap.add_argument("--cameras", type=Path, help="override cameras.json (takes precedence over --depth)")
    ...
    ap.add_argument("--stature-units", type=float,
    ...
    ap.add_argument("--fit-scale0", action="store_true",
    ...
    if not a.no_metric_depth:
        anchors["metricDepth"] = anchor_metric_depth(
            Path(world), pick, str(clip), scale0, [c["sourceIndex"] for c in pick],
            person_masks=person_masks_for(a.clip), ds=a.ds,
            device=a.metric_device, model_id=a.metric_model, depth_cache=a.depth_cache)
    if a.vlm:
        anchors["vlmObject"] = anchor_vlm(pick, str(clip), Path(world), scale0, a.vlm_model,
                                          cache=a.vlm_cache)
    anchors["gravity"] = anchor_gravity(a.object_fit, mpu_person or 1.0)
    sweep = vsweep = None
    if a.fit_scale0 and mpu_person:
        if not a.no_metric_depth:
            sweep = fit_scale0(Path(world), pick, str(clip), mpu_person, scale0,
                               person_masks=person_masks_for(a.clip), ds=a.ds,
                               device=a.metric_device, model_id=a.metric_model,
                               depth_cache=a.depth_cache)
        if a.vlm:
            vsweep = fit_scale0_vlm(Path(world), pick, str(clip), mpu_person, scale0,
                                    a.vlm_model, a.vlm_cache)

    sc = (place.get("surface") or {}).get("sizeCheck") or {}
    stature = a.stature_units or sc.get("statureUnits")
    cam_units = (sc.get("cameraHeightM") / mpu_person) if (sc.get("cameraHeightM") and mpu_person) else None
    cam_span = None
    if fa is not None:
    ...
    doc = dict(clip=a.clip, world=str(world), scale0=scale0, mpuPerson=mpu_person,
               rotfix=(None if fa is None else dict(
                   applied=True, tiltFromYDeg=fa["tiltFromYDeg"], source=fa["source"],
                   cameraHeightUnits=cam_units, cameraHeightSpanUnits=cam_span)),
               anchors=anchors, bands={k: dict(lo=v[0], hi=v[1], valueUnits=v[2], unit=v[3])
                                       for k, v in bands.items()},
               verdict=verdict, scale0Fit=sweep, scale0FitVlm=vsweep)
    if mpu_person and verdict.get("adopted"):
        r = verdict["adopted"] / mpu_person
        doc["personVsWorld"] = dict(ratio=round(float(r), 4), offBy=f"{abs(r - 1) * 100:.1f}%",

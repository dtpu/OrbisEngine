...
"""Ask MultiHMR whether the scorer's top reference candidates can actually be used.
  ...
  KMP_DUPLICATE_LIB_OK=TRUE uv run --locked --group inference python scripts/probe_pose_estimator.py \
...
downstream build can still refuse the winner: `worker/stages/lhm_person.py` runs MultiHMR on
...
DEFAULT_MODAL = os.environ.get(
    "WANDER_MODAL", shutil.which("modal") or str(ROOT / ".venv/bin/modal")
)
...
def main():
    ...
    ap.add_argument(
        "--scores", required=True, help="score_reference_frames.py output for this clip"
    )
    ap.add_argument("--work", required=True, help="where the candidate cut-outs are written")
    ap.add_argument("--out", required=True)
    ap.add_argument("--top", type=int, default=8)
    ap.add_argument(
        "--also", default="", help="extra frame indices to probe (e.g. the frame in use today)"
    )
    ...
    ap.add_argument(
        "--track-motion",
        help="one track's motion.json: use THAT identity's own box in "
    ...
    for i in candidates(scores, a.top, extra):
        ...
        if not (d / "source.png").exists():
            ...
            if rep.get("empty") or not rep["portrait"]:
                ...
                skipped[str(i)] = dict(
                    detected=False, score=0.0, reason=f"unusable person mask: {rep}"
                )
                continue
            Image.fromarray(rgb).save(d / "source.png")
            Image.fromarray(m).save(d / "mask.png")
        reqs.append(
            dict(clip=name, index=i, source=str(d / "source.png"), mask=str(d / "mask.png"))
        )
    ...
    if reqs:
        cmd = [
            a.modal,
            "run",
            str(ROOT / "worker" / "modal_pose_probe.py"),
            "--requests",
            str(req_path),
            "--out",
            str(a.out),

...
def bands(P0):
    y = P0[:, 1]
    q = np.quantile(y, [0, 0.2, 0.45, 0.7, 0.88, 1.0])
    lab = [
        "low 20% (feet/shins)",
        "20-45% (legs)",
        "45-70% (hips/torso)",
...
if __name__ == "__main__":
    ...
    ap.add_argument("--to-m", type=float, default=1.0)
    ...
    print(
        f"  centroid accel  mean {np.nanmean(m['acc_centroid']):8.2f}  p95 {np.nanpercentile(m['acc_centroid'], 95):8.2f} m/s^2"
    )
    print(
        f"  per-splat accel mean {np.nanmean(m['acc_splat']):8.2f}  p95 {np.nanpercentile(m['acc_splat'], 95):8.2f} m/s^2"
    )
    print(
        f"  HF residual RMS mean {1000 * m['hf'].mean():8.1f}  p95 {1000 * np.percentile(m['hf'], 95):8.1f} mm"
    )
    rng = np.linalg.norm(m["centroid"] - m["centroid"][0], axis=-1)
    print(
        f"  root travel max {rng.max():.3f} m ; bbox y extent {(P[0][:, 1].max() - P[0][:, 1].min()) * a.to_m:.3f} m"
    )
    for lab, mask in bands(P[0]):
        mb = metrics(P[:, mask], seq["fps"], a.to_m)
        print(
            f"    {lab:24s} accel {np.nanmean(mb['acc_splat']):7.2f} m/s^2   HF {1000 * mb['hf'].mean():6.1f} mm"
        )

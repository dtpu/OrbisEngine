...
after them. (--marble image inpaints frame 0 on its own instead, which unblocks Marble in ~83 s.)
  ...
  WLT_API_KEY=... uv run --locked --group inference python scripts/run_clip.py --clip public/clips/bedroom.mp4 --name bedroom \
...
MODAL = os.environ.get("WANDER_MODAL", shutil.which("modal") or str(ROOT / ".venv/bin/modal"))
...
MARBLE_DIR = (
    Path(os.environ.get("WANDER_MARBLE_DIR", ROOT / ".context/marble")).expanduser().resolve()
)
LOCAL_ENV = {
...
def world_half(marble: str):
    ...
    deps["review"] = list(stages[:-1])
    if marble in ("image", "both"):
        stages.append("marble_image")
        deps["marble_image"] = ["clean_first", "review"]
    if marble in ("video", "both"):
        stages.append("marble_video")
        deps["marble_video"] = ["clean", "review"]
    if marble == "multi":
        stages.append("marble_multi")
        deps["marble_multi"] = ["clean_multi", "review"]
    world = {"multi": "marble_multi", "video": "marble_video", "both": "marble_video"}.get(
        marble, "marble_image"
    )
    ...
    deps.update(
        {
            ...
            "frame_align": ["pi3x"],
            "person_prep": [],
            "lhm_frozen": ["person_prep"],
            "lhm_motion": ["lhm_frozen", "pi3x"],
            "package": ["lhm_motion", "frame_align"],
        }
    )
    if objects:
        stages += ["objects"]
...
def parse_object_spec(spec: str) -> dict:
    ...
    roi is `joint=TRACK,JOINT,RADIUS` or `box=x0,y0,x1,y1`.
    Omitted means the whole frame, which is right
    for something that fills a decent part of it and wrong for anything small -- see
    the Current pipeline section in docs/objects.md.
    """
    parts = spec.split(":", 3)
    if len(parts) < 3:
    ...
    if motion not in ("attached", "free", "handoff", "worldDynamic"):
        raise SystemExit(
            f"--object motion must be attached|free|handoff|worldDynamic, got {motion!r}"
        )
...
class Pipeline:
    def __init__(
        self, a, clip: Path | None = None, name: str | None = None, shot: dict | None = None
    ):
        self.a = a
        ...
        if not self.clip.is_relative_to(public):
            ...
            if (
                not dest.exists()
                or dest.stat().st_size != self.clip.stat().st_size
                or dest.stat().st_mtime_ns != self.clip.stat().st_mtime_ns
            ):
        ...
        self.stages, self.deps = (
            multiperson_graph(self.people_limit, self.marble, want_objects)
            if self.multi
            else single_graph(self.marble, want_objects)
        )
        self.certs = subprocess.run(
            [PY, "-m", "certifi"], check=True, stdout=subprocess.PIPE, text=True
        ).stdout.strip()
        self.state.record(
            "_run",
            clip=str(self.clip),
    ...
    def track_or_skip(self, idx):
        ...
        if ident.exists() and track["track"] in (
    ...
    def clean_first(self):
        ...
        run(
            [
                ...
                "0",
                "--frames-out",
                str(self.first_png.parent),
                "--report",
                str(self.ctx / "clean-first.json"),
                *self.clean_flags(),
            ],
    ...
    def clean(self):
        run(
            [
                ...
                str(self.clean_mp4),
                "--frame0",
                str(self.ctx / "clean-f0.png"),
                "--masks",
                str(self.ctx / "masks.npz"),
                "--report",
                str(self.ctx / "clean.json"),
                *self.clean_flags(),
            ],
            self.ctx / "clean.log",
        )
    ...
    def review(self):
        ...
        if report.exists():
            ...
            say(
                f"   clean: {r.get('frames')} frames, person mask {100 * r.get('maskFraction', 0):.1f}% "
        ...
        for s in shots:
            say(f"     {s}")
        say(
            "   look for: the person fully gone; nothing of his left hanging in mid-air (a bag, a "
        ...
        if not self.a.gate_pass:
            raise GateStop(
                "re-run with --gate-pass once you have looked at the frames above "
    ...
    def marble_submit(self, input_type, target, suffix, extra=None) -> str:
        ...
        try:
            run(
                ...
                log,
                {"WLT_API_KEY": self.key, "SSL_CERT_FILE": self.certs},
                attempts=1,
            )
    ...
    def marble_poll(self, input_type, op_id: str, suffix):
        ...
        for attempt in range(self.a.poll_attempts):
            try:
                ...
                last = e
                say(
                    f"   poll {op_id} attempt {attempt + 1}/{self.a.poll_attempts} failed; retrying in 60s"
                )
                time.sleep(60)
        raise RuntimeError(
    ...
    def marble_reuse(self, input_type, suffix):
        ...
        if not self.key:
            raise RuntimeError(
                f"--reuse-world needs ${self.a.marble_key} to fetch the world metadata"
            )
        run(
            [
                PY,
    ...
    def marble_world(self, input_type, target, suffix, extra=None):
        ...
        if op:
            self.marble_poll(input_type, op, suffix)
    ...
    def marble_image(self):
        ...
        if self.prompt_json.exists():
            extra = [
                "--prompt",
                json.loads(self.prompt_json.read_text())["text_prompt"],
                "--seed",
                "7",
            ]
    ...
    def world_mode(self, decide_only=False):
        ...
        if cam.exists():
            ...
            say(
                "   no camera poses and no coarse solve: cannot measure the mode, leaving --marble as given"
            )
            return None
    ...
    def marble_multi(self):
        ...
        self.marble_world(
            ...
            None,
            "multi",
            extra=["--images", *images, "--prompt-file", str(self.prompt_json), "--seed", "7"],
        )
    ...
    def pose_guard(self):
        ...
        if p.returncode == 4:
            doc = json.loads((self.ctx / "pose-guard.json").read_text())
            raise RuntimeError(
                f"the pose guard could not check the solve: {doc.get('reason')}. "
                f"An unchecked solve is a failed solve here -- hp33 wrote no "
                f"cameras.json and would otherwise sail through. Re-run pi3x."
            )
        if p.returncode != 0:
            raise RuntimeError(
                f"the pose guard itself failed (exit {p.returncode}); the solve is "
    ...
    def tracks(self):
        ...
        run(
            [
                ...
                str(self.clip),
                "--cameras",
                str(self.ctx / "pi3x" / "cameras.json"),
                "--fps",
                str(self.a.fps),
                "--det-thresh",
                str(self.a.det_thresh),
                "--out",
                str(self.ctx / "tracks"),
            ],
            self.ctx / "tracks.log",
        ...
        doc = self.track_doc()
        say(
            f"   {doc['trackCount']} person track(s): "
            + ", ".join(
    ...
    def identity_guard(self):
        ...
        if p.returncode == 3:
            ...
            if failed and len(failed) < len(doc.get("tracks", [])) and not doc.get("reason"):
                ...
                say(
                    f"   identity: track(s) {failed} changed person and will be skipped; "
                    f"{[t for t in doc['tracks'] if t not in failed]} held"
                )
                return
            raise RuntimeError(
                "the identity audit says a track changed person. Do not build avatars "
                f"from these tracks; see {self.ctx / 'identity.json'}."
            )
        if p.returncode == 4:
            raise RuntimeError(
                f"the identity audit could not run; see {self.ctx / 'identity.json'}"
            )
        if p.returncode != 0:
    ...
    def person_prep(self, idx=None):
        if idx is None:
            frame = (
                self.a.person_frame if self.a.person_frame is not None else self.info["frames"] // 2
            )
            run(
                [
                    PY,
                    "scripts/prepare_lhm_person.py",
                    str(self.clip),
                    "--frame",
                    str(frame),
                    "--method",
                    self.a.person_method,
                    "--out",
                    str(self.ctx / "prepared-person"),
                ],
        ...
        run(
            [
                PY,
                "scripts/prepare_track_person.py",
                str(self.ctx / "tracks"),
                "--track",
                str(idx),
                "--clip",
    ...
    def lhm_motion(self, idx=None):
        if idx is None:
            ...
            run(
                [
                    ...
                    "worker/modal_lhm.py",
                    "--canonical",
                    str(self.ctx / "lhm-frozen"),
                    "--video",
                    str(self.clip),
                    "--cameras",
                    str(self.ctx / "pi3x" / "cameras.json"),
                    "--out",
                    str(self.ctx / "lhm-motion"),
                ],
        ...
        run(
            [
                ...
                str(self.clip),
                "--canonical",
                str(self.ctx / f"lhm-frozen-{idx:02d}"),
                "--cameras",
                str(self.ctx / "pi3x" / "cameras.json"),
                "--track-dir",
                str(self.ctx / "tracks" / f"track_{idx:02d}"),
                "--track-id",
                str(idx),
                "--depth-roi",
                roi,
    ...
    def package(self):
        ...
        run(
            [
                PY,
                "scripts/package_person_sequence.py",
                str(self.ctx / "lhm-motion"),
                "--cameras",
                str(self.ctx / "pi3x" / "cameras.json"),
                "--clip",
    ...
    def package_people(self):
        ...
        run(
            [
                PY,
                "scripts/package_multiperson.py",
                *motions,
                "--tracks",
                str(self.ctx / "tracks"),
                "--cameras",
            ...
            if motion in ("free", "handoff"):
                ...
                if not t2.exists():
                    run(
                        [
                            PY,
                            "scripts/track_object_2d.py",
                            "--clip",
                            str(self.clip),
                            "--tracks",
                            str(self.ctx / "tracks" / "tracks.json"),
                            "--out",
                            str(t2),
                            "--patches",
                            str(odir / "patches.npz"),
                        ],
                        odir / "track2d.log",
                    )
                fit = odir / "fit3d.json"
                if not fit.exists():
                    run(
                        [
                            PY,
                            "scripts/lift_object_3d.py",
                            "--track2d",
                            str(t2),
                            "--cameras",
                            str(self.cameras()),
                            "--people",
                            str(world / "people.json"),
                            "--tracks-dir",
                            str(self.ctx / "tracks"),
                            "--thrower",
                            "1",
                ...
                if not ori.exists():
                    ...
                    run(
                        [
                            PY,
                            "scripts/object_orientation.py",
                            "--track2d",
                            str(t2),
                            "--patches",
                            str(odir / "patches.npz"),
                            "--v0",
                            ",".join(f"{v:.6f}" for v in f["v0"]),
                            "--g",
                            f"0,-{json.loads(fit.read_text())['gravityUnitsPerS2']:.6f},0",
                            "--out",
                            str(ori),
                        ],
                        odir / "orientation.log",
                    )
    ...
    def scale_file(world: Path):
        ...
        for n, keys in (
            ("placement.json", ("metresPerWorldUnit",)),
            ("collision.json", ("metresPerWorldUnit",)),
    ...
    def objects_auto(self, world: Path):
        ...
        if scale is None:
            raise SkipStage(
                "no metre scale on disk for this world yet (place_fit has not run), "
        ...
        if not flights.exists():
            cmd = [
                PY,
                "scripts/detect_object_flights.py",
                "--clip",
                str(self.clip),
                "--tracks",
                str(tracks),
        ...
        say(
            ...
            % (
                len(fl),
                ", ".join(
                    f"f{r['frames'][0]}-{r['frames'][-1]} ({r['nObs']} obs, {r['speedMs']:.1f} m/s)"
                    for r in fl
                ),
            )
        )
        if not multi:
            ...
            say(
                "   flights recorded, not lifted: the single-person layout carries no floor fit or "
        ...
        if not fj["flights"]:
            ...
            say(
                "   %d flight(s) detected, none reached a tracked wrist at both ends: "
                "%s. Not packaged"
                % (len(fl), "; ".join(d["rejected"] for d in fj["droppedFlights"]))
            )
            return
        ...
        dd = dj["description"]
        say(
            f"   it looks like: {dd['label']} (confidence {dd.get('confidence', 0):.2f}), "
            ...
            try:
                run(
                    [
                        ...
                        "0.85",
                        "--note",
                        "auto path: crop picked by detect_object_flights.py sharpness, prompt "
                ...
                run(
                    [
                        PY,
                        "scripts/object_appearance.py",
                        "--in",
                        str(shape / "object.ply"),
                        "--out",
                        str(model),
                        "--largest-cluster",
                        "--budget",
        ...
        pkg = [
            ...
            str(flights),
            "--size-m",
            ",".join(f"{v:.4f}" for v in dj["sizeMetres"]),
            "--color-srgb",
            ",".join(f"{v:.3f}" for v in dd["colourSRGB"]),
            "--colour-provenance",
        ...
        if model.exists():
            ...
            pkg += [
                "--shape-provenance",
                "proxy capsule sized from the tracklet's apparent scale at the "
        ...
        self.state.record(
            ...
            ids=[oid],
            flights=len(fl),
            lifted=len(fj["flights"]),
            world=str(world / "objects.json"),
            label=dd["label"],
        )
        say(
            f"   packaged '{oid}' ({dd['label']}): {len(fj['flights'])} flight(s) -> {world / 'objects.json'}"
        )
    ...
    def frame_align(self):
        ...
        self.state.record(
            ...
            cameraHeightUnits=fa["cameraHeightUnits"],
            file=str(out),
        )
        say(
            f"   camera 0 is {fa['tiltFromYDeg']:.1f} deg off gravity ({fa['source']}); estimators "
    ...
    def diag(self, scale0: float, probe: int):
        ...
        rows = [
            ...
            for m in re.findall(
                r"\(ratio ([0-9.]+)\), p10 ([0-9.]+) p90 ([0-9.]+)", log.read_text()
            )
        ]
        if not rows:
    ...
    def scale_fit(self):
        ...
        if s is None:
            ...
            for i in range(self.a.scale_probes):
                ...
                probes.append((s, gm))
                say(
                    f"   probe {i}: scale0 {s:.4f} -> ratios {[round(r, 3) for r in ratios]} "
                    f"(geomean {gm:.4f})"
                )
                ...
                raise RuntimeError(
                    f"scale0 did not converge in {self.a.scale_probes} probes "
        ...
        self.state.record(
            ...
            p90=p90,
            probes=probes,
            gateOk=not spread and not wide,
            medianOk=not spread,
            spreadOk=not wide,
            figure=str(fig),
        )
        say(
            f"   fitted scale0 {s:.4f}, ratios {[round(r, 3) for r in ratios]}, geomean {gm:.4f}, "
        ...
        if spread or wide:
            say(
                f"   !! --allow-scale-spread: shipping the geomean fit; medians "
    ...
    def place_fit(self):
        ...
        cmd = [
            PY,
            "scripts/place_solve.py",
            "--clip",
            self.name,
            "--world",
            str(self.world_spz()),
            "--cameras",
            str(wdir / "cameras.json"),
            "--scale",
            f"{s:.6f}",
            "--floor",
            f"{floor:.6f}",
        ...
        self.state.record(
            ...
            out=str(out),
            samples=doc["samples"],
            metresPerWorldUnit=doc["metresPerWorldUnit"],
            sizeCheck=sc,
            residualCm=res,
            reprojectionCostPx=doc["reprojectionCostPx"],
        )
        say(
            f"   1 u = {doc['metresPerWorldUnit']:.3f} m; source camera "
        ...
        for pid in res["before"]:
            say(
                f"   {pid}: contact residual {res['before'][pid]['rms']:.1f} -> "
    ...
    def anchors(self):
        ...
        cmd = [
            PY,
            "scripts/anchor_checks.py",
            "--clip",
            self.name,
            "--tol",
            str(self.a.ruler_tol),
            "--json-out",
            str(out),
        ]
        if self.a.ruler_vlm:
        ...
        if p.returncode == 3:
            ...
            raise RuntimeError(
                f"anchor checks failed: {', '.join(doc.get('failed', ['unknown']))}. "
    ...
    def finetune(self):
        """Export fitted camera/Pi3X anchors, train once on the configured host, and render figures.

        The generic exporter uses cameras.json and Pi3X anchors. Shipping still requires a human
        review of off-path figures; this stage stops after training and producing that evidence.
        """
        if not self.a.gpu_box:
            raise RuntimeError(
                "Fine-tuning requires --gpu-box or WANDER_GPU_BOX; use --skip-finetune to omit it"
            )
        ...
        run(
            [
                ...
                f"{s:.6f}",
                "--mask-dilate",
                str(self.a.ft_mask_dilate),
                "--out",
                str(export),
            ],
            self.ctx / "finetune_export.log",
            attempts=1,
        )
        ...
        say(
            f"   {n} cameras, 1 native unit = {m_per_unit:.3f} m -> clamp {clamp:.3f} sigma "
        ...
        run(
            ["rsync", "-az", "-e", " ".join(ssh), f"{export}/", f"{box}:{remote}/"],
            self.ctx / "finetune_rsync.log",
        )
        run(
            ["rsync", "-az", "-e", " ".join(ssh), "scripts/finetune_gsplat.py", f"{box}:{remote}/"],
        ...
        run(
            [
                *ssh,
                box,
                f"cd {remote} && ~/venv/bin/python finetune_gsplat.py "
        ...
        run(
            ["rsync", "-az", "-e", " ".join(ssh), f"{box}:{remote}/run5/", str(runs) + "/"],
            self.ctx / "finetune_fetch.log",
        )
        run(
            [
                PY,
                "scripts/finetune_figures.py",
                "--run",
                str(runs),
                "--data",
                str(export),
                "--tag",
                f"finetune-{self.name}",
                "--poses",
                poses,
                "--offpath-frame",
                str(mid),
                "--world-label",
                f"Marble {self.name}-clean (as is)",
            ],
            self.ctx / "finetune_figures.log",
        )
        run(
            [
                PY,
                "scripts/finetune_import.py",
                "--npz",
                str(runs / "finetuned.npz"),
                "--world",
                str(self.world_spz()),
                "--out",
                str(out),
            ],
            self.ctx / "finetune_import.log",
        )
        stats = runs / "stats.json"
        if stats.exists():
            d = json.loads(stats.read_text())
            say(
                f"   held-out PSNR {d.get('psnr_before')} -> {d.get('psnr_after')}, "
    ...
    def m_per_unit(self) -> float:
        ...
        m = re.search(
            r"1 native unit = ([0-9.]+) m", (self.ctx / "finetune_export.log").read_text()
        )
        if not m:
            raise RuntimeError(
                "the exporter did not print its metres-per-native-unit estimate; "
    ...
    def placement(self) -> dict:
        ...
        for suffix in ("clean", "image", "multi"):
            ...
            if msf:
                out.update(
                    metricScaleFactor=msf,
                    groundPlaneOffset=gpo,
                    height=1.7 * msf,
                    marbleFloor=-gpo / msf,
    ...
    def summary(self, wall):
        ...
        print("\n" + "=" * 78)
        print(
            f"{self.name}: {self.info['duration']:.1f} s source, {self.info['frames']} frames "
        ...
        for s in self.stages:
            ...
            if st:
                print(
                    f"{s:<18}{st.get('seconds', 0):>9.1f}  {st.get('status')}"
                    + (
                        f"  {st.get('error', '')[:44]}"
                        if st.get("status") in ("failed", "gate")
                        else ""
                    )
                )
        ...
        world = "<world>.spz"
        for cand in (
            f"marble-{self.name}-finetuned.spz",
        ...
        if p.get("floor") is not None:
            ...
            if p.get("scaleGateOk") is False:
                print(
                    f"  CAVEAT: depth ratios {[round(r, 3) for r in p['depthRatios']]} -- the "
            ...
            print(
                "  UNFITTED: that height is 1.7 x Marble's metric_scale_factor, which has been "
    ...
    def go(self):
        a = self.a
        named = (set(a.only.split(",")) if a.only else set()) | (
            set(a.force.split(",")) if a.force else set()
        )
        unknown = sorted(named - set(self.stages))
        if unknown:  # never silently drop a stage the caller asked for
            sys.exit(
                f"unknown stage(s) {unknown} for --marble {self.marble}"
        ...
        say(
            f"marble mode: {self.marble}"
            + (
                f" (reusing world {a.reuse_world}, no credits)"
                if a.reuse_world
                else ", ONE generation = 1600 credits"
                if self.marble in ("video", "image")
                else ", TWO generations = 3200 credits"
                if self.marble == "both"
                else ""
            )
        )
        ...
        def launch(stage):
            def body():
                ...
                try:
                    ...
                    vacant.add(stage)
                    self.state.record(
                        stage, status="skipped", seconds=time.time() - start, error=str(e)
                    )
        ...
        def status(dep):
            ...
            if (
                dep in vacant
                or self.vacant(dep)
                or self.state.data["stages"].get(dep, {}).get("status") == "skipped"
            ):
                return "ok"
            if dep in gated:
                return "gate"
        ...
        while pending or running:
            for stage in sorted(pending):
                ...
                if "gate" in states:
                    ...
                    gated.add(stage)
                    self.state.record(
                        stage, status="gate", seconds=0.0, error="waiting on the clean-review gate"
                    )
...
def resolve_shots(a) -> list[tuple[str, Path, dict | None]]:
    ...
    if a.one_shot:
        say(
            f"   --one-shot: processing the whole clip across {doc['cutCount']} detected cut(s) "
    ...
    if a.all_shots:
        ...
        if "best" not in doc:
            raise SystemExit(
                f"{doc['cutCount']} cut(s) and no shot long enough to reconstruct "
        ...
        say(
            f"   {doc['cutCount']} cut(s) at {[c['time'] for c in doc['cuts']]} s. NOT processing "
            f"across them."
        )
        say(
            f"   chose shot {doc['best']} ({chosen[0]['start']:.2f}-{chosen[0]['end']:.2f} s): "
            f"{doc['bestWhy']}"
        )
        say(
...
def main():
    ...
    ap.add_argument(
        "--scale-probes", type=int, default=5, help="depth-ratio probes for the scale fit"
    )
    ap.add_argument(
        "--ruler-tol",
        type=float,
    ...
    ap.add_argument(
        "--people", type=int, help="like --all-people with an explicit cap on the person count"
    )
    ap.add_argument(
        "--det-thresh",
        type=float,
    ...
    ap.add_argument(
        ...
        help="configured fine-tune host (or WANDER_GPU_BOX)",
    )
    ap.add_argument(
        "--gpu-key",
        default=os.environ.get("WANDER_GPU_KEY"),
        help="optional SSH identity (or WANDER_GPU_KEY)",
    )
    ap.add_argument(
        "--no-publish",
        action="store_true",
    ...
    try:
        ...
        if not a.no_publish:
            ...
            command = ["bun", "scripts/publish-runs.mjs"]

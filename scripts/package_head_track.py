...
orientation, never across a gap in visibleSampleRuns. Raw joints otherwise: nothing is re-fitted.
  ...
  uv run --locked --group inference python scripts/package_head_track.py --world elevator-4d --world hpwide-4d
"""
...
def main():
    ...
    ap.add_argument(
        "--world", action="append", required=True, help="public/worlds/<world>, repeatable"
    )
    ...
    for world in a.world:
        ...
        pj_path = os.path.join(wdir, "people.json")
        ...
        for p in pj["people"]:
            ...
            if not os.path.exists(pt):
                print(f"{world}/{p['id']}: no {pt}, skipped", file=sys.stderr)
                continue
            ps = torch.load(pt, map_location="cpu", weights_only=False)
            ok = [
                k for k, q in enumerate(ps["poses"]) if q is not None and q.get("j3d") is not None
            ]  # a sample with no fit is skipped, not invented
            pidx = np.asarray(ps["sourceIndices"], float)[ok]
            J = np.array(
                [np.asarray(ps["poses"][k]["j3d"]) for k in ok]
            )  # (n, 127, 3) camera space
            scale = p["registrationScale"] * p.get("sizeCorrection", {}).get("multiplier", 1.0)
            sf = np.array(p["sourceIndices"], float)
            n = len(sf)
            ...
            fwd = np.zeros((n, 3))
            for k in range(n):
                up = unit(head[k] - neck[k])
                right = rsh[k] - lsh[k]
                right = unit(right - (right @ up) * up)
                back = unit(np.cross(right, up))
                fwd[k] = -back
            ...
            runs = [
                (max(0, int(r[0]) - f0), min(n - 1, int(r[1]) - f0))
                for r in p.get("visibleSampleRuns") or [[f0, f0 + n - 1]]
            ]
            ...
            for q0, q1 in runs:
                visible[q0 : q1 + 1] = True
            eye_ok = (
                eyes is not None
                and 0.03 * scale
                <= np.median(np.linalg.norm(eyes - head, axis=1)) / 1.0
                <= 0.25 * scale
            )
            out = dict(
                schema="wander.head/1",
                person=p["id"],
                track=p["track"],
                fps=pj["fps"],
                space="raw SfM world, identical to the person PLYs before transform.translation (the objects.json bakedTrack contract)",
                sourceFrames=[int(v) for v in sf],
                sampleIndex=list(range(n)),
                ...
                scale=scale,
                rawHeadRmsFromSmoothedUnits=float(np.sqrt(((head - head_s) ** 2).sum(1).mean())),
            )
            ...
            print(
                f"{world}/{p['id']} (track {p['track']}): {n} samples from {len(ok)}/{len(ps['poses'])} fits, head y {head[:, 1].min():.3f}..{head[:, 1].max():.3f} u, "

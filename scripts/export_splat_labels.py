...
Outputs, next to the .spz (public/*.bin is gitignored):
    ...
    python scripts/export_splat_labels.py elevator lobby
    python scripts/export_splat_labels.py elevator --cluster-class door --render-dir share/labels
"""
import argparse
import json
import subprocess
...
def vote(xyz_raw, cams, scale, clip, a, seg_h, device):
    ...
    for b0 in range(0, len(cams), a.batch):
        ...
        print(f'  frames {min(b0 + a.batch, len(cams))}/{len(cams)}  {time.time() - t0:.0f}s', flush=True)
...
def cluster_class(P, sel, radius_units):
    ...
    for k in order[:10]:
        m = comp == k + 1
        clusters.append(dict(splats=int(sizes[k]), fraction=float(sizes[k] / len(Q)),
                             bboxMin=Q[m].min(0).tolist(), bboxMax=Q[m].max(0).tolist()))
    return dict(splats=int(len(Q)), voxelUnits=float(radius_units), nClusters=int(nlab),
                bboxMin=Q.min(0).tolist(), bboxMax=Q.max(0).tolist(), clusters=clusters)
...
def add_legend(ax, top_ids, names, counts, extra):
    ...
    handles = [Line2D([], [], marker='s', ls='', ms=9, color=PALETTE[k],
                      label=f'{names[cid]} ({counts[cid]:,})') for k, cid in enumerate(top_ids)]
...
def render_camera_view(out, xyz_w, labels, cams, scale, clip, a, seg_h, names, counts, top_ids, device):
    ...
    sel, idx = front_surface(u, v, z, ok, gw, gh, a.tol)
    sel = sel.cpu().numpy()
    canvas = np.ones((gh, gw, 3), dtype=np.float32) * np.array([int(SURFACE[i:i + 2], 16) for i in (1, 3, 5)]) / 255
    zz = z[sel].cpu().numpy()
    ...
    axs[0].imshow(img); axs[0].set_title(f'source frame {cam["sourceIndex"]}', fontsize=9)
    axs[1].imshow(tab[lab2d]); axs[1].set_title('SegFormer-b0 ADE20K, 2D', fontsize=9)
    axs[2].imshow(canvas, interpolation='nearest'); axs[2].set_title(f'splat labels, front surface from camera 0 ({len(sel):,} splats)', fontsize=9)
    for ax in axs:
        ax.set_xticks([]); ax.set_yticks([])
        for s in ax.spines.values():
            s.set_visible(False)
    add_legend(axs[2], top_ids, names, counts, [('other classes', OTHER), ('unobserved (255)', UNSEEN)])
    fig.tight_layout()
    fig.savefig(out, dpi=150, facecolor=SURFACE)
    plt.close(fig)


def render_top_down(out, xyz_w, labels, cams, scale, a, names, counts, top_ids, ceiling_id, metres_per_unit):
    ...
    ax.scatter(Q[:, 0], Q[:, 2], c=tab[labels[idx]], s=0.4, marker='.', linewidths=0, rasterized=True)
    ...
    ax.set_aspect('equal'); ax.set_facecolor(SURFACE)
    ax.set_xlabel(f'x (world units, {metres_per_unit:.2f} m/unit)', fontsize=8); ax.set_ylabel('z (world units)', fontsize=8)
    ax.tick_params(labelsize=7, colors='#52514e')
    for s in ax.spines.values():
        s.set_color('#d9d8d4')
    ax.set_title(f'top-down, {len(idx):,} of {len(xyz_w):,} splats, ceiling class hidden; dot = camera 0', fontsize=9)
    add_legend(ax, top_ids, names, counts, [('other classes', OTHER), ('unobserved (255)', UNSEEN), ('camera path', '#0b0b0b')])
    fig.tight_layout()
    fig.savefig(out, dpi=150, facecolor=SURFACE)
    plt.close(fig)
...
def main():
    ...
    ap.add_argument('clips', nargs='+', help='clip names; world, cameras, scale and clip resolve from public/worlds/<clip>-4d/')
    ...
    ap.add_argument('--clip', help='override the source clip (default: cameras.json sourceClip, else public/clips/<clip>.mp4)')
    ...
    ap.add_argument('--seg-width', type=int, default=960, help='frame width fed to the segmenter (it resizes to 512 internally)')
    ap.add_argument('--batch', type=int, default=8)
    ap.add_argument('--occluders', type=int, nargs='*', default=[12], help='ADE20K ids whose pixels vote for nobody (default person)')
    ap.add_argument('--occluder-dilate', type=int, default=8, help='px at --seg-width')
    ap.add_argument('--min-votes', type=int, default=2, help='splats with fewer votes are written as 255')
    ...
    ap.add_argument('--cluster-class', help='ADE20K class name to cluster spatially and report (e.g. door)')
    ap.add_argument('--cluster-radius-m', type=float, default=0.10, help='voxel size for connectivity, metres')
    ap.add_argument('--render-dir', type=Path, help='write <clip>-labels-cam0.png and <clip>-labels-topdown.png here')
    ap.add_argument('--render-count', type=int, default=400000, help='max splats drawn in the top-down render')
    a = ap.parse_args()
    ...
    for clip_name in a.clips:
        ...
        clip = Path(a.clip) if a.clip else Path(camj.get('sourceClip') or ROOT / 'public' / 'clips' / f'{clip_name}.mp4')
        if not clip.exists():
        ...
        print(f'{clip_name}: {spz.name} scale {scale} {len(cams)} frames from {clip.name} on {a.device}', flush=True)
        ...
        classes = [dict(id=int(c), name=names[int(c)], splats=int(counts[c]), shareOfLabelled=float(counts[c] / max(labelled.sum(), 1)),
                        pixelShare=float(pixel_counts[c] / pixel_counts.sum()))
                   for c in order if counts[c] > 0]
        never_fired = [names[c] for c in range(N_CLASSES) if pixel_counts[c] == 0]
        fired_no_splats = [dict(name=names[c], pixelShare=float(pixel_counts[c] / pixel_counts.sum()))
                           for c in range(N_CLASSES) if pixel_counts[c] > 0 and counts[c] == 0]
        report = dict(
            schema='wander.labels/1', clip=clip_name, world=str(spz.relative_to(ROOT)), cameras=str((wdir / 'cameras.json').relative_to(ROOT)),
            sourceClip=str(clip), scale=scale, metresPerWorldUnit=place.get('metresPerWorldUnit'),
            segmenter='nvidia/segformer-b0-finetuned-ade-512-512', frames=len(cams), splats=n,
            params=dict(ds=a.ds, tol=a.tol, near=a.near, segWidth=a.seg_width, segHeight=seg_h, occluders=a.occluders,
                        occluderDilatePx=a.occluder_dilate, minVotes=a.min_votes),
            votesThreshold=a.min_votes,
            observed=dict(count=int((total > 0).sum()), fraction=float((total > 0).mean()), note='at least one front-surface vote'),
            labelled=dict(count=int(labelled.sum()), fraction=float(labelled.mean()), note=f'>= {a.min_votes} votes; the rest are 255'),
            unobserved=dict(count=int((~labelled).sum()), fraction=float((~labelled).mean())),
            votesPerLabelledSplat=dict(median=float(np.median(total[labelled])) if labelled.any() else 0, max=int(total.max())),
            occludedVotesDropped=int(occluded),
            purity=dict(note='top-class votes / all votes, over labelled splats', mean=float(purity.mean()) if labelled.any() else 0,
        ...
        print(f'  observed {report["observed"]["fraction"] * 100:.1f}%  labelled {report["labelled"]["fraction"] * 100:.1f}%  '
        ...
        if a.cluster_class:
            rep = report['cluster']
            print(f'  {a.cluster_class}: {rep["splats"]:,} splats, {rep.get("nClusters", 0)} components at {a.cluster_radius_m} m, '
        ...
        if a.render_dir:
            ...
            render_camera_view(a.render_dir / f'{clip_name}-labels-cam0.png', xyz_w, labels, cams, scale, clip, a, seg_h,
                               names, counts, top_ids, a.device)
            render_top_down(a.render_dir / f'{clip_name}-labels-topdown.png', xyz_w, labels, cams, scale, a, names, counts,
                            top_ids, ceiling_id, float(place.get('metresPerWorldUnit') or 1.0))

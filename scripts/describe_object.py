...
sys.path.insert(0, os.path.join(ROOT, 'worker', 'experiments'))
...
def size_estimate(fl, cameras):
    ...
    for r in fl['flights']:
        ...
        per.append(dict(flight=r['frames'][:1] + r['frames'][-1:], depthM=z, focalPx=f,
    ...
    return dict(perFlight=per,
                ...
                rule=('bbox side in px x fitted depth / focal, per observation; the MIN over a flight is '
...
def main():
    ...
    ap.add_argument('--out', required=True, help='description.json; best-crop.png is written beside it')
    ...
    ap.add_argument('--no-vlm', action='store_true', help='size and crops only; label falls back to generic')
    a = ap.parse_args()
    ...
    desc = dict(label='small thrown object', category='other', segmentWord='a small thrown object',
                refinePrompt='a small handheld object', colourSRGB=None, longOverShort=None,
                translucent=None, confidence=0.0, notes='VLM not consulted')
    if not a.no_vlm:
    ...
    out = dict(schema='wander.object-description/1', flights=a.flights,
               cropsUsed=[dict(flight=i, **c) for i, c in picks],
               description=desc, size=size, sizeMetres=[float(v) for v in size_m],
               sizeRule=('[short, long, short] in the object frame where +y is the long axis; short from the '
    ...
    print(json.dumps(dict(label=desc['label'], category=desc['category'], sizeMetres=out['sizeMetres'],
                          confidence=desc['confidence'], crops=[c['file'] for _, c in picks]), indent=1))
    print('wrote', a.out)

...
Appearance preparation described in docs/objects.md. Geometry may come from several sources --
...
def main():
    ...
    ap.add_argument(
        "--budget", type=int, default=6000, help="Gaussian count after decimation; 0 keeps all"
    )
    ...
    if a.largest_cluster and len(xyz):
        ...
        if n > 1:
            per = lab[g[:, 0], g[:, 1], g[:, 2]]
            counts = np.bincount(per, minlength=n + 1)
            counts[0] = 0
    ...
    rgb = np.clip(
        np.column_stack([v["f_dc_0"], v["f_dc_1"], v["f_dc_2"]]).astype(np.float64)[idx][pick],
        -2,
        2,
    )
    ...
    rep = dict(
        source=a.src,
        out=a.out,
        gaussiansIn=int(before),
        afterOpacityCut=int(len(idx)),
        written=int(len(pick)),
        longAxisSpanBefore=round(span, 6),
        radiusPlusY=round(r_hi, 5),
        radiusMinusY=round(r_lo, 5),

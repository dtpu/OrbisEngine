// Real desktop wrapper + scene runtimes. Requires the private elevator, plaza and accepted gym assets.
...
try {
  ...
  await page.click('.card[data-id="plaza"]');
  await ready('plaza');
  await frame.waitForFunction(
  ...
  await page.click('.card[data-id="gym-accepted"]');
  await stalled;
  await page.click('.card[data-id="elevator"]');
  await ready('elevator');

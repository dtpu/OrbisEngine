...
const W = 1280, H = 720, SAMPLE_MS = 100;

await mkdir(OUT, { recursive: true });
const browser = await chromium.launch({ channel: 'chrome', headless: true, args: ['--use-angle=metal', '--ignore-gpu-blocklist'] });
const state = () => { const w = window.wander, p = w.camera.position; return { x: +p.x.toFixed(3), y: +p.y.toFixed(3), z: +p.z.toFixed(3), floor: +w.walk.floor.toFixed(3), eyeRatio: +((p.y - w.walk.floor) / w.walk.stature).toFixed(3), cellFloor: w.walk.cellAt(p.x, p.z).floor, blockedUnder: w.walk.blockedAt(p.x, p.z) }; };
async function hold(page, code, ms, trace) {
  await page.keyboard.down(code); const t0 = performance.now();
  while (performance.now() - t0 < ms) { await page.waitForTimeout(SAMPLE_MS); trace.push({ t: +((performance.now() - t0) / 1000).toFixed(2), ...await page.evaluate(state) }); }
...
try {
  const ctx = await browser.newContext({ viewport: { width: W, height: H }, deviceScaleFactor: 1 });
  {
    ...
    const r = { errors, grid: await page.evaluate(() => window.wander.walk.grid), stepUp: await page.evaluate(() => +window.wander.walk.stepUp.toFixed(3)), upm: await page.evaluate(() => +window.wander.upm.toFixed(4)), start: await page.evaluate(state), trace: [] };
    ...
    r.rise = { units: +(r.end.floor - r.start.floor).toFixed(3), metres: +((r.end.floor - r.start.floor) / r.upm).toFixed(2),
      distinctFloorLevels: new Set(floors.map(f => f.toFixed(2))).size, largestJump: +Math.max(...floors.slice(1).map((f, i) => f - floors[i])).toFixed(3), everDescended: floors.slice(1).some((f, i) => f < floors[i] - 1e-3) };
    out.stairs2 = r; await page.close();
  }
  {
    const { page, errors } = await open(ctx, `${BASE}/fourd.html?demo=elevator&walk=1`);
    const r = { errors, grid: await page.evaluate(() => window.wander.walk.grid), upm: await page.evaluate(() => +window.wander.upm.toFixed(4)), start: await page.evaluate(state), fwd: [], left: [], right: [] };
    ...
    r.rowBlocked = await page.evaluate(() => { const w = window.wander, p = w.camera.position, g = w.walk.grid, row = []; for (let x = -2.5; x <= 3; x += g.cell) { const c = w.walk.cellAt(x, p.z); row.push(w.walk.blockedAt(x, p.z) ? '#' : Number.isNaN(c.floor) ? (c.inside ? '~' : ' ') : '.'); } return { from: -2.5, cell: +g.cell.toFixed(4), z: +p.z.toFixed(2), row: row.join(''), legend: '# footprint blocked at the walker height, . known floor, ~ unknown inside, space outside' }; });
    ...
    const settled = (tr, k) => { const last = tr.slice(-10).map(s => s[k]); return +(Math.max(...last) - Math.min(...last)).toFixed(4); };
    r.stops = { leftX: r.afterLeft.x, leftMovingOverLastSecond: settled(r.left, 'x'), rightX: r.afterRight.x, rightMovingOverLastSecond: settled(r.right, 'x') };
    out.elevator = r; await page.close();
  }

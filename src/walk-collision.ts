...
export interface WalkCellSupport {
  floor: number;
  inside: number;
  dist: number;
  solid: number;
...
/** A finite explicit walking surface contributes support beyond the sampled floor hull.
 * Occupied cells remain occupied; collision and step-height checks still apply separately.
 */
export function withExplicitWalkSupport(
  cell: WalkCellSupport,
  support: number | undefined,
): WalkCellSupport {
  if (support === undefined || !Number.isFinite(support)) return cell;
  return {
    ...cell,
    floor: Number.isFinite(cell.floor) ? Math.max(cell.floor, support) : support,
    inside: 1,
    dist: 0,
  };
}

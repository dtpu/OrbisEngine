...
def main(requests: str, out: str, det_thresh: float = 0.3):
    ...
    items = [
        dict(
            clip=r["clip"],
            index=int(r["index"]),
            source=Path(r["source"]).read_bytes(),
            mask=Path(r["mask"]).read_bytes(),
        )
        for r in reqs
    ]
    ...
    for r in res["results"]:
        by.setdefault(r["clip"], {})[str(r["index"])] = {
            k: v for k, v in r.items() if k not in ("clip", "index")
        }
    doc = dict(
        method=res["method"],
        detThreshold=res["detThreshold"],
        seconds=res["seconds"],
        gpu=res["gpu"],
        codeRevision=res["codeRevision"],
        clips=by,
    )

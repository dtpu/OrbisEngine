...
modal run worker/modal_motion.py --experiment vdpm --video clip.mp4 --out .context/vdpm
modal run worker/modal_motion.py --experiment pi3x --video clip.mp4 --out .context/dense
...
image = (
    ...
    .pip_install(
        ...
        "pillow", "opencv-python-headless==4.11.0.86", "plyfile",
        "huggingface_hub", "safetensors", "scipy", "einops",
        "transformers==4.46.3", "omegaconf", "roma", "timm",
    )
    .run_commands(
    ...
    .env({"PYTHONPATH": "/root/worker:/opt/vggt", "HF_HUB_ENABLE_HF_TRANSFER": "0", "PYTHONUNBUFFERED": "1",
    ...
    .add_local_dir(HERE / "experiments", "/root/worker/experiments", ignore=["__pycache__"])
)
app = modal.App("wander-overnight-motion")
cache = modal.Volume.from_name("wander-overnight-motion-cache", create_if_missing=True)


@app.function(image=image, gpu="L4", cpu=4, memory=32768, timeout=1500,
              scaledown_window=2, max_containers=1, retries=0,
              volumes={"/cache": cache})
def reconstruct(video_bytes: bytes, experiment: str, batch: int = 16) -> dict:
    ...
    try:
        if experiment == "vdpm":
            ...
            command = [sys.executable, "/root/worker/experiments/vdpm_person.py",
                       "--video", str(source), "--out", str(out), "--repo", "/opt/vdpm",
                       "--views", "16", "--targets", "0"]
        elif experiment == "pi3x":
            command = [sys.executable, "/root/worker/experiments/dense_pi3x.py",
                       "--video", str(source), "--out", str(out), "--repo", "/opt/pi3",
                       "--fps", "12", "--anchors", "8", "--batch", str(batch)]
        else:
    ...
    report = dict(experiment=experiment, inferenceWallSeconds=inference_wall, error=error,
                  gpu="L4", cpu=4, memoryGiB=32, timeoutSeconds=1500,
    ...
    with tarfile.open(fileobj=buf, mode="w:gz") as archive:
        ...
        report.update(seconds=elapsed, estimatedComputeUSD=elapsed * (0.000222 + 4 * 0.0000131 + 32 * 0.00000222))

...
def main():
    ...
    for name in ('prepared','out','model','model-id','model-revision'):ap.add_argument('--'+name,required=True)
    ap.add_argument('--repo',default='/opt/lhm');a=ap.parse_args()
    prepared=Path(a.prepared);out=Path(a.out);out.mkdir(parents=True,exist_ok=True)
    os.chdir(a.repo);sys.path.insert(0,a.repo);started=time.time()
    import torch
    ...
    Image.fromarray((image[0].permute(1,2,0).numpy()*255).astype('uint8')).save(out/'model-input.png')
    for name in ('source-pose.pt','source-pose.json','head-input.png'):shutil.copyfile(prepared/name,out/name)
    ...
    torch.cuda.synchronize();xyz=gaussians.xyz.detach().float().cpu().numpy()
    sha=lambda p:hashlib.sha256(p.read_bytes()).hexdigest()
    result=dict(model=a.model_id,modelRevision=a.model_revision,codeRevision='4f88aaeb3629249fbbddb4d0784a06962d9e1338',
        seconds=time.time()-started,peakVRAMGB=torch.cuda.max_memory_allocated()/1e9,gpu=torch.cuda.get_device_name(),torch=torch.__version__,
        points=len(xyz),bounds=np.percentile(xyz,[2,50,98],axis=0).tolist(),sourceIntrinsics=pose_record['sourceIntrinsics'],
        prepared=json.loads((prepared/'prepared.json').read_text()),
        sourceImageSha256=sha(prepared/'source.png'),maskSha256=sha(prepared/'mask.png'),sourcePoseSha256=sha(prepared/'source-pose.pt'),
        headInputSha256=sha(prepared/'head-input.png'),modelInputPreviewSha256=sha(out/'model-input.png'),
        modelInputTensorSha256=hashlib.sha256(image.numpy().tobytes()).hexdigest(),headDetected=bool(np.any(head)),
        ...
        nativeCoordinates='OpenCV source camera: x right, y down, z forward.',coverage='Learned body prior; hidden identity remains unverified inference.')

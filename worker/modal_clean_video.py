...
itself is uploaded and decoded remotely, so no frame dumps cross the wire.
  ...
  modal run worker/modal_clean_video.py --clip public/clips/bedroom.mp4 \
      --out .context/clips/bedroom-clean.mp4 \
      --frame0 .context/own/bedroom/clean-f0.png --dilate 40 --bottom-extra 80

Weights live in the wander-clean-video-cache volume (big-lama.pt staged once with `modal volume put`,
...
image = (
    ...
    .pip_install(
        "torch==2.5.1", "numpy==1.26.4", "pillow==10.4.0",
        "opencv-python-headless==4.11.0.86", "transformers==4.46.3", "safetensors",
    )
    .pip_install("simple-lama-inpainting==0.1.2", extra_options="--no-deps")
    .env({"PYTHONPATH": "/root/worker", "PYTHONUNBUFFERED": "1",
          "HF_HOME": "/cache/huggingface", "TORCH_HOME": "/cache/torch", "LAMA_MODEL": LAMA_PT})
...
@app.function(image=image, gpu="L4", cpu=4, memory=32768, timeout=3600, retries=0,
              max_containers=2, volumes={"/cache": cache}, scaledown_window=2)
def clean(video_bytes: bytes, fps: float = 12.0, width: int = 1920, height: int = 1080,
          dilate: int = 20, bottom_extra: int = 40, passes: int = 1, lama_px: int = 960,
          crf: int = 18, only: str | None = None, mask_batch: int = 8,
          return_frames: bool = False, encode: bool = True, masks_npz: bytes = b"",
          moved: bool = False, max_added_frac: float = 0.05) -> dict:
    ...
    try:
        probe = json.loads(subprocess.run(
            ["ffprobe", "-v", "error", "-select_streams", "v:0", "-show_entries",
             "stream=avg_frame_rate,nb_frames,width,height,duration", "-of", "json", str(src)],
        ...
        t = time.time()
        raw = subprocess.run(["ffmpeg", "-loglevel", "error", "-i", str(src), "-vf",
        ...
        indices = [min(int(round(i * src_fps / fps)), int(probe.get("nb_frames") or 10 ** 9) - 1)
                   for i in range(n)] if src_fps else list(range(n))
        print(f"{n} frames decoded from {src_fps:.3f} fps source ({timings['decode']:.0f}s)", flush=True)
        ...
        if moved and not masks_npz:
            ...
            mres = moved_content_masks(imgs, dilate_px=dilate, batch_size=mask_batch,
            ...
            masks = np.unpackbits(np.load(io.BytesIO(masks_npz))["masks"], axis=-1)[:, :H, :W].astype(bool)
            timings["masks"] = time.time() - t
        else:
        ...
        if bottom_extra > 0 and not masks_npz:
            ...
            masks[need] = np.stack([cv2.dilate(m.astype(np.uint8), k, anchor=(dilate, bottom_extra)).astype(bool)
                                    for m in masks[need]])
        timings["masks"] = time.time() - t
        covered = int(masks[need].reshape(len(need), -1).any(1).sum())
        print(f"masks: people {masks[need].mean()*100:.2f}% of pixels, {covered}/{len(need)} frames "
              f"({timings['masks']:.0f}s)", flush=True)
        ...
        for i in sel:
            ...
            if m.any():
                for _ in range(passes):
                    ...
                    alpha = np.clip(cv2.GaussianBlur(m.astype(np.float32), (0, 0), 3)[..., None] * 1.5, 0, 1)
        ...
        buf = io.BytesIO()
        np.savez_compressed(buf, masks=np.packbits(masks, axis=-1), shape=np.array([n, H, W]),
        ...
        if encode and not only:
            ...
            subprocess.run(["ffmpeg", "-y", "-loglevel", "error", "-framerate", str(fps), "-i",
                            str(work / "f_%04d.png"), "-c:v", "libx264", "-crf", str(crf),
                            "-pix_fmt", "yuv420p", "-movflags", "+faststart", str(enc)], check=True)
        ...
        result = dict(frames=n, inpainted=len(kept), indices=indices, sourceFps=src_fps, moved=moved,
                      movedStats=moved_stats,
                      fps=fps, width=W, height=H, dilate=dilate, bottomExtra=bottom_extra,
                      passes=passes, lamaPx=lama_px, crf=crf, maskFraction=float(masks[need].mean()),
    ...
    report = dict(seconds=elapsed, error=error, gpu="L4", cpu=4, timings=timings,
                  estimatedComputeUSD=elapsed * (0.000222 + 4 * 0.0000131 + 32 * 0.00000222), **result)
...
def stage_weights() -> dict:
    ...
    cache.commit()
    return dict(lama=os.path.exists(LAMA_PT), lamaBytes=os.path.getsize(LAMA_PT) if os.path.exists(LAMA_PT) else 0,
                segformer=MODEL_ID)


@app.local_entrypoint()
def main(clip: str, out: str = "", frame0: str = "", masks: str = "", report: str = "",
         fps: float = 12.0, width: int = 1920, height: int = 1080, dilate: int = 20,
         bottom_extra: int = 40, passes: int = 1, lama_px: int = 960, crf: int = 18,
         only: str = "", frames_out: str = "", masks_in: str = "", moved_mask: bool = False,
         max_added_frac: float = 0.05):
    import io, json, tarfile
    r = clean.remote(Path(clip).read_bytes(), fps=fps, width=width, height=height, dilate=dilate,
                     bottom_extra=bottom_extra, passes=passes, lama_px=lama_px, crf=crf,
                     only=only or None, return_frames=bool(frames_out), encode=not only,
                     masks_npz=Path(masks_in).read_bytes() if masks_in else b"", moved=moved_mask,
                     max_added_frac=max_added_frac)
    rep = r["report"]
    if out and r["mp4"]:
    ...
    print(json.dumps({k: v for k, v in rep.items() if k not in ("indices", "movedStats")}, indent=2))
    if rep["error"]:

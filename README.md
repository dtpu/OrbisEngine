...
While a headset shares its view, the desktop demo's stage shows the headset's left-eye scene in
place of the desktop viewer, and returns to the desktop viewer when sharing stops. Open both
devices through the same local demo server, then enter VR on the Quest. The view follows head and
joystick movement, including the in-scene hands and body. It does not include Meta system menus or
audio. Capture pauses when no visible desktop tab is watching. The local relay retains only the
latest image in memory, with no recording. Capture and display target 60 frames per second at up
to 512 pixels; a badge shows the actual displayed rate. Speed depends on headset rendering, encoding
and the connection, and cannot exceed the headset's rendered frame rate. Capture overlaps the
preceding upload, with at most one following frame and no accumulating queue.
`xrview=0` disables sharing from the headset.
...
limits. `bun scripts/test-quest-stage-browser.ts` checks the stage view's connection and
visibility states. After building, `bun scripts/test-quest-view-browser.ts` checks real rendered
pixels through the encoder, relay and stage view with synthetic XR input on an isolated local server.
